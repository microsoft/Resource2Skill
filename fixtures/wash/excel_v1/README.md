...
* AC-2 (deterministic wash) — ``wiki-wash --frozen-fixtures
  fixtures/wash/excel_v1`` must report ``fixture_violations: 0`` on
  every machine.
* AC-3 (smoke fixtures) — paired with ``domains/excel/smoke_fixtures/``
...
```
python cli.py wiki-wash --domain excel \
    --emit-frozen-fixture-update fixtures/wash/excel_v1
```
