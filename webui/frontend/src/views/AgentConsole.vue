<template>
  <div>
    <el-card shadow="never" style="margin-bottom: 16px">
      ...
      <el-form label-width="72px">
        <el-form-item :label="t('任务')">
          <el-input v-model="taskText" type="textarea" :rows="4" :placeholder="t('用自然语言描述要 Agent 完成的任务，例如：为一款产品生成立项文档并产出阶段交付物')" style="max-width: 760px" />
        </el-form-item>
        <el-form-item :label="t('模型')">
          <el-input v-model="model" :placeholder="t('留空则用 LLM 模板模型')" style="width: 240px" />
        </el-form-item>
        <el-form-item :label="t('推理档位')">
          <el-select v-model="reasoning" :placeholder="t('留空则用 LLM 模板值')" style="width: 200px" clearable>
        ...
        <el-form-item :label="t('迭代上限')">
          <el-input-number v-model="maxIter" :min="1" :max="60" />
        </el-form-item>
        <el-form-item :label="t('参考技能数')">
          <el-input-number v-model="nSkills" :min="0" :max="20" />
        </el-form-item>
        <el-form-item :label="t('TopK')">
          <el-input-number v-model="topK" :min="1" :max="50" />
        </el-form-item>
        <el-form-item>
          <el-checkbox v-model="dryRun">{{ t('Dry-run（占位工具，不落盘交付物）') }}</el-checkbox>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :disabled="!ctx.state.activeDomain || !taskText || running" @click="onStart">{{ t('运行 Agent') }}</el-button>
          <span style="color: #909399; font-size: 12px; margin-left: 12px">{{ t('单 worker 串行执行，运行期间请勿再提交其它任务') }}</span>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <div v-if="extra && extra.tool_calls && extra.tool_calls.length" style="margin-top: 12px">
      <div style="font-weight: 600; margin-bottom: 8px">{{ t('工具调用记录') }}</div>
      <el-table :data="extra.tool_calls" size="small" border max-height="240">
        <el-table-column prop="tool" :label="t('工具')" width="220" />
        <el-table-column :label="t('结果')" width="90">
          <template #default="{ row }">
            <el-tag size="small" :type="row.success ? 'success' : 'danger'">{{ row.success ? 'OK' : 'ERR' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="args" :label="t('参数')" show-overflow-tooltip />
        <el-table-column prop="error" :label="t('错误')" show-overflow-tooltip />
      </el-table>
    </div>
    <el-empty v-if="!taskId" :description="t('选择项目/领域并填写任务后点击「运行 Agent」')" />
  </div>
</template>
...
async function onStart() {
  try {
    ...
    extra.value = null
    running.value = true
    ElMessage.success(t('已提交 Agent 任务'))
  } catch (e) {
