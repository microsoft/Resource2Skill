<template>
  <div>
    ...
    <el-card shadow="never" style="margin-bottom: 16px">
      <template #header>
        <div style="display: flex; align-items: center; justify-content: space-between">
          <span style="font-weight: 600">{{ t('项目（多项目隔离）') }}</span>
          <el-button type="primary" size="small" @click="openProject">{{ t('新建项目') }}</el-button>
        </div>
      </template>
      <el-table :data="ctx.state.projects" border style="width: 100%" @current-change="onRow">
        ...
        <el-table-column :label="t('领域数')" width="80">
    ...
    <el-card shadow="never">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between">
          <span style="font-weight: 600">
            ...
            <span v-if="current" style="color: #888; font-weight: 400"
              >{{ t('· 当前项目：') }}{{ current }}<template v-if="ctx.state.activeDomain">{{ t(' · 当前领域：') }}{{ ctx.state.activeDomain }}</template></span
            >
            <span v-else style="color: #c00; font-weight: 400">{{ t('（请先在上方选择或激活一个项目）') }}</span>
          </span>
          <el-button
            type="primary"
            size="small"
      ...
      <el-empty v-if="!current" :description="t('请先在上方选择或激活一个项目')" />
      <el-table v-else :data="domainRows" border style="width: 100%">
        <el-table-column prop="name" :label="t('领域')" width="160" />
        <el-table-column :label="t('激活')" width="70">
          <template #default="{ row }">
            <el-tag v-if="row.name === ctx.state.activeDomain" type="success">{{ t('是') }}</el-tag>
            <span v-else>{{ t('否') }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="t('校验结果')" width="120">
          <template #default="{ row }">
            <template v-if="row.result === undefined">
              <span style="color: #888">{{ t('未校验') }}</span>
            </template>
        ...
        <el-table-column :label="t('错误详情')" min-width="200">
          <template #default="{ row }">
            <span v-if="!row.result || row.result.valid" style="color: #888">{{ t('—') }}</span>
        ...
        <el-table-column :label="t('操作')" width="240">
          <template #default="{ row }">
            <el-button size="small" @click="onValidate(row)">{{ t('校验') }}</el-button>
            <el-button
              size="small"
              type="success"
    ...
    <!-- 新建项目对话框 -->
    <el-dialog v-model="projectDialog" :title="t('新建项目')" width="460px">
      <el-form :model="projectForm" label-width="80px">
        <el-form-item :label="t('名称')">
          <el-input v-model="projectForm.name" :placeholder="t('仅字母/数字/-/_')" />
        </el-form-item>
      ...
      <template #footer>
        <el-button @click="projectDialog = false">{{ t('取消') }}</el-button>
        <el-button type="primary" @click="onSaveProject">{{ t('创建') }}</el-button>
      </template>
    </el-dialog>

    <!-- 新建领域对话框 -->
    <el-dialog v-model="domainDialog" :title="t('新建领域')" width="460px">
      <el-form :model="domainForm" label-width="80px">
        <el-form-item :label="t('领域名')">
          <el-input v-model="domainForm.domain" :placeholder="t('如 marketing / finance / 自定义（仅字母/数字/-/_）')" />
        </el-form-item>
        <el-form-item :label="t('播种来源')">
          <el-select
            v-model="domainForm.seedFrom"
            :placeholder="t('留空=生成空白脚手架')"
            clearable
            style="width: 100%"
          >
            <el-option
          ...
          <div style="font-size: 12px; color: #888; margin-top: 4px">
            {{ t('选择仓库已有领域可直接复制其 domain.yaml 与引用文件（校验直接 PASS）。') }}
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="domainDialog = false">{{ t('取消') }}</el-button>
        <el-button type="primary" @click="onSaveDomain">{{ t('创建并校验') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
async function onActivateProject(row) {
  ...
  ElMessage.success(t('已激活：') + row.name)
  current.value = row.name
  await loadDomainsHere()
}
...
async function onDeleteProject(row) {
  ...
  ElMessage.success(t('已删除（移入 .trash）：') + row.name)
...
async function onSaveDomain() {
  ...
  if (r.ok) {
    ElMessage.success(t(r.seeded ? '已从仓库播种' : '已生成空白脚手架'))
...
async function onValidate(row) {
  ...
  if (r.valid) ElMessage.success(row.name + t(' 校验 PASS'))
  else ElMessage.error(row.name + t(' 校验 FAIL'))
}

async function onActivateDomain(row) {
  await activateDomain(current.value, row.name)
  ElMessage.success(t('已激活领域：') + row.name)
