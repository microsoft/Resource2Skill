<template>
  <div>
    <el-card shadow="never" style="margin-bottom: 16px">
      <el-form :inline="true">
        ...
        <el-form-item label="领域">
          <el-select v-model="domain" placeholder="选择领域" style="width: 200px" @change="refresh">
        ...
        <el-form-item>
          <el-upload
            ...
            multiple
          >
            <el-button type="primary" :disabled="!domain">上传素材</el-button>
          </el-upload>
        </el-form-item>
        <el-form-item>
          <span style="color: #909399; font-size: 12px"
            >支持 PDF / DOCX / PPTX / XLSX / MD / TXT；仅文档类</span
          >
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never">
      <el-table :data="rows" v-loading="loading" empty-text="暂无素材，先选择领域并上传">
    ...
    <el-dialog v-model="previewVisible" :title="'预览：' + previewName" width="720px">
      ...
      <el-alert v-else type="warning" :closable="false" :title="(previewData && previewData.message) || '无可用文本'" />
      <template #footer v-if="previewData && previewData.available && previewData.truncated">
        <span style="color: #909399; font-size: 12px">仅显示前 {{ previewData.text.length }} 字（共 {{ previewData.total_chars }} 字）</span>
      </template>
    </el-dialog>
  </div>
</template>
...
function beforeUpload(file) {
  ...
  if (!ok) {
    ElMessage.error('不支持的类型：' + file.name)
    return false
  }
  return true
}
...
function onUploadError() {
  ElMessage.error('上传失败（类型不支持或服务端拒绝）')
}
...
async function onToggle(row, val) {
  try {
    ...
    row.enabled = !val
    ElMessage.error('更新启用状态失败：' + (e.response?.data?.detail || e.message))
  }
}

async function onDelete(row) {
  try {
    await ElMessageBox.confirm(`确认删除素材「${row.name}」？将移入回收站（可恢复）。`, '删除确认', {
      type: 'warning',
    })
  } catch {
    return
