...
> **Skill Name**: Executive Dashboard Layout Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a pristine, multi-chart dashboard layout by disabling sheet gridlines and row/col headers, formatting a unified title banner, and precisely anchoring and sizing a primary stacked column chart alongside secondary line charts and a control panel placeholder.
* **Applicability**: Use for executive summaries, KPI dashboards, or performance reports requiring a clean, software-like interface rather than a raw spreadsheet look. Best when data is pre-aggregated (or powered by pivot tables on hidden sheets) and needs a polished presentation layer.

### 2. Structural Breakdown

- **Data Layout**: Raw or aggregated chart data is relegated to a hidden backend sheet (`DashboardData`), keeping the presentation layer completely clean.
- **Formula Logic**: None required for the shell itself; charts are bound directly to the hidden data ranges.
- **Visual Design**: Gridlines and axis headers are disabled (`showGridLines = False`). A merged, solid-filled area acts as a global title banner. A light-gray panel on the left reserves space for interactive controls (slicers/timelines).
- **Charts/Tables**: 
  - Primary Chart: Stacked Column (`BarChart` with `grouping="stacked"` and `overlap=100`), anchored to `D5`.
  - Secondary Charts: Line charts tracking trends, legends disabled for cleanliness, anchored sequentially on the right (`L5` and `L16`).
- **Theme Hooks**: Utilizes `theme["bg"]` for the primary title banner background and `theme["fg"]` for the banner text.
