...
> **Skill Name**: Multi-Sheet Navigable Dashboard

* **Tier**: archetype
* **Core Mechanism**: Generates a multi-sheet workbook structure featuring a persistent left-hand "sidebar" on every sheet. Uses intra-document cell hyperlinks to simulate web-app navigation between views (e.g., Dashboard, Inputs, Settings), and native cell fills/borders to create a modern "card-based" layout structure without relying on floating shapes.
* **Applicability**: Perfect for complex reports or operational tools that require distinct views while maintaining a cohesive, app-like user experience. Replaces the need for users to hunt through standard Excel sheet tabs at the bottom.

### 2. Structural Breakdown

- **Data Layout**: Dedicated sheets for distinct concerns (Dashboard vs Data vs Settings). Column A acts as the global navigation menu.
- **Formula Logic**: Cell `hyperlink` properties are mapped to `#'SheetName'!A1` to jump between sheets seamlessly.
- **Visual Design**: Uses a light gray (`#F3F4F6`) global background with high-contrast dark blue (`#1E3A8A`) for the sidebar. The active tab is highlighted in a distinct blue block, removing the hyperlink underline to indicate state.
- **Charts/Tables**: Implements a reusable `draw_card()` helper to create white, thinly-bordered regions that act as containers for KPIs or charts.
- **Theme Hooks**: Sidebar background (`primary`), Sidebar active state (`accent`), text colors (`text_light`, `text_dark`), and generic background fills.
