...
> **Skill Name**: Themed Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Sets up a multi-sheet workbook consisting of a raw 'Data' table, a hidden 'Calc' sheet for aggregations, and a presentation 'Dashboard' sheet. The Dashboard disables gridlines, features a bold theme-driven header, and strategically aligns multiple charts (Stacked Bar, Line) to mimic a professional, interactive Pivot Dashboard layout.
* **Applicability**: Use when building executive summaries or KPI dashboards. It cleanly separates raw data, calculation logic, and presentation into distinct layers, ensuring the final visual is uncluttered and highly readable.

### 2. Structural Breakdown

- **Data Layout**: 
  - `Data`: Contains the raw, transactional data formatted as an Excel Table.
  - `Calc`: Hidden sheet holding aggregated metrics specifically structured for charting.
  - `Dashboard`: The presentation layer with `showGridLines = False`.
- **Formula Logic**: Aggregations are handled pre-charting in the `Calc` sheet (simulating PivotTable outputs for the charts to consume).
- **Visual Design**: Gridlines are disabled on the Dashboard. A merged, centered header spans the top of the dashboard, colored using the theme's primary color.
- **Charts/Tables**: 
  - Chart 1: Stacked Column Chart (`grouping="stacked", overlap=100`) to show composition across categories.
  - Chart 2 & 3: Line Charts stacked vertically to show temporal trends, with legends disabled for cleaner aesthetics.
- **Theme Hooks**: Uses the `theme` parameter to extract a `primary` color for the dashboard header background and a `text` color for the header font.
