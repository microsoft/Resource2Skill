...
> **Skill Name**: Themed KPI Dashboard Sheet

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a grid of standalone "KPI Cards" by aggressively merging cells, applying dynamic formatting, and structuring a standardized 4-column sub-grid for each metric. Uses `CellIsRule` conditional formatting anchored to dynamic on-sheet target cells to color-code performance automatically.
* **Applicability**: Executive summaries, financial controllership dashboards, and high-level metric reporting where visual emphasis on "Actual vs. Target" and "Actual vs. Prior" is critical. 

### 2. Structural Breakdown

- **Data Layout**: Global month dropdown anchored at the top left. The sheet is broken into thematic sections via full-width merged headers (e.g., "Working Capital Efficiency"). Each KPI card spans 4 columns and 3 rows, standardizing metric title, primary value, and two comparison labels.
- **Formula Logic**: Simulated logic relies on absolute reference conditional formatting (e.g., `=$C$9`) where the main KPI value cell dynamically evaluates against its local target cell.
- **Visual Design**: Gridlines are hidden. Main values use an ultra-large bold font (size 24). Conditional formatting entirely replaces the cell background (Green/Red) and text color based on threshold logic.
- **Charts/Tables**: Bypasses traditional charts in favor of highly stylized grid-layout metric blocks that act as visual indicators.
- **Theme Hooks**: Utilizes `primary` for section headers, `secondary` for card titles, `accent` for the dropdown, and `good_bg`/`bad_bg` for conditional formatting rules.
