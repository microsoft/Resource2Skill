...
> **Skill Name**: KPI Table with Embedded Data Bars

* **Tier**: component
* **Core Mechanism**: Generates a performance summary table where numeric columns are augmented with in-cell Data Bars via Conditional Formatting. Applies custom header fills, column-specific number formats, and dynamic column widths to create a dashboard-quality visual without requiring standalone charts.
* **Applicability**: Ideal for scorecards, agent performance tracking, or any scenario comparing a list of entities across multiple metrics. Provides instant visual benchmarking directly within the grid, saving space on dense dashboards.

### 2. Structural Breakdown

- **Data Layout**: Tabular grid starting at `anchor`. Top row is for headers, followed by rows of entity data.
- **Formula Logic**: Purely value-driven in this component, relying on conditional formatting rules to automatically evaluate cell values relative to the column's min/max.
- **Visual Design**: Dark themed headers with bold text. Light gray thin borders around data cells. Numeric alignment ensures readability.
- **Charts/Tables**: Employs `DataBarRule` conditional formatting in lieu of bar charts to overlay visual magnitude directly behind the cell text.
- **Theme Hooks**: Uses `primary_color` for the header background and `background_color` for header text.
