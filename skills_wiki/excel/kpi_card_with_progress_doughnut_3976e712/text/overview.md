...
> **Skill Name**: KPI Card With Progress Doughnut

* **Tier**: component
* **Core Mechanism**: Constructs a styled 4x5 cell region using borders and merged cells to act as a physical "card". Embeds a borderless Doughnut Chart next to the primary metric, injecting individual `DataPoint` colors to create a "progress ring" effect showing actual vs. remainder.
* **Applicability**: Ideal for executive dashboards and summary sheets where you need to visualize progress toward a specific target (like sales quotas, budgets, or completion percentages) alongside the absolute metric.

### 2. Structural Breakdown

- **Data Layout**: Stores the raw percentage calculations (Actual vs Remainder) offset +10 columns from the anchor so they can be hidden or sit outside the print area.
- **Formula Logic**: Python computes `pct = actual / target` and `remainder = 1 - pct` to feed the progress ring.
- **Visual Design**: Uses a white pattern fill with thin gray top/left borders and a medium gray bottom/right border to simulate a drop shadow.
- **Charts/Tables**: A `DoughnutChart` with `holeSize=65`, no legend, and transparent borders. The `DataPoint` at index 0 (Actual) is assigned the accent color; index 1 (Remainder) is set to a neutral gray.
- **Theme Hooks**: The `actual_val` text and the filled portion of the doughnut chart consume the theme's primary accent color.
