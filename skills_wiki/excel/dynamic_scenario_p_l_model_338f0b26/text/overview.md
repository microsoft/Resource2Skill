...
> **Skill Name**: Dynamic Scenario P&L Model

* **Tier**: sheet_shell
* **Core Mechanism**: Use the `CHOOSE` function tied to a Data Validation dropdown to dynamically pull assumption values from multiple scenario blocks (Base, Best, Worst) into a single "Live Case" block. The financial model calculates exclusively off this Live Case, allowing instant, VBA-free scenario toggling.
* **Applicability**: Ideal for 3-statement models, budget forecasts, and sensitivity analyses where users need to present and toggle between different business cases (e.g., upside/downside) without duplicating entire calculation engines.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Rows 4-10**: The Income Statement (Revenue down to Net Income).
  - **Rows 13-19**: The "Live Case" assumptions which feed the Income Statement.
  - **Rows 21+**: Hardcoded scenario blocks (Scenario 1, Scenario 2, Scenario 3).
- **Formula Logic**: 
  - Scenario Routing: `=CHOOSE($J$2, C22, C30, C38)` inside the Live Case.
  - Growth Calculation: `=PreviousYearOrders * (1 + CurrentYearGrowth)`.
  - Conditional Profit/Tax: `=IF(OperatingProfit>0, OperatingProfit*TaxRate, 0)`.
- **Visual Design**: The dropdown control cell is highlighted in a distinct accent color. Subtotals in the P&L get single top borders and double bottom borders. Year headers use primary theme colors.
- **Charts/Tables**: Clean structured grid layout; no native Excel Tables due to the forecast column structure.
- **Theme Hooks**: Uses a custom dictionary fallback for `primary` (header backgrounds), `text` (header text), and `highlight` (scenario control cell).
