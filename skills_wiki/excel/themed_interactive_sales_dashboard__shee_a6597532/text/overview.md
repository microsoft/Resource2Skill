...
> **Skill Name**: Themed Interactive Sales Dashboard (Sheet Shell)

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill constructs a visually cohesive and interactive Excel dashboard on a dedicated sheet. It establishes a multi-section layout for key performance indicators (KPIs) and data visualization charts, dynamically linking text boxes to aggregated data from an analysis sheet. Interactive elements like slicer-driven conditional formatting are applied to enhance data exploration.
*   **Applicability**: This skill is ideal for creating professional-looking, single-sheet dashboards that summarize key metrics and trends. It assumes underlying data has already been prepared and summarized (e.g., in PivotTables on an 'Analysis' sheet). It's suitable for sales performance tracking, project monitoring, or any scenario requiring an at-a-glance overview with interactive filtering.

### 2. Structural Breakdown

-   **Data Layout**: The dashboard sheet itself does not contain raw data. It links directly to summarized data on an 'Analysis' sheet. KPIs are displayed in text boxes linked to individual cells, while charts draw their data from PivotTables on the 'Analysis' sheet. A main Sales Agent KPIs table on the dashboard directly displays data (e.g., from a PivotTable) and allows interaction.
-   **Formula Logic**:
    -   **KPI Text Boxes**: `=Analysis!B<row_number>` (e.g., `='Analysis'!B4` for Total Calls) links directly to aggregated values.
    -   **Conditional Formatting for Sales Agent KPIs**: `=INDIRECT("D"&ROW())='Analysis'!$A$11` (assuming D is the column containing salesperson names on the dashboard, and `Analysis!$A$11` holds the name selected in the slicer). This highlights the row of the selected salesperson.
-   **Visual Design**:
    -   **Theme**: A custom theme is applied using specific purple and gold hues (`#49165B` for dark purple, `#F2EFF5` for light purple, `#FFD700` for gold).
    -   **Header**: Dark purple background for rows 1-8. Main title (white, font size 36) and subtitle (yellow, font size 16) are placed here.
    -   **Body**: Light purple background for rows 9-40.
    -   **KPI Section**: Shapes with rounded corners (gold and white) are used as containers, separated by thin gold vertical lines. Icons (phone, target, prize, money) are inserted and colored purple.
    -   **Charts/Tables**: Charts and tables have transparent backgrounds, no borders, and no gridlines. Data bars in custom gold/purple colors are used for the 'Sales Agent KPIs' table.
    -   **Fonts**: Aptos Narrow is used throughout.
-   **Charts/Tables**:
    -   **Sales Agent KPIs Table**: A PivotTable (or similar structured table) displaying Name, Total Calls (data bars gold), Calls Reached (data bars yellow), Deals Closed (data bars purple), and Deal Value (data bars light purple), sorted by Deal Value descending.
    -   **Sum of Calls Reached & Deals Closed Chart**: Stacked column chart by month. Calls Reached in yellow, Deals Closed in purple. Data labels inside end.
    -   **Total Sales Chart**: Column chart by month in purple, with a yellow dashed linear trendline.
    -   **Average Call Duration Chart**: Column chart by month in yellow.
    -   **Average Call Drop Rate Chart**: Area chart by month with a gradient fill from light yellow to white.
    -   **Chart Formatting**: Axes removed where data labels are present. Vertical axes start at 0. Shadows are applied to all charts for depth.
-   **Theme Hooks**: `header_bg`, `body_bg`, `title_fg`, `subtitle_fg`, `accent_1`, `accent_2`, `text_color`, `border_color`.
