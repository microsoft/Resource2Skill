...
> **Skill Name**: Dynamic Scenario P&L (CHOOSE Method)

* **Tier**: sheet_shell
* **Core Mechanism**: Implements a standard financial modeling scenario selector using Data Validation and the `CHOOSE` function. A central "Live Case" assumption block dynamically pulls values from multiple hardcoded scenario blocks (Base, Upside) based on a dropdown, which then drives the entire P&L forecast. Enforces standard financial color-coding (blue for hardcoded inputs, black for formulas).
* **Applicability**: Ideal for financial models, budgets, and forecasts where multiple operational scenarios (Base, Best, Worst case) need to be evaluated on a single output sheet without duplicating the core calculation logic.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Top** (Rows 3-10): 5-Year Income Statement calculations.
  - **Middle** (Rows 13-19): Live Case Assumptions (formulas routing to the active scenario).
  - **Bottom** (Rows 22+): Distinct Scenario Assumption Blocks containing the hardcoded inputs.
- **Formula Logic**: 
  - `=CHOOSE($I$2, C23, C31)` dynamically selects the input cell based on the scenario toggle in `I2`.
  - `=Prev_Year * (1 + Growth_Rate)` handles horizontal quantity forecasting.
  - `=IFERROR(IF(Profit>0, Profit*TaxRate, 0), 0)` provides robust tax logic avoiding negative taxes on losses.
- **Visual Design**: Hardcoded input cells use strict Blue font (`0000FF`), while formula-driven cells use Black (`000000`). The scenario toggle cell uses a pale yellow fill to signal interactivity. Top/bottom double borders denote final profit margins.
- **Theme Hooks**: Uses standard financial modeling conventions over a strict color palette, but can hook into `theme.bg_highlight` for the scenario dropdown if desired.
