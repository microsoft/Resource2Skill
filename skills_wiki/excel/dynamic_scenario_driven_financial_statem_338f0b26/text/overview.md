...
> **Skill Name**: Dynamic Scenario-Driven Financial Statement

* **Tier**: sheet_shell
* **Core Mechanism**: Builds an Income Statement that dynamically recalculates based on an active scenario. Uses a Data Validation dropdown to select a scenario index (1 or 2). A "Live Case" assumptions block uses the `CHOOSE` function to pull values from hardcoded scenario blocks (Base, Upside). The Income Statement then calculates strictly off the Live Case, applying financial modeling conventions (blue text for hardcoded inputs, black for formulas).
* **Applicability**: Ideal for 3-statement models, DCFs, or budget forecasts where users need to instantly toggle between different sets of assumptions (e.g., Base, Best, Worst case) to analyze P&L impact without overwriting data.

### 2. Structural Breakdown

- **Data Layout**: Vertical segmentation. Rows 1-11 hold the calculated P&L. Rows 13-18 hold the "Live Case" assumptions. Rows 20+ contain distinct hardcoded data blocks for each scenario.
- **Formula Logic**: `=CHOOSE($J$2, C21, C28)` maps the dropdown value in `J2` to the correct scenario row. Standard financial formulas flow upward into the P&L (e.g., Revenue = Orders * AOV).
- **Visual Design**: Standard financial modeling color conventions. Hardcoded inputs are blue (`0000FF`); formulas are black. Top and bottom thin borders emphasize key profit lines. The active scenario toggle is highlighted with a light yellow fill to invite user interaction.
- **Charts/Tables**: N/A (Formula-driven tabular layout).
- **Theme Hooks**: Consumes `header_bg` for column headers and scenario block titles, and `input_fg` for hardcoded parameter text.
