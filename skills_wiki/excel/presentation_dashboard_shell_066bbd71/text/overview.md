...
> **Skill Name**: Presentation Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Creates a presentation-ready interactive dashboard layout by disabling gridlines/headers, establishing a strong themed title banner, setting up a background data region (hidden), and composing multiple charts in an aligned grid layout.
* **Applicability**: Essential for summarizing organizational data into a single "executive view" worksheet without the visual clutter of standard spreadsheet cells.

### 2. Structural Breakdown

- **Data Layout**: Places aggregation data in a hidden column range (`AA:AD`) so the visible area remains a pristine dashboard canvas.
- **Visual Design**: Uses a prominent `A1:V3` merged header with a corporate primary color fill and contrasting text. Disables `showGridLines` and `showRowColHeaders` for a clean, app-like UI.
- **Charts/Tables**: Integrates a primary stacked column chart on the left and two vertically stacked line charts on the right, simulating the classic "1 main KPI, 2 trend lines" layout.
- **Theme Hooks**: Utilizes `primary` and `text_on_primary` from the active theme.
