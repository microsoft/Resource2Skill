...
> **Skill Name**: Themed Cell-Based KPI Card

* **Tier**: component
* **Core Mechanism**: Constructs a distinct KPI visual module using a 3x2 grid of styled cells to emulate a UI card. It places the region/title at the top, a label and main value below it, and vertically merges the adjacent column to create a high-contrast 'badge' for the secondary metric (e.g., market share). Background fills and font styling simulate modern dashboard shapes while maintaining robust programmatic linkability.
* **Applicability**: Best used in dashboard header sections or summary sheets where key metrics need to be highlighted with a clean, card-like appearance without relying on fragile, hard-to-automate floating shapes. 

### 2. Structural Breakdown

- **Data Layout**: A 3-row by 2-column block anchored at a specified cell. Column 1 holds the Title, Label, and Value stacked vertically. Column 2 is merged across all 3 rows to display a percentage badge.
- **Formula Logic**: Metric values can be injected statically or dynamically linked to calculation cells using formula strings (e.g., `={value_ref}`).
- **Visual Design**: Uses a dark theme background for the main body with a slightly darker/contrasting shade for the badge area. Fonts are set to white and bolded for emphasis on the main data points.
- **Charts/Tables**: N/A (Constructed entirely with rich cell formatting).
- **Theme Hooks**: Consumes `card_bg` (primary card background), `badge_bg` (accent metric background), and `card_fg` (text color).
