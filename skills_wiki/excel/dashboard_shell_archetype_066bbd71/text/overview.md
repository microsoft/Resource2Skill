...
> **Skill Name**: Dashboard Shell Archetype

* **Tier**: archetype
* **Core Mechanism**: Constructs a multi-sheet reporting workbook where a hidden 'Data' sheet holds aggregated summary tables, and a 'Dashboard' sheet acts as a clean presentation canvas. Gridlines are disabled, a themed header anchors the layout, and multiple charts are precisely aligned into a grid based on cell anchors.
* **Applicability**: Best used for generating polished, static end-of-period reports or automated dashboard exports where data needs to be visualized cleanly without exposing the underlying dataset to the immediate view.

### 2. Structural Breakdown

- **Data Layout**: Raw aggregate tables stacked vertically on a dedicated "Data" sheet, which is then hidden (`sheet_state = "hidden"`).
- **Formula Logic**: N/A (Data is pre-aggregated and injected directly for charting).
- **Visual Design**: Gridlines disabled on the presentation layer. A large merged region (A1:P3) serves as a title banner with centered text and a solid theme background fill.
- **Charts/Tables**: Includes a Stacked Column chart (`overlap=100`) for categorical data and Line charts for time-series trends. Legends are disabled on the line charts to maximize the plot area.
- **Theme Hooks**: Primary palette color (`bg`) is used for the header fill, and contrast text (`fg`) for the title font.
