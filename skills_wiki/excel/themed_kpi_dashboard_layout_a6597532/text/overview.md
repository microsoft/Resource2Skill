...
* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a sleek, modern dashboard layout using native cells instead of fragile Excel shapes. Utilizes merged cells, thick accent borders, and contrasting background fills to simulate floating KPI cards. Pairs the header strip with a summary table enriched by in-cell conditional formatting Data Bars.
* **Applicability**: Perfect for high-level executive or sales performance reports where top-line metrics must be prominently displayed above tabular rankings or charts.

### 2. Structural Breakdown

- **Data Layout**: Minimal padding in Column A. A solid-colored theme header spanning Rows 1-8. KPI cards are dynamically spaced horizontally starting at Column D (Rows 5-6). A detailed performance table is anchored below the fold (Row 12).
- **Formula Logic**: Layout-driven structure designed to accept pre-aggregated metrics (often extracted from PivotTables or external sources via GETPIVOTDATA).
- **Visual Design**: Dark-themed header background contrasting with white KPI cards. Cards feature a heavy solid left border to mimic modern UI components. Worksheet gridlines are globally disabled.
- **Charts/Tables**: Renders a summary table with a custom themed header, alternating row fills (zebra striping), and applies `DataBarRule` conditional formatting for rapid visual comparison of numeric columns.
- **Theme Hooks**: Expects color tokens. Falls back to deep purple (`primary_bg`), white text (`primary_fg`), gold (`accent1`), and light purple (`accent2`) for data bars and accents.
