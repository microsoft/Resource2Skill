...
> **Skill Name**: Segmented KPI Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a presentation-layer dashboard using a grid of 4-column-wide "KPI Cards". Each card displays a prominent primary metric, context footer metrics (Target and Prior Period), and uses dynamic conditional formatting to change the primary metric's background color (red/green) based on its relation to the target. 
* **Applicability**: Best used for executive summaries, monthly operational reporting, or top-level metric tracking. It serves as the frontend view that reads pre-calculated metrics from a staging or raw data tab.

### 2. Structural Breakdown

- **Data Layout**: Cards are spaced 1 column apart. Each card spans 4 columns (e.g., B to E). Card layout: Row 1 = Title (Merged), Row 2 = Value (Merged), Row 3 = 4 distinct cells for footer labels ("Vs. Target", Target Value, "Vs. Prior", Prior Value).
- **Formula Logic**: Standardizes visual performance logic using conditional formatting directly referencing the Target cell within the same card's footer.
- **Visual Design**: Gridlines are hidden. Main category headers use heavy, themed background fills with bold white text. KPI cards use lighter themed headers. Main values use Size 24 fonts, centered.
- **Charts/Tables**: Replaces traditional charts with high-density, stylized typography grids.
- **Theme Hooks**: Uses `primary` for section banners, `secondary` for card headers, and standard red/green hex codes for conditional performance states.
