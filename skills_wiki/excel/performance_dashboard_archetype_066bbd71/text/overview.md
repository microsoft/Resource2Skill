...
> **Skill Name**: Performance Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Constructs a multi-sheet reporting workbook by writing pre-aggregated metrics to a hidden `ChartData` sheet, then plotting them onto a gridline-free `Dashboard` presentation sheet using precisely anchored and styled Bar and Line charts.
* **Applicability**: Best used for executive summaries or KPI dashboards where multiple metrics (like categorical breakdowns and time-series trends) need to be arranged in a visually cohesive, single-page layout. 

### 2. Structural Breakdown

- **Data Layout**: Pre-aggregated data tables are written to a secondary hidden sheet (`ChartData`) sequentially, separated by spacer rows.
- **Formula Logic**: Standard cell number formatting (`"$"#,##0` and `#,##0`) is applied directly to the source data so the resulting chart axes automatically inherit the correct format.
- **Visual Design**: The `Dashboard` sheet hides gridlines (`showGridLines = False`) for a modern app-like appearance and creates a unified full-width header by merging `A1:P3` and applying the primary theme color.
- **Charts/Tables**: 
  - Stacked `BarChart` (`overlap=100`, `grouping="stacked"`) for categorical comparison.
  - Dual `LineChart`s positioned vertically to track time-series metrics.
  - Legends are disabled (`chart.legend = None`) for simple trend charts to maximize data-ink ratio.
- **Theme Hooks**: Consumes `header_bg` and `header_fg` for the unified dashboard title banner.
