...
> **Skill Name**: Dynamic Revenue Scenario Analysis

*   **Tier**: component
*   **Core Mechanism**: Sets up a dynamic scenario analysis block for revenue growth rate. It calculates best, base, and worst-case forecast values based on a base case input and user-defined offsets. The main revenue growth assumption then uses Excel's `CHOOSE` function, linked to a scenario selector cell, to dynamically display one of these three scenarios.
*   **Applicability**: Ideal for financial models, budgeting tools, or any forecast where sensitivity analysis across multiple scenarios (optimistic, neutral, pessimistic) is crucial. Enables easy toggling of projections without manual data entry, providing an interactive element for scenario planning.

### 2. Structural Breakdown

-   **Data Layout**:
    -   A main "Revenue Growth Rate" assumption row (e.g., starting at `C17`), with historical actual values and forecast values.
    -   A "Revenue Scenarios" header row (e.g., `B26`).
    -   Three subsequent rows (e.g., `B27:B29`) labeled "Best Case", "Base Case", and "Worst Case".
    -   Forecast values for each of these three scenarios are placed in corresponding columns (e.g., `G27:K29`), relative to the scenario header.
-   **Formula Logic**:
    -   **Scenario Forecast Values (e.g., `G27`, `G28`, `G29` and dragged right)**:
        -   Best Case: `=Base_Case_Value + Best_Case_Offset`
        -   Base Case: `=Base_Case_Value` (direct input or link)
        -   Worst Case: `=Base_Case_Value - Worst_Case_Offset`
    -   **Dynamic Revenue Growth Rate (e.g., `G17` and dragged right)**:
        `=CHOOSE(scenario_control_cell_reference, Best_Case_Forecast_Value_Cell, Base_Case_Forecast_Value_Cell, Worst_Case_Forecast_Value_Cell)`
        (e.g., `=CHOOSE(Cover!$C$6, Forecast!G27, Forecast!G28, Forecast!G29)`).
-   **Visual Design**:
    -   Main assumption label (`Revenue Growth Rate`): Bold font.
    -   Historical actual values: Blue font (`#0000FF`).
    -   Forecast values (driven by `CHOOSE` function): Black font.
    -   "Revenue Scenarios" header: Dark blue fill, white bold font.
    -   Scenario-specific forecast values (Best, Base, Worst Case): Blue font (`#0000FF`).
    -   All percentage values formatted to one decimal place (`0.0%`).
    -   Rows containing the Best, Base, and Worst Case scenarios are grouped and initially hidden, allowing the user to expand them for detail.
-   **Charts/Tables**: This skill provides dynamic input for other parts of the financial model, which might then feed into charts or tables, but does not render them directly.
-   **Theme Hooks**: `header_bg_dark`, `header_fg`, `input_fg_blue`, `formula_fg_black`.
