...
> **Skill Name**: KPI Progress Doughnut Panel

* **Tier**: component
* **Core Mechanism**: Constructs a self-contained KPI widget panel by drawing a grid-based card boundary and rendering a background-transparent Doughnut chart. By anchoring the chart over specific cells and hiding the chart's background (`noFill=True`), we can display dynamically formatted text (the completion percentage) precisely inside the doughnut hole—circumventing Excel's inability to embed cell-linked text boxes directly inside openpyxl-generated charts.
* **Applicability**: Ideal for executive dashboards, operational reports, and tracking actuals against targets. This widget works best when arrayed horizontally across the top of a dashboard to form a "KPI strip".

### 2. Structural Breakdown

- **Data Layout**: Uses a 5-column by 6-row cell grid for the card body. KPI name, actual value, and target labels are assigned to the left columns. Hidden chart data (Complete % and Remaining %) is stored in the rightmost column.
- **Formula Logic**: Computes progress completion bounding it to a maximum of 100% (`min(actual / target, 1.0)`) to cleanly render the doughnut, while still displaying the true percentage in the text readout.
- **Visual Design**: The cells are unified with a solid background and a thin grey outer border to create a "card" effect. The text placed behind the doughnut hole uses a large, bold font colored to match the primary chart slice.
- **Charts/Tables**: A `DoughnutChart` scaled to ~2 columns wide and 6 rows high. `ManualLayout(x=0, y=0, w=1, h=1)` stretches the plot area, and `holeSize=75` creates ample room for the center text.
- **Theme Hooks**: `panel_bg`, `panel_border`, `text_main`, `text_light`, `primary` (completed progress slice), `secondary` (remaining progress slice).
