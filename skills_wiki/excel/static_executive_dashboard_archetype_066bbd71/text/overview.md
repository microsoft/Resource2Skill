...
> **Skill Name**: Static Executive Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Creates a multi-sheet reporting structure with a clean front-end dashboard (hidden gridlines, header banner) and a hidden data calculation sheet. Uses Stacked Column and Line charts pulling from the calculation sheet to emulate a professional executive dashboard layout.
* **Applicability**: When building an end-to-end report presentation where raw data should be hidden from the user, presenting only cleaned charts and high-level metrics in a visually structured, app-like layout.

### 2. Structural Breakdown

- **Data Layout**: Uses a hidden sheet `ChartData` with separate tabular blocks for categorical aggregations (e.g., market by product) and time-series trends (e.g., monthly sales).
- **Formula Logic**: Pre-aggregated metrics are written as static values (simulating the PivotTables seen in the interactive tutorial).
- **Visual Design**: Gridlines are disabled on the Dashboard sheet. A prominent top banner spanning `A1:R3` is filled with the primary theme color and large, bold white text. 
- **Charts/Tables**: Contains a Stacked BarChart (`type="col"`, `grouping="stacked"`) and two smoothed LineCharts (`legend=None`). All charts are precisely sized and anchored to align neatly in a grid.
- **Theme Hooks**: Consumes `primary` and `text_on_primary` (fallback to dark blue and white) for the header banner.
