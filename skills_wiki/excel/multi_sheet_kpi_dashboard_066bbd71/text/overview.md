...
> **Skill Name**: Multi-Sheet KPI Dashboard

* **Tier**: archetype
* **Core Mechanism**: Build a professional dashboard architecture by strictly separating concerns into three layers: a clean presentation sheet (gridlines off, themed header), a hidden calculation sheet (for aggregating metrics), and a raw data sheet (formatted as an Excel Table). This allows charts to be systematically arranged on a dedicated canvas without exposing the underlying data processing.
* **Applicability**: Best used when generating comprehensive performance reports from flat transactional data. It provides a robust template for combining categorical breakdowns (stacked columns) and time-series trends (line charts) into a single, executive-friendly view.

### 2. Structural Breakdown

- **Data Layout**: 
  - `Dashboard`: Clean canvas, gridlines disabled.
  - `Calculations` (Hidden): Contains summarized cross-tabulations and time-series tables to feed the charts.
  - `Data`: Flat transactional data structured as an Excel Table.
- **Formula Logic**: Aggregations (simulating Excel's Pivot Tables) are computed prior to charting and deposited in the calculation layer.
- **Visual Design**: The presentation layer removes standard Excel UI elements (gridlines) to simulate a standalone app. A prominent, solid-filled merged header acts as the dashboard title.
- **Charts/Tables**: 
  - Main Chart: Stacked Column chart comparing multiple categories (e.g., Profit by Market and Product).
  - Sub Charts: Line charts visualizing trends over time (e.g., Monthly Units, Monthly Profit).
- **Theme Hooks**: The title header background and text colors consume `primary` and `text_on_primary` theme tokens.
