...
> **Skill Name**: Themed Interactive Chart Set

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill demonstrates how to prepare data using pivot tables, extract aggregated data into dedicated helper ranges, and then create multiple theme-aware charts (e.g., line chart, bar chart, map chart) on a single dashboard sheet. It includes a specific workaround for creating map charts from pivot-derived data by using intermediate cells. KPIs are dynamically linked from pivot tables to display cells for easy overview. The entire visual presentation is tied to a custom color theme.
*   **Applicability**: This is applicable for users who need to create visually consistent and interactive data reports within Excel using `openpyxl`. It's ideal for scenarios where data can be summarized by pivot tables and presented through various chart types, especially when dealing with geographical data, and a consistent corporate branding is desired.

### 2. Structural Breakdown

-   **Data Layout**:
    *   `Orders` (main data table on a dedicated sheet, with calculated columns for `Days to Deliver`, `Weeknum`, `Gender Value`).
    *   `Pivots_Helper` (sheet containing various pivot table outputs for KPIs, trends, quantity distribution, customer satisfaction).
    *   `Map_Data_Helper` (sheet with data for map charts, derived from pivots, includes `IF` logic to handle blank/zero values from pivot data, preventing map chart errors).
    *   `Prod_Piv_Helper` (sheet with products by quantity data derived from pivot).
-   **Formula Logic**:
    ...
    *   Dashboard KPI Display (on the main dashboard sheet, linked to `Pivots_Helper`):
        *   Orders: `=TEXT(Pivots_Helper!A2,"#,##0")`
        *   Amount: `=TEXT(Pivots_Helper!C2,"$#,##0,.0k")`
        *   Average Rating: `=TEXT(Pivots_Helper!D2,"0.0")`
    *   Map Chart Data Prep (on `Map_Data_Helper` sheet):
        *   State/County cells: `=IF(ISBLANK(Pivots_Helper!AXX),"",Pivots_Helper!AXX)` (where AXX is the corresponding pivot cell). This prevents map chart errors for missing data.
-   **Visual Design**:
    *   **Dashboard Background**: Left panel (columns A-B) filled with `header_bg_dark`, main area (columns C-AB) filled with `header_bg_light`.
    *   **KPIs**: Displayed by linking dashboard cells to pivot-derived values. Font is `text_light`, bold, larger size.
    *   **Charts**: Use themed colors (`accent1` to `accent6`) for series. Titles are descriptive, often reflecting business questions. Legends are sometimes moved to top or hidden for clarity. Axes are formatted (e.g., currency, percentages). Map chart colors are adjusted for sequential display.
-   **Charts/Tables**:
    *   **KPI Display**: Cells directly linked to pivot outputs on `Pivots_Helper`.
    *   **Line Chart (Trends)**: From `Pivots_Helper` data (mocked), shows two series (quantity and amount), amount on a secondary axis, `Weeknum` on x-axis.
    *   **Column Chart (Quantity Distribution)**: From `Pivots_Helper` data (mocked), x-axis grouped by quantity ranges, `gapWidth = 0`.
    *   **Stacked Bar Chart (Popular Products)**: From `Prod_Piv_Helper` data (mocked), stacked by gender, y-axis (products) reversed to show most popular at top, `gapWidth = 10%`.
    *   **Donut Chart (Overall Gender Split)**: From `Pivots_Helper` data (mocked), labels show percentages and category names.
    *   **Map Chart (Customer Location)**: Created from `Map_Data_Helper` (derived from pivot data, using `IF` logic for blanks). One map for quantity, one for amount. Uses theme-based sequential color scales.
    *   **Clustered Column Chart (Customer Satisfaction)**: From `Pivots_Helper` data (mocked), shows satisfaction ratings (1-5) by month.
-   **Theme Hooks**: Uses `colors.header_bg_dark`, `colors.header_bg_light`, `colors.accent1` through `accent6`, `colors.neutral_mid`, `colors.text_light`.
