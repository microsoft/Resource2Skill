...
> **Skill Name**: Interactive Dashboard Layout

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill establishes a full-sheet Excel dashboard layout featuring a left-hand navigation sidebar and a main content area structured with visually distinct rounded rectangular shapes. These shapes act as themed containers for various data visualizations and KPIs, providing a clean, organized, and navigable interface.
*   **Applicability**: This pattern is suitable for any multi-sheet Excel report or dashboard where user-friendly navigation and a clear, modular presentation of key information are paramount. It enhances readability and user experience for management reports, sales performance trackers, or any data summary requiring a structured overview.

### 2. Structural Breakdown

-   **Data Layout**: The skill primarily defines the visual framework of the dashboard sheet. Actual data for charts and KPIs resides on separate 'Inputs' or 'Contacts' sheets, which are intended to be linked or referenced.
-   **Formula Logic**: The core structural layout uses shapes. Dynamic linking for sidebar icons (to other sheets or email) and embedded text boxes (to display KPI values) relies on Excel's native hyperlink and formula bar capabilities, which are manually applied in the video (not directly replicable via `openpyxl`'s current shape API for dynamic updates).
-   **Visual Design**:
    *   **Sheet Background**: Gridlines are hidden for a clean, report-like appearance.
    *   **Sidebar (Column A)**: Filled with a dark blue theme color, creating a distinct navigation panel. Intended to house interactive icons (conceptually represented in code).
    *   **Main Content Area Shapes**:
        *   **Form**: Rounded rectangles are used as containers for various sections.
        *   **Fill**: Solid white to make content stand out.
        *   **Outline**: No outline.
        *   **Shadow**: A subtle "Outer, Offset Center" shadow effect (visually shown in video, `openpyxl` has limited direct support for this on simple shapes).
        *   **Text (Titles within Shapes)**: Dark blue (theme `header_fg`), centered. Main titles are bold and larger (e.g., 20pt), subtitles are smaller and regular font (e.g., 10pt).
    *   **Overall Theme**: Predominantly dark blue and white, creating a professional and cohesive brand identity.
-   **Charts/Tables**: This skill provides the framework. Specific charts (donut charts, line charts, map charts, radar charts) and their data are intended to be placed within these shapes in subsequent steps.
-   **Theme Hooks**: `sidebar_bg`, `header_bg`, `header_fg`, `text_dark`.
