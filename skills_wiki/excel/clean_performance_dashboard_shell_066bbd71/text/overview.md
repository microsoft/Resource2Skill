...
> **Skill Name**: Clean Performance Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a presentation-ready dashboard by disabling worksheet gridlines, stashing pre-aggregated report data in an out-of-view region, and orchestrating a multi-chart layout (Stacked Column alongside multiple Line charts). 
* **Applicability**: Best used for executive summaries, KPI reports, and final presentation layers where visual cleanliness is paramount and data should be explored via charts rather than raw grid tables.

### 2. Structural Breakdown

- **Data Layout**: Places source aggregated data far to the right (e.g., columns AA-AI) to keep the primary viewport clean and act as a hidden data cache for the charts.
- **Formula Logic**: Purely visualization-focused; relies on static or pre-calculated aggregations for rendering the charts.
- **Visual Design**: Turns off native gridlines (`ws.sheet_view.showGridLines = False`). Implements a bold, themed header label.
- **Charts/Tables**: 
  - Chart 1: `BarChart` (Stacked Column) for dimensional breakdown (Market x Product).
  - Chart 2 & 3: `LineChart` for time-series trends (Units and Profit over time).
- **Theme Hooks**: Uses the primary text color and accent colors from the active theme for the header and can extend to chart series outlines.
