...
> **Skill Name**: Grouping/Outlining Rows

*   **Tier**: component
*   **Core Mechanism**: This skill applies Excel's built-in grouping feature to a specified range of rows, creating an interactive outline that allows users to collapse and expand detailed sections of a worksheet. This enhances navigation and readability by abstracting detail when not needed.
*   **Applicability**: Ideal for financial models, large reports, or any complex data presentation where different levels of detail need to be toggled. It helps organize data into logical sections (e.g., historical data, projections, specific financial statements) that can be easily hidden or revealed.

### 2. Structural Breakdown

-   **Data Layout**: A contiguous block of rows containing related data (e.g., an income statement breakdown). The skill assumes a header for this block exists at the top of the grouped range.
-   **Formula Logic**: The grouping skill itself does not generate formulas. However, in the context of the tutorial, it's applied to data that would typically contain formulas linking to other parts of the model or other sheets. Shortcuts like `F2` (edit cell/check links) and `F4` (anchor cells) are complementary to working with grouped data.
-   **Visual Design**: A distinctive header for the grouped section (e.g., dark blue background, white bold text), often with a bottom border. The grouped rows typically contain standard black text. Gridlines are often removed globally for a cleaner look (`Alt+W+V+G`).
-   **Charts/Tables**: Not directly applicable to this specific skill component.
-   **Theme Hooks**: `header_bg_color`, `header_text_color`, `border_color`.
