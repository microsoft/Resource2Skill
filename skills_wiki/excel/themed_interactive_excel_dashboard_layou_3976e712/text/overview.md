...
> **Skill Name**: Themed Interactive Excel Dashboard Layout

*   **Tier**: sheet_shell
*   **Core Mechanism**: Creates a dynamic and visually appealing dashboard layout on a single Excel worksheet. It features a navigable sidebar with hyperlinked icons and a main content area structured by rounded rectangular shapes. These shapes serve as containers for key performance indicators (KPIs) with dynamically linked text values and donut charts, as well as various trend, geographical, and satisfaction charts, all formatted with a consistent theme.
*   **Applicability**: Ideal for business users and analysts to quickly set up professional, interactive dashboards in Excel. It's best suited for scenarios where data is pre-processed in supporting sheets (like 'Inputs') and needs to be presented concisely across multiple visual elements on a single overview page, allowing users to navigate directly to detailed data or contacts.

### 2. Structural Breakdown

-   **Data Layout**: Assumes underlying data in a sheet named 'Inputs' (for KPI values, chart data ranges) and 'Contacts' sheet (for manager contact information) within the same workbook. Specific cells on the 'Inputs' sheet are referenced for dynamic content.
-   **Formula Logic**:
    -   **KPI Numerical Values**: Text boxes are dynamically linked to specific cells in the 'Inputs' sheet (e.g., `='Inputs'!D5` for Sales amount, `='Inputs'!D7` for Sales completion percentage).
    -   **Hyperlinks**: Icons in the sidebar are hyperlinked to other sheets within the workbook ('Inputs', 'Contacts') or to an external email address.
-   **Visual Design**:
    -   **Overall Theme**: Predominantly white content area with a contrasting dark blue sidebar, incorporating accent colors for visual elements.
    -   **Sidebar (Column A)**: Dark blue background (`theme.header_bg`). Contains static McDonald's logo and several interactive icons (e.g., dashboard, data grid, contacts, email, help), each hyperlinked.
    -   **Main Title Area**: A rounded rectangle (white fill, no line, subtle shadow) spanning columns B to M. Contains the dashboard title ("Sales Dashboard South America 2022") and subtitle ("Figures in millions of USD") in dark blue, bold, and varying font sizes.
    -   **KPI Boxes (3)**: Three smaller rounded rectangles (white fill, no line, subtle shadow) for "Sales", "Profit", and "# of Customers". Each contains its title (dark blue, bold), a dynamically linked large numerical value, and a donut chart with a dynamically linked percentage in a central text box.
    -   **Charts Areas (3)**:
        -   **2021-2022 Sales Trend (in millions)**: A large rounded rectangle for a line chart.
        -   **Customer Satisfaction**: A smaller rounded rectangle for a radar chart.
        -   **Sales by Country 2022**: A rounded rectangle for a map chart (visualized as a shape placeholder).
        All chart container shapes have white fill, no line, and subtle shadow, with titles in dark blue, bold.
    -   **Text Box Formatting**: No fill, no line. Font: Dark blue (`theme.text_main`), size 16 (for numbers) or 11 (for percentages), bold, centered.
    -   **Donut Chart Formatting**: No fill, no line for chart area. Series 1 (completed) `theme.accent_primary`, Series 2 (remainder) `theme.accent_secondary_light`. Donut hole size 65%.
    -   **Line Chart Formatting**: No fill, no line for chart area. Line 1 `theme.accent_warning` (red), Line 2 `theme.accent_primary` (dark blue). Markers: built-in circles, solid fill white, solid line matching series color, size 5.
    -   **Radar Chart Formatting**: No fill, no line for chart area. Line `theme.accent_primary`. Markers: built-in circles, solid fill white, solid line matching series color, size 5.
-   **Charts/Tables**:
    -   Three `openpyxl.chart.PieChart` instances (configured as donuts) for KPI completion percentages.
    -   One `openpyxl.chart.LineChart` for sales trend.
    -   One `openpyxl.chart.RadarChart` for customer satisfaction.
    -   A simple `openpyxl.drawing.shapes.Shape` (rectangle) acts as a placeholder for the geographical map chart, as `openpyxl` does not natively support filled map charts.
-   **Theme Hooks**: `header_bg`, `header_fg`, `accent_primary`, `accent_secondary_light`, `accent_warning`, `text_main`, `text_light`, `shadow_color`.
