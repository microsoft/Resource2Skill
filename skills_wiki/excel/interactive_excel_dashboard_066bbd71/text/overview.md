...
> **Skill Name**: Interactive Excel Dashboard

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs an interactive dashboard in Excel by converting raw tabular data into an Excel Table, from which multiple PivotTables are derived. Each PivotTable is then visualized using PivotCharts (stacked column for multi-category comparison, line charts for time series). The charts are arranged on a dedicated dashboard sheet, which is then made interactive by adding Slicers and Timelines (conceptually, as openpyxl doesn't directly support these). The dashboard is further refined by hiding gridlines and headings, and applying a custom theme.
*   **Applicability**: Ideal for business reporting where users need to explore key performance indicators (KPIs) and trends interactively without complex VBA. Suitable for any structured dataset with categorical and temporal dimensions that can be summarized into meaningful charts.

### 2. Structural Breakdown

-   **Data Layout**: Raw data is structured in a tabular format with columns including `Country`, `Product`, `Units Sold`, `Revenue`, `Cost`, `Profit`, and `Date`. This raw data is then explicitly converted into an Excel Table named "SalesData".
-   **Formula Logic**: Relies heavily on Excel's built-in PivotTable functionality to summarize and aggregate data (`SUM` for Units Sold, Revenue, Cost, Profit) across various dimensions (Country, Product, Date grouped by Month). No custom cell formulas are explicitly shown on the dashboard itself, but PivotTable calculations drive the charts.
-   **Visual Design**: The dashboard features a prominent merged cell header (A1:P3) for branding and title, styled with a dark background and light text. Gridlines and row/column headings are turned off for a cleaner, more report-like appearance. Chart areas are kept clean, removing default field buttons and legends where clarity is improved. The overall aesthetic is customizable through Excel's theme engine.
-   **Charts/Tables**:
    -   **Excel Table**: "SalesData" covers the raw data, allowing for automatic expansion and PivotTable updates upon refreshing.
    -   **PivotTables**: Three separate PivotTables are created on hidden helper sheets (these are logical representations as openpyxl doesn't fully support dynamic PivotTable creation from API):
        1.  Profit by Market & Cookie: Index by Country, Columns by Product, Values by Profit (Sum).
        2.  Units Sold each Month: Index by Date (grouped monthly), Values by Units Sold (Sum).
        3.  Profit by Month: Index by Date (grouped monthly), Values by Profit (Sum).
    -   **PivotCharts**: Three charts embedded on the dashboard sheet:
        1.  Stacked Column Chart: Visualizes Profit by Market & Cookie Type.
        2.  Line Chart: Shows Units Sold each Month.
        3.  Line Chart: Shows Profit by Month.
    -   **Slicers/Timeline (Conceptual)**: Placeholder for a Date Timeline, and Slicers for Country and Product are indicated. These are logically connected to all PivotTables to enable interactive filtering across all charts, though direct programmatic embedding of live slicers is a limitation of openpyxl.
-   **Theme Hooks**: `header_bg`, `text_light` (for header text), `neutral_bg` (for background elements like slicer placeholders), `border_color` (for visual separators/borders), and chart series colors (implicitly handled by Excel's theme when applied to PivotCharts).
