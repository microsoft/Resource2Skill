...
> **Skill Name**: Dynamic Scenario Matrix via CHOOSE

* **Tier**: sheet_shell
* **Core Mechanism**: Creates a scenario analysis engine by structuring assumptions into a central "Live Case" block and multiple specific cases (e.g., Upper, Lower). The Live Case uses the `CHOOSE` function, pointing to a central toggle cell, to dynamically pull in values from the corresponding specific case. The toggle cell utilizes Data Validation to restrict input to valid scenario index numbers.
* **Applicability**: Essential for financial models, project appraisals, or forecasting tools where users must instantly switch between different sets of assumptions (Optimistic, Pessimistic, Base) to observe the impact on calculated outputs like P&L.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Top Area**: A prominently labeled scenario toggle cell (e.g., cell F2) alongside the model output (a simple Income Statement).
  - **Middle Area**: "Live Case" assumptions that feed directly into the model's formulas.
  - **Bottom Area**: Hard-coded assumption blocks for "Scenario 1" and "Scenario 2".
- **Formula Logic**: Live case cells use `=CHOOSE($Toggle_Cell, Scenario_1_Cell, Scenario_2_Cell)`. Output models link *only* to the Live Case cells.
- **Visual Design**: Hard-coded input assumptions are colored blue (industry standard for financial modeling), while formula-driven cells are black. The toggle cell receives a bright highlight fill and border to indicate interactivity.
- **Data Validation**: A List validation rule is applied to the toggle cell (e.g., `"1,2"`) to prevent users from breaking the `CHOOSE` index.
- **Theme Hooks**: Utilizes `header_bg` and `header_fg` for statement headers, `input_fg` for hard-coded assumptions, and an `accent` or highlight color for the toggle cell.
