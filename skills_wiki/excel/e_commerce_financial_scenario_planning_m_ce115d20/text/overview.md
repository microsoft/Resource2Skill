...
> **Skill Name**: E-Commerce Financial Scenario Planning Model

*   **Tier**: archetype
*   **Core Mechanism**: This skill generates a comprehensive, dynamic 24-month financial planning model tailored for e-commerce businesses selling multiple product lines. It creates an "Assumptions" tab for all input variables and a "Model" tab for monthly calculations (revenue, gross profit, marketing spend, contribution margin), including a total revenue sanity check and a line chart visualizing product line revenues and contribution margin over time. The model is built with dynamic formulas, allowing real-time scenario analysis by adjusting assumptions.
*   **Applicability**: This skill is highly applicable for finance professionals, business analysts, or e-commerce managers who need to quickly build or enhance detailed revenue and profitability forecasts. It's suitable for scenario planning, budgeting, and understanding the financial impact of operational changes (e.g., changes in order value, growth rates, or marketing spend) for businesses with distinct product categories.

### 2. Structural Breakdown

-   **Data Layout**:
    -   **Assumptions Tab**:
        -   Clearly labeled sections for 'ACCESSORIES', 'DEVICES', and 'GENERAL' assumptions.
        -   Input variables include Average Order Value, Starting Monthly Orders, Monthly Growth Rate (%), Gross Margin (%), Forecast Horizon (Months), Model Start Date (Month), and Marketing Spend (% of Total Revenue).
        -   Input cells are distinct (e.g., blue font).
    -   **Model Tab**:
        -   Horizontal display of 24 monthly periods starting from the specified Model Start Date.
        -   Detailed sections for 'ACCESSORIES' and 'DEVICES' with rows for Average Order Value, Orders, Revenue, Gross Margin (%), and Gross Profit.
        -   'COMBINED TOTALS' section includes Total Revenue, Total Gross Profit, Blended Gross Margin (%), Marketing Spend (% of Rev), Marketing Spend ($), and Contribution Margin ($).
        -   'SANITY CHECK' row validates that sum of product line revenues equals total revenue for each month.
        -   A dedicated section for chart data, mirroring key monthly financial metrics.
-   **Formula Logic**:
    -   All calculation cells on the 'Model' tab reference corresponding input variables on the 'Assumptions' tab using dynamic cell references.
    -   Monthly Orders: `=(Previous_Month_Orders * (1 + Monthly_Growth_Rate))`
    -   Revenue: `=(Orders * Average_Order_Value)`
    -   Gross Profit: `=(Revenue * Gross_Margin_Percentage)`
    -   Total Revenue (Combined): `=(Accessories_Revenue + Devices_Revenue)`
    -   Total Gross Profit (Combined): `=(Accessories_Gross_Profit + Devices_Gross_Profit)`
    -   Blended Gross Margin (%): `=(Total_Gross_Profit / Total_Revenue)`
    -   Marketing Spend ($): `=(Total_Revenue * Marketing_Spend_Percentage_from_Assumptions)`
    -   Contribution Margin ($): `=(Total_Gross_Profit - Marketing_Spend_Dollars)`
    -   Sanity Check: `IF(SUM(Accessories_Revenue_Month_X, Devices_Revenue_Month_X) = Total_Revenue_Month_X, "✔ Match", "X Mismatch")`
-   **Visual Design**:
    -   Main titles are bold and larger.
    -   Section headers (e.g., "ACCESSORIES", "COMBINED TOTALS") are bold.
    -   Month headers are wrapped text, centered, with a light blue background and bold font.
    -   Input cells on the "Assumptions" tab have a blue font color.
    -   Monetary values are formatted as currency ($#,##0), and percentages as '0.0%'.
    -   Sanity check cells use light green fill with dark green text for "Match".
    -   Column widths are adjusted for readability.
-   **Charts/Tables**:
    -   One line chart: "Monthly Revenue & Contribution Margin by Product Line".
    -   X-axis: Months (Month 1 to Month 24).
    -   Y-axis: Amount ($).
    -   Three data series: Accessories Revenue (blue line), Devices Revenue (orange line), and Contribution Margin (green line).
-   **Theme Hooks**: `header_bg`, `header_fg`, `input_fg`, `border_color`, `chart_line_1` (for Accessories Revenue), `chart_line_2` (for Devices Revenue), `chart_line_3` (for Contribution Margin), `match_color`, `match_text_color`.
