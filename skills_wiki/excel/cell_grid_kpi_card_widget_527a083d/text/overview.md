...
> **Skill Name**: Cell-Grid KPI Card Widget

* **Tier**: component
* **Core Mechanism**: Builds a modular KPI card widget by formatting a 3x3 grid of contiguous cells to simulate a custom shape widget. It applies cohesive background colors, merged regions for centering main values, and an optional contrasting "badge" cell for secondary metrics (like market share percentage). This keeps the dashboard robust by avoiding fragile floating shape objects.
* **Applicability**: Ideal for executive dashboards, report headers, and summary sheets where top-level metrics need to stand out visually, mimicking a modern web dashboard layout while remaining purely grid-based and formula-linkable.

### 2. Structural Breakdown

- **Data Layout**: A 3x3 cell matrix spanning 3 columns and 3 rows. The top two rows are fully merged across for titles and subtitles. The bottom row is split between the main metric and an optional secondary "badge" metric.
- **Formula Logic**: Cell values are passed programmatically, but in a full sheet generation script, these can be set to `=SUM(...)` or direct references to pivot table data.
- **Visual Design**: Uses dark/bold background fills with high-contrast white text. Alignment is centered. Sizing is explicitly set for rows and columns to form a widget-like rectangular proportion.
- **Charts/Tables**: Replaces the need for SmartArt or Shape graphics by utilizing strict cell formatting.
- **Theme Hooks**: Utilizes `bg`, `fg`, `badge_bg`, and `badge_fg` to adapt to active color palettes (e.g., navy blue for corporate, dark grey for dark mode).
