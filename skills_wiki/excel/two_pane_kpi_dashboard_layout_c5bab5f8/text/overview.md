...
> **Skill Name**: Two-Pane KPI Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Establishes a modern app-like dashboard layout by hiding gridlines and segmenting the worksheet into two distinct color blocks. A dark, fixed-width left column serves as a side panel for the dashboard title and high-level KPIs, while a lighter main canvas area on the right hosts detailed data and charts. 
* **Applicability**: Best used for executive summaries, tracking scorecards, or interactive dashboards where high-level summary metrics (Orders, Revenue, Ratings) must be pinned to the left side while breakdowns and trends are explored on the right.

### 2. Structural Breakdown

- **Data Layout**: Column A is a narrow margin (width=2). Column B is the wide KPI side panel (width=25). Column C is another narrow margin. Columns D onwards form the main chart canvas.
- **Formula Logic**: None required for the shell itself, but in practice, KPI values placed in the side panel are typically linked to pivot table outputs on a separate calculation sheet.
- **Visual Design**: Gridlines are disabled. The side panel receives a solid dark fill (`primary_color`) with white, bold text. The main canvas receives a solid light fill (`bg_color`) to make charts blend seamlessly without standard cell borders.
- **Charts/Tables**: A standard BarChart is placed in the main canvas as a placeholder to demonstrate layout. Chart backgrounds can be made to match the canvas or kept white for contrast.
- **Theme Hooks**: Consumes `primary_color` (for the left panel background and main titles) and `bg_color` (for the main canvas background). 
