...
> **Skill Name**: Dynamic Scenario Forecast

* **Tier**: sheet_shell
* **Core Mechanism**: Drives live financial assumptions using the `CHOOSE` function tied to a Data Validation dropdown cell. This allows multiple isolated assumption blocks (e.g., Base, Upside, Downside) to instantly cascade into the main "Live Case" model without rewriting formulas.
* **Applicability**: Essential for financial modeling, budgeting, or operational forecasts where multiple what-if scenarios need to be evaluated and presented dynamically on a single dashboard.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Rows 4-8**: Live Income Statement (outputs).
  - **Rows 10-13**: Live Case Assumptions (pulls from active scenario).
  - **Rows 16-19**: Upper Case / Scenario 1 inputs.
  - **Rows 22-25**: Lower Case / Scenario 2 inputs.
- **Formula Logic**: 
  - `CHOOSE($H$2, C17, C23)` to pull the correct row based on the selected scenario index.
  - Basic arithmetic for Revenue (`Orders * AOV`) and COGS (`Orders * Cost`) linking back to the Live Case.
- **Visual Design**: The scenario selector cell is highlighted with a distinct background color and borders to indicate interactivity. Input assumptions are conventionally colored blue, while calculated live cells remain black.
- **Charts/Tables**: Clean tabular layout with bolded headers, grouped line items, and consistent column widths for temporal data (Year 1 to Year 5).
- **Theme Hooks**: Primary headers use a solid theme color (defaulting to dark blue) with white bold text. The selector cell uses an accent or highlight color (yellow).
