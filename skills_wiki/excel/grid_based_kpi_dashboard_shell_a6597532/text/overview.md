...
* **Tier**: sheet_shell
* **Core Mechanism**: Simulates a modern, floating-card dashboard UI entirely within the spreadsheet grid. It uses a pale background fill for the "canvas" and white filled cell ranges for "cards", eliminating the need for complex, floating shapes. It integrates KPI metric blocks and inline conditional formatting (Data Bars) for tabular data.
* **Applicability**: Best for executive summaries and high-level metric dashboards where you want a clean, web-like layout directly in Excel. This pattern is highly resilient and doesn't break when users resize rows/columns, unlike floating shapes.

### 2. Structural Breakdown

- **Data Layout**: 
  - Margin columns (A, D, G, J, M) act as "gutters" between cards.
  - Symmetrical 2-column wide blocks (B:C, E:F, H:I, K:L) hold the top KPI metrics.
  - Large blocks (B:F and H:L) hold the main charts and tables below the KPIs.
- **Formula Logic**: Acts as a presentation shell; data values are injected statically or can be wired to other sheets via standard cell references.
- **Visual Design**: 
  - **Canvas**: Filled with a soft, muted color (e.g., `#F2EFF5`).
  - **Header Block**: Deep primary color with contrasting white and accent text.
  - **Cards**: White fill with a thick accent-colored top border to draw the eye. Gridlines are removed globally.
- **Charts/Tables**: Demonstrates a summary table using openpyxl's `DataBarRule` to create inline bar charts directly in the cells.
- **Theme Hooks**: Consumes `primary`, `bg_light`, `accent`, `text_light`, `text_dark`, and `text_muted`.
