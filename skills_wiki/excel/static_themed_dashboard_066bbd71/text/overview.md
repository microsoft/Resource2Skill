...
> **Skill Name**: Static Themed Dashboard

* **Tier**: archetype
* **Core Mechanism**: Generates a gridless presentation layer with a thematic header and pre-arranged standard charts. Summary data is stored in a hidden `Data` worksheet, keeping the front-end dashboard clean and focused on visuals.
* **Applicability**: Best for executive summaries and KPI reporting where a polished, multi-chart layout is needed, but complex interactive PivotTables or Slicers are either unnecessary or unsupported by the generation environment.

### 2. Structural Breakdown

- **Data Layout**: Data is segregated onto a hidden `Data` worksheet with discrete table ranges separated by empty rows (rows 1-5, 10-14, 20-24) to neatly isolate the series for each chart.
- **Formula Logic**: No in-cell formulas are used; chart `Reference` objects are carefully mapped to the hidden data sheet ranges to drive the visuals dynamically.
- **Visual Design**: The `Dashboard` sheet hides all gridlines (`showGridLines = False`). Cells A1:R4 are merged to form a commanding top header banner utilizing the theme's primary color.
- **Charts/Tables**: 
  - One Stacked Column Chart (`BarChart` with `type="col"`, `grouping="stacked"`, `overlap=100`).
  - Two Line Charts (`LineChart`) with legends disabled for a cleaner look.
  - Charts are dimensioned explicitly and anchored to specific cell coordinates (B6, J6, J17) to establish a grid layout.
- **Theme Hooks**: `primary` (Header Background), `text_on_primary` (Header Font).
