...
> **Skill Name**: Dynamic Scenario Financial Model

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a fully linked 5-year financial projection model (Income Statement) driven by a dynamic assumptions block. It uses Data Validation to create a dropdown scenario toggle, and the `CHOOSE` function to dynamically pull inputs from multiple hardcoded scenario blocks (e.g., Upper Case, Lower Case) into the "Live Case" block, which powers the P&L calculations.
* **Applicability**: Perfect for financial modeling, forecasting, budgeting, and sensitivity analysis where end-users need to toggle a single control to review different business outcomes (e.g., Base, Best, and Worst-case scenarios) without modifying the core calculations.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Top Row (Row 2)**: Scenario selector with a highlighted input cell.
  - **Rows 4-12**: The core Income Statement (Revenue down to Net Profit), referencing only the Live Case block.
  - **Rows 14-20**: "Live Case" assumptions block (populated via formulas).
  - **Rows 22-28 & 30-36**: Hardcoded assumption blocks for Scenario 1 (Upper Case) and Scenario 2 (Lower Case).
- **Formula Logic**: 
  - Scenario Pull: `=CHOOSE($G$2, C23, C31)` populates the Live Case.
  - Revenue: `=C16*C17` (Orders * AOV).
  - COGS: `=C16*C18` (Orders * COGS per order).
  - Corporate Tax: `=IF(C10>0, C10*C20, 0)` to prevent tax credits on losses.
- **Visual Design**: Uses a distinct bold fill (e.g., yellow) to indicate the interactive Scenario toggle cell. Section headers (Income Statement, Live Case, Scenarios) use dark thematic backgrounds with bold white text.
- **Charts/Tables**: Standard spreadsheet projection layout (Columns C through G represent Years 1 through 5).
- **Theme Hooks**: Utilizes `header_fill` for section separation and standard text colors.
