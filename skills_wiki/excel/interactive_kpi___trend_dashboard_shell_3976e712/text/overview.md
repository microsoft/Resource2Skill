...
> **Skill Name**: Interactive KPI & Trend Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a modern, shape-free dashboard layout using an Excel grid system. Employs background fills to create elevated "cards" on a shaded canvas, features customized Doughnut charts for target tracking, and styles Line/Radar charts with minimalist formatting (no gridlines, custom markers, removed borders).
* **Applicability**: Best for executive summaries, financial overviews, and KPI reporting where a clean, web-like UI is required natively in Excel without relying on fragile floating shapes or macros.

### 2. Structural Breakdown

- **Data Layout**: Consists of two sheets: a hidden `[Name]_Data` sheet acting as the semantic layer (storing KPI targets, timeseries, and scoring matrices), and a visible presentation sheet.
- **Formula Logic**: Direct `Reference` bindings bridge the visible dashboard charts to the hidden data structure, ensuring the frontend remains structurally clean. 
- **Visual Design**: Uses a dark vertical sidebar (`Column A`) for navigation hierarchy. The sheet background is shaded a light gray (`F5F6F8`) with gridlines disabled. Specific cell clusters (e.g., `C4:F8`) are filled solid white with a subtle border to mimic interactive web cards.
- **Charts/Tables**: 
  - **Doughnut Charts**: Used as progress rings. `holeSize` is increased to 65% for a modern look, and slices are manually mapped to primary/gray tones using `DataPoint` targeting.
  - **Line Chart**: Stripped of gridlines and legends. The primary data series is highlighted using increased line thickness and a custom white-filled circular marker with a thick colored border.
  - **Radar Chart**: Used for multivariate satisfaction scoring, styled to match the primary theme palette.
- **Theme Hooks**: Utilizes `primary` for the sidebar, key markers, and chart series; `bg` for the canvas; and `card` (white) for the container areas.
