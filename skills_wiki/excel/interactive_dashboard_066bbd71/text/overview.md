...
> **Skill Name**: Interactive Dashboard

* **Tier**: archetype
* **Core Mechanism**: This skill constructs a multi-chart, interactive performance dashboard from raw tabular sales data. It leverages Excel's native PivotTable and PivotChart features for data summarization and visualization. The *interactive* aspect, demonstrated by dynamic filtering via Slicers and Timelines, allows users to explore data trends across different dimensions. While the underlying data preparation, chart creation, and thematic styling are reproducible programmatically, the direct insertion and linking of Slicers and Timelines are primarily UI-driven features in Excel, and their full programmatic reproduction in `openpyxl` is limited.
* **Applicability**: Ideal for business users to quickly visualize key performance indicators (KPIs) and explore data trends across different dimensions (e.g., market, product, time). Suitable for any tabular dataset that requires dynamic filtering and multiple linked visualizations for management reporting or operational monitoring.

### 2. Structural Breakdown

- **Data Layout**: Raw sales data (Country, Product, Units Sold, Revenue, Cost, Profit, Date) is organized into an Excel Table on a dedicated "Data" worksheet. Helper sheets for PivotTable summary data are generated and subsequently hidden.
- **Formula Logic**: Primarily relies on PivotTable aggregations (SUM of Profit, SUM of Units Sold). The displayed numeric values in charts are derived from these summarized PivotTable outputs.
- **Visual Design**:
    - The main "Dashboard" sheet has gridlines and row/column headings turned off for a clean, report-like appearance.
    - A prominent header featuring a company logo (conceptual) and dashboard title is placed at the top.
    - Charts and slicer placeholders are precisely aligned using snapping (conceptual) and alignment tools.
    - Number formatting (currency, commas, no decimals) is applied to PivotTable values.
    - The overall color scheme, fonts, and chart styles are managed by applying an Excel Theme to the workbook.
- **Charts/Tables**:
    - **Chart 1 (Profit by Market & Cookie Type)**: Stacked Column Chart, derived from PivotTable data, showing total profit broken down by cookie type for various countries. Countries are sorted by total profit (descending).
    - **Chart 2 (Units Sold each month)**: Line Chart, derived from PivotTable data, illustrating monthly unit sales trends.
    - **Chart 3 (Profit by month)**: Line Chart, derived from PivotTable data, illustrating monthly profit trends.
    - **Slicers (Conceptual)**: Placeholder Slicers for 'Country' and 'Product' dimensions, allowing dynamic filtering of linked charts. Headers are removed for conciseness.
    - **Timeline (Conceptual)**: Placeholder Timeline for the 'Date' dimension, enabling interactive time-based filtering.
- **Theme Hooks**: The skill consumes various theme palette tokens for comprehensive styling: `header_bg`, `header_fg`, `accent`, `accent_light` (for slicer backgrounds), `text_color`, and `chart_colors`. The overall workbook theme is applied to control general fonts and colors.
