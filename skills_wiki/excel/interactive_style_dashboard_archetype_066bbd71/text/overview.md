...
> **Skill Name**: Interactive-Style Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Transforms a standard Excel workbook into a presentation-ready dashboard. It employs a three-sheet architecture: a "Raw Data" sheet formatted as a formal Table, a hidden "Calc Data" sheet for aggregations, and a "Dashboard" sheet with hidden gridlines, a themed title banner, and precisely aligned charts (a stacked bar and two trend lines).
* **Applicability**: Best used for executive summaries, KPI reporting, and visual data overviews where you want to isolate the presentation layer from the underlying data and calculations.

### 2. Structural Breakdown

- **Data Layout**: Employs a robust three-tier architecture: 
  1. `Dashboard` (Presentation)
  2. `Calc Data` (Aggregations driving the charts, often hidden)
  3. `Raw Data` (Source records formatted as a Table for easy expansion).
- **Formula Logic**: Standardizes chart references to point strictly to the `Calc Data` matrix blocks, keeping the presentation sheet devoid of raw numbers.
- **Visual Design**: Turns off `showGridLines` on the Dashboard sheet to create a clean "canvas". A merged range (`A1:N3`) serves as a solid-fill header banner driven by the theme's primary colors.
- **Charts/Tables**: Places a large Stacked Column chart on the left for categorical comparisons, and stacks two Line charts vertically on the right for time-series trends. Removes legends from single-series line charts to maximize data-ink ratio.
- **Theme Hooks**: Consumes `primary_bg` and `primary_fg` for the dashboard title banner, providing an instant corporate-branded look.
