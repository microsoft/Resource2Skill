...
> **Skill Name**: Two-Tone Dashboard Canvas & KPI Strip

* **Tier**: sheet_shell
* **Core Mechanism**: Creates a modern, shape-free dashboard layout by applying a split-color background (dark header band, light body band) using row-wide fills, and overlays a horizontal strip of KPI "cards" built entirely from merged cells. This avoids fragile floating shapes while perfectly replicating the sleek, interactive dashboard aesthetic.
* **Applicability**: Ideal for the landing sheet of an analytical workbook or executive summary. Use when you need a professional, high-contrast dashboard framework to anchor pivot charts and tables.

### 2. Structural Breakdown

- **Data Layout**: Canvas uses rows 1-8 as a dark header and rows 9-40 as a light body area. KPIs anchor at row 5, bridging the header space visually. 
- **Formula Logic**: Purely static layout preparation; expects calculated KPI values to be passed in. 
- **Visual Design**: Disables gridlines. Uses contrasting text colors for the title/subtitle. Simulates floating cards by giving a thin left column an accent fill (simulating an icon block) and merging the adjacent column for the values and labels.
- **Charts/Tables**: Leaves the body grid (rows 9+) open for chart/table components to be layered in.
- **Theme Hooks**: Consumes `primary` for the header band, `secondary` (lightened) for the background body, `accent` for the KPI card highlights, and `bg` for the card panels.
