...
> **Skill Name**: Interactive Dashboard Shell with Themed KPIs & Chart Layout

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill constructs a complete interactive dashboard layout on a single Excel sheet. It utilizes static background shapes to define visual zones (e.g., sidebar, main content area) and dynamically links Key Performance Indicators (KPIs) from underlying PivotTables into formatted text boxes with visual enhancements like emojis. Placeholder elements are created for various charts and slicers, demonstrating their intended placement and interactive connections across the dashboard.
*   **Applicability**: This skill is ideal for creating professional-looking, interactive business dashboards in Excel (even older versions like Excel 2016) without relying on specialized external tools. It's suitable for presenting aggregated data, visualizing trends, and allowing users to filter data dynamically. Useful for small businesses, consultants, or anyone needing to quickly create impactful data reports using familiar Excel features.

### 2. Structural Breakdown

-   **Data Layout**:
    -   **'Data' Sheet**: Contains the raw transactional data in an Excel Table (e.g., named 'Orders'). Includes columns like `TX ID`, `Product`, `Quantity`, `Unit Price`, `Amount`, `Order Date`, `Ship Date`, `Customer Gender`, `Order Mode`, `Rating C`, `State`, `County`.
    -   **Calculated Columns in 'Data'**:
        -   `Days to Deliver`: `= [@[Ship Date]] - [@[Order Date]]` (calculates shipment duration).
        -   `Weeknum`: `= WEEKNUM([@[Order Date]])` (derives the week number for trend analysis).
        -   `Gender Value`: `= IFS([@[Customer Gender]]="M","Male",[@[Customer Gender]]="F","Female",[@[Customer Gender]]="O","Other",TRUE,"Unknown")` (converts shorthand gender codes to full names for cleaner display and slicers).
    -   **'Pivots' Sheet**: Houses multiple PivotTables derived from the 'Data' table. Each pivot aggregates data for a specific visual (e.g., `pvfSummary` for overall KPIs, `pvfTrend` for weekly data, `pvfOrderModeGender` for purchase patterns, etc.).
    -   **'Pivots' Display Area**: A dedicated section on the 'Pivots' sheet (e.g., F2:K4) where key KPI metrics (Orders, Quantity, Amount, Avg. Rating, Avg. Days) are linked directly from their respective PivotTables. These cells use `TEXT` formulas for custom number formatting (e.g., `$#,##0,.0k` for thousands in currency).
    -   **'Pivots' Heatmap Matrix**: A linked and conditionally formatted cell range on the 'Pivots' sheet mirroring the 'Order Mode' by 'Gender' pivot table, used for dynamic heatmap display.

-   **Formula Logic**:
    -   KPI Text Box Linking: Text boxes on the Dashboard sheet are linked to the formatted KPI cells in the 'Pivots' Display Area. Example for Orders: `='Pivots'!G4`.
    -   Heatmap Linked Picture: The heatmap visual is created by copying the conditionally formatted matrix from the 'Pivots' sheet and pasting it onto the Dashboard sheet using "Paste Special" -> "Linked Picture". This ensures the visual updates dynamically with changes to the underlying pivot data and conditional formatting.
    -   Pivot Table Naming: Each PivotTable on the 'Pivots' sheet is given a descriptive name (e.g., `pvfSummary`, `pvfTrend`) via "PivotTable Analyze" -> "PivotTable Name". This facilitates easier management and connection for slicers.

-   **Visual Design**:
    -   **Dashboard Layout**: Defined by two main `Shape` objects (rectangles) on the 'Dashboard' sheet: a dark green sidebar for slicers and KPIs, and a light green main area for charts. Both shapes have subtle drop shadows for depth.
    -   **Custom Color Theme**: A custom color palette (e.g., 'VivaCalif') is defined under "Page Layout" -> "Colors" -> "Customize Colors". This theme dictates the default colors for all charts and shapes, allowing for global style changes with a single click to match corporate branding.
    -   **KPI Text Boxes**: Bold, large font (e.g., Aptos Narrow 18), white text color. Emojis are used for visual cues.
    -   **Chart Formatting**: All chart elements (field buttons, legends, gridlines, titles) are selectively hidden or formatted to maximize data-ink ratio and align with a clean aesthetic. Axis labels are adjusted for readability and position (e.g., "Categories in reverse order" for bar charts, moving labels to 'High' position for clearer view).
    -   **Borders**: White solid line borders (0.75pt) are applied between map counties and to heatmap cells for visual separation.

-   **Charts/Tables**:
    -   **KPI Summary**: Text boxes with dynamic values linked to PivotTable outputs.
    -   **Last 13 Week Trends**: Line chart (e.g., `Line with Markers`) showing `Sum of Quantity` and `Sum of Amount` over `Weeknum`, with `Sum of Amount` on a secondary axis.
    -   **How they like to buy**: Heatmap (linked picture) displaying `% of Grand Total` of `Amount` by `Order Mode` and `Gender`.
    -   **How many they buy**: Column chart (e.g., `Clustered Column`) showing `Count of TX ID` by grouped `Quantity` bins (1-5, 6-10, >10).
    -   **Which Products are Popular**: Stacked Bar chart (e.g., `Stacked Bar`) showing `Sum of Quantity` by `Product` (sorted descending) broken down by `Gender Value`.
    -   **Overall Gender Split**: Donut chart showing `Sum of Quantity` by `Gender Value`, with percentage data labels.
    -   **Where our customers live**: Two Map charts (e.g., `Filled Map`), one for `Quantity` and one for `Amount`, showing data distribution by `County` in California. Smaller amount map placed over quantity map.
    -   **How long to ship**: Column chart (e.g., `Clustered Column`) showing `Count of TX ID` by `Days to Deliver`.
    -   **Customer Satisfaction**: Clustered Column chart (e.g., `Clustered Column`) showing `Count of TX ID` by `Months` and `Rating C`.
    -   **Chart Titles**: Dynamically linked to business questions from the 'Questions & KPIs' sheet, or explicitly typed, and formatted consistently.

-   **Theme Hooks**:
    -   `Rectangle` fill: Uses dark green for sidebar and light green for main dashboard (defined in the custom theme's accent colors).
    -   `Text Box` text fill: Uses white or dark grey, depending on background (text/background colors from theme).
    -   `Charts`: All chart elements (bars, lines, backgrounds) automatically inherit colors from the active theme's accent colors, providing a cohesive look.
    -   `Conditional Formatting`: Color scales are defined using theme colors (e.g., Blue-White-Red or Sequential Green/Orange).
