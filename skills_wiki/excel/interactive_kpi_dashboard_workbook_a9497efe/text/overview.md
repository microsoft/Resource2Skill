...
> **Skill Name**: Interactive KPI Dashboard Workbook

* **Tier**: archetype
* **Core Mechanism**: Creates a multi-sheet architecture separating raw data from presentation. Uses a Data Validation dropdown to drive `INDEX`/`MATCH` formulas inside stylized "KPI Cards", applying dynamic conditional formatting to highlight whether the current period meets its target.
* **Applicability**: Executive reporting, monthly scorecards, and high-level operational dashboards where users need to flip through periods and instantly see performance against targets.

### 2. Structural Breakdown

- **Data Layout**: Separation of concerns. A `Data` sheet holds rows of metrics (Actual, Target, Prior) across columns representing months. A `Dashboard` sheet holds the UI.
- **Formula Logic**: Uses `=INDEX(Data_Range, 1, MATCH(Selected_Month, Month_Headers, 0))` to dynamically fetch the exact KPI value for the dropdown's active month.
- **Visual Design**: KPI Cards are built by merging columns for the header and main value, while keeping the footer unmerged to store comparative metrics side-by-side.
- **Charts/Tables**: Emphasizes "Big Number" displays with conditional formatting (`CellIsRule`) referencing a target cell inside the same card, coloring the text and background green/red.
- **Theme Hooks**: Consumes `primary`, `secondary`, `success_bg`/`success_fg`, and `danger_bg`/`danger_fg` tokens.
