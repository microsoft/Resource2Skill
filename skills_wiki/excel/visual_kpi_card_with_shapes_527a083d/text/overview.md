...
> **Skill Name**: Visual KPI Card with Shapes

*   **Tier**: component
*   **Core Mechanism**: This skill constructs a visual Key Performance Indicator (KPI) card using multiple Excel drawing shapes (a rounded rectangle and an oval). It populates these shapes with static text derived from specified Excel cells, applying theme-aware colors, fonts, and alignment. The shapes are positioned to form a cohesive visual unit on the worksheet.
*   **Applicability**: Use this skill to create visually appealing and branded KPI displays in dashboards or reports. It's suitable for highlighting key metrics (e.g., revenue, market share, growth rates) for individual categories or totals. The visual design enhances readability and draws attention to critical data points. Note that the dynamic linking of shape text to cell values (as demonstrated in the Excel UI) is not directly supported by `openpyxl`, so the displayed values are static representations at the time of generation.

### 2. Structural Breakdown

-   **Data Layout**: The KPI cards reference data from a simple tabular layout (e.g., `Region` in A2, `Revenue` in B2, `Market Share` in C2). The component expects cell references for a region name, a revenue figure, and a market share percentage.
-   **Formula Logic**: In the tutorial, shapes are dynamically linked to cell values by selecting the shape and typing an `=CELL_REFERENCE` (e.g., `=$B$2`) into the formula bar. This dynamic linking, where the shape's text updates automatically with cell changes, is a feature of the Excel UI and is **not directly reproducible** with `openpyxl`. The reproduction code will populate shapes with static text based on the *current value* of the referenced cells.
-   **Visual Design**:
    *   **Main KPI Card (Rounded Rectangle)**:
        *   Shape Fill: Navy blue (or `theme.primary_dark`).
        *   Shape Outline: No outline.
        *   Text: Multi-line, centered. Top line is a region name (e.g., "Asia") in bold, 18pt, white font. Subsequent lines are "Revenue" and the revenue value (e.g., "$369,989") in bold, 11pt, white font.
    *   **Market Share Indicator (Oval)**:
        *   Shape Fill: White (or `theme.fill_light`).
        *   Shape Outline: No outline.
        *   Text: Market share percentage (e.g., "5%") in bold, 11pt, black font, centered within the oval.
    *   **Positioning**: The oval is placed within the main rectangle, towards the bottom-right corner, appearing as an overlay.
-   **Charts/Tables**: This skill does not generate charts or tables directly; it focuses on individual visual KPI components.
-   **Theme Hooks**: `primary_dark` (for main card background), `text_light` (for main card text), `fill_light` (for oval background), `text_dark` (for oval text).
