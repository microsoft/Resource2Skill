...
> **Skill Name**: Static Multi-Chart Dashboard Canvas

* **Tier**: archetype
* **Core Mechanism**: Generates a unified 'Dashboard' worksheet with gridlines disabled and an anchored layout of multiple openpyxl charts (Stacked Column and Line). Sources data from a hidden secondary worksheet and applies explicit cell number formatting so chart axes automatically display as formatted currency/units.
* **Applicability**: Best used when generating static reporting dashboards where data is pre-aggregated via Python. *(Note: While the video uses PivotCharts and Slicers, openpyxl does not natively support creating interactive Slicers from scratch. This archetype reproduces the final clean dashboard layout and chart composition using standard charts and a hidden data sheet.)*

### 2. Structural Breakdown

- **Data Layout**: A hidden "Data" worksheet stores the aggregated plot points sequentially.
- **Formula Logic**: Relies on static data injection (Python-side aggregation) since openpyxl chart references target static ranges.
- **Visual Design**: Disables gridlines (`showGridLines = False`) to create a blank canvas effect. Centers a large, themed title across the top rows. 
- **Charts/Tables**: 
  - A Stacked BarChart (`overlap = 100`) for categorical breakdown.
  - Two LineCharts with legends disabled (`legend = None`) to maximize plot area.
- **Theme Hooks**: Uses the theme's `primary` color for the main dashboard title.
