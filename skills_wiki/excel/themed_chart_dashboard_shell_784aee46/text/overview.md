...
> **Skill Name**: Themed Chart Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a presentation-ready dashboard by disabling gridlines, building a full-width themed header using merged cells, and arranging multiple charts (Line, Clustered Column) in a structured grid layout. Safely routes chart source data to a dedicated hidden worksheet to keep the presentation layer clean.
* **Applicability**: Best used for generating static, executive-facing dashboards that summarize KPIs, trends, and categorical comparisons without exposing or cluttering the visual area with raw data tables.

### 2. Structural Breakdown

- **Data Layout**: A separate, hidden data worksheet (`<sheet_name>_Data`) is generated to hold tabular data for the charts (e.g., Monthly Trend, Category Comparison, Top 5 Performers).
- **Formula Logic**: None (data is assumed to be aggregated pre-render or written directly as static values for the chart references).
- **Visual Design**: Sheet gridlines are disabled (`showGridLines = False`). A prominent header spans `A1:N3` using the primary theme color and bold, centered text.
- **Charts/Tables**: Employs `LineChart` for time-series trends and `BarChart` (type `col`) for categorical rankings. Charts are anchored strategically (e.g., `B5`, `J5`, `B16`) to create a 2-column dashboard grid.
- **Theme Hooks**: Uses `primary` for the header background and `text` for the header font color.
