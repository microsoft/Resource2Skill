...
> **Skill Name**: Static Multi-Chart Dashboard

* **Tier**: archetype
* **Core Mechanism**: Separates presentation from raw numbers by creating a "Data" sheet for aggregated metrics and a "Dashboard" sheet with disabled gridlines. A styled banner is placed across the top, and multiple charts (Stacked Bar, Line) are instantiated and anchored seamlessly to form a cohesive view.
* **Applicability**: Ideal for creating executive summaries, KPIs, and performance dashboards. Because `openpyxl` cannot natively generate interactive Slicers or PivotTables, this archetype relies on feeding pre-aggregated tabular data into the "Data" sheet to drive the presentation visuals.

### 2. Structural Breakdown

- **Data Layout**: Pre-aggregated tables written sequentially on the `Data` sheet (e.g., Matrix for Stacked Columns, Time-series table for Line charts), isolated from the viewer.
- **Formula Logic**: Pure data bindings via `Reference` objects linking the `Dashboard` charts to the `Data` sheet.
- **Visual Design**: 
  - `ws.sheet_view.showGridLines = False` on the presentation sheet to give it a canvas/web-app feel.
  - Merged header (A1:N3) with a bold theme-driven background fill and centered large typography.
- **Charts/Tables**: 
  - `BarChart` (Type: 'col', Grouping: 'stacked', 100% overlap) for categorical breakdowns.
  - 2x `LineChart` for trend analysis, with legends disabled (`chart.legend = None`) for cleaner single-series display.
- **Theme Hooks**: Utilizes `primary` for the massive header background and `text` (white) for header text.
