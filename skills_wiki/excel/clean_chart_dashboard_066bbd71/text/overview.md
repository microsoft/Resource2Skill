...
> **Skill Name**: Clean Chart Dashboard

* **Tier**: archetype
* **Core Mechanism**: Configures a dedicated dashboard worksheet by hiding gridlines, adding a prominent branded title banner, and arranging perfectly aligned charts fed by a separate data sheet.
* **Applicability**: Ideal for executive summaries and KPI reporting where visual clarity is paramount. Separating raw data onto a different sheet ensures the presentation layer remains a clean, professional aesthetic.

### 2. Structural Breakdown

- **Data Layout**: Raw data resides on a dedicated "Data" sheet; aggregated visualizations sit on a clean "Dashboard" sheet.
- **Formula Logic**: Not heavily formula-driven; relies on charting primitives directly linked to data ranges on the backend sheet.
- **Visual Design**: Gridlines are completely disabled (`showGridLines = False`). The top rows are merged into a large, themed title banner with bold, contrasting text and centered alignment.
- **Charts/Tables**: Includes a `BarChart` and `LineChart`, sized consistently (height/width) and placed at specific cell anchors (e.g., `B5`, `K5`) to emulate "snap-to-grid" alignment without manual dragging.
- **Theme Hooks**: The title banner background consumes `primary` theme colors, with text utilizing a high-contrast foreground color (`text`).
