...
> **Skill Name**: Presentation Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Builds a multi-sheet workbook separating presentation from data. It populates aggregated summary tables into a hidden `ChartData` sheet, then builds a clean front-end `Dashboard` sheet. The dashboard hides native Excel gridlines and row/col headers, features a bold thematic title, and precisely aligns multiple openpyxl charts (Stacked Column, Line) on the blank canvas.
* **Applicability**: Best used for executive summaries, high-level KPI reporting, and any automated pipeline where the final deliverable is a visual "dashboard" rather than a raw data table.

### 2. Structural Breakdown

- **Data Layout**: A hidden `ChartData` worksheet containing multiple contiguous summary tables, allowing clear separation of charting data ranges without visually cluttering the workbook.
- **Formula Logic**: Aggregation is handled via Python prior to sheet population, allowing the workbook to remain lightweight and fast.
- **Visual Design**: The dashboard sheet disables `showGridLines` and `showRowColHeaders` via `sheet_view`. A large merged range (`B2:O4`) forms a prominent header, styled with a primary theme color background and bold text.
- **Charts/Tables**: 
  - One Stacked Column Chart (`type="col"`, `grouping="stacked"`, `overlap=100`) for categorical breakdowns.
  - Two Line Charts for time-series trends.
  - All charts dynamically reference the hidden `ChartData` sheet.
- **Theme Hooks**: Consumes a `primary` color for the header background and `text` color for the header typography. 
