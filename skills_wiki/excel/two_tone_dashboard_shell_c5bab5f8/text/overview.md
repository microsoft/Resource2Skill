...
* **Tier**: sheet_shell
* **Core Mechanism**: Sets up a grid-based dashboard canvas by applying contrasting `PatternFill` colors to create a dark control sidebar and a light main content area. It disables gridlines and programmatically paints white "cards" with borders across specific cell ranges to act as clean, aligned containers for charts, maps, and tables. 
* **Applicability**: Best for single-page executive summaries or KPI dashboards. Using cell background fills rather than floating shapes (as done in manual Excel) guarantees robust, pixel-perfect alignment across different screen sizes and environments when generating reports via Python.

### 2. Structural Breakdown

- **Data Layout**: Column A acts as a left margin. Column B is widened to serve as the Sidebar for high-level KPIs or future Slicers. Column D acts as a gutter. Columns E through N form the main dashboard grid.
- **Formula Logic**: None in the shell itself (serves as a pure presentation layer).
- **Visual Design**: Gridlines disabled. Left panel filled with a dark primary color, main panel filled with a light background color. Chart containers ("widgets" or "cards") are filled with solid white and encapsulated in a thin gray border.
- **Charts/Tables**: This shell creates the *containers* for charts, setting up headers and bounding boxes where charts (like Line charts or Doughnut charts) should be anchored. 
- **Theme Hooks**: Uses a `theme` dictionary to map `sidebar`, `canvas`, `card`, `text_light`, `text_dark`, and `border` colors. Fallbacks ensure it safely defaults to a Corporate Blue or Botanical Green (matching the video's aesthetic) depending on the passed parameter.
