...
> **Skill Name**: Gridless KPI Dashboard Shell

* **Tier**: archetype
* **Core Mechanism**: Constructs a presentation-ready dashboard by separating data into hidden worksheets and building a clean, grid-free front layer. Arranges multiple coordinated charts (Stacked Column, Line) into a unified grid, unified by a bold banner header. 
* **Applicability**: Best for executive summaries, performance reports, or any workbook meant to be consumed visually rather than analyzed row-by-row. Emulates the structural foundation of an interactive dashboard before Slicers are manually applied.

### 2. Structural Breakdown

- **Data Layout**: Summary dimensions and metrics are organized into contiguous tables on a dedicated `ChartData` sheet, which is then hidden to protect the clean presentation layer.
- **Formula Logic**: Charts use explicit `Reference` objects spanning across worksheets to pull live data into the presentation layer without exposing the underlying cells.
- **Visual Design**: The dashboard sheet features `showGridLines = False`. A merged header block uses a dark theme background and contrasting white text to anchor the report visually. 
- **Charts/Tables**: Uses a `BarChart` with `grouping="stacked"` for categorical breakdowns (e.g., Market by Product) and multiple `LineChart`s for temporal trends, explicitly sizing them to tile nicely.
- **Theme Hooks**: The dashboard banner relies on the theme's primary/accent colors (e.g., `theme_bg` and `theme_fg`) to match organizational branding.
