...
> **Skill Name**: Interactive App-Like Dashboard Shell

* **Tier**: archetype
* **Core Mechanism**: Constructs a modern web-app layout by using a narrow, dark-filled left column for hyperlinked navigation icons. Removes native gridlines, fills the canvas with a soft gray background, and builds clean metric "cards" using merged cells with white fills and subtle borders. 
* **Applicability**: Best for top-level summary workbooks where users need an intuitive, interactive UI to seamlessly navigate between a visual dashboard and the underlying raw data sheets.

### 2. Structural Breakdown

- **Data Layout**: A two-sheet architecture ("Dashboard" and "Data"). Raw metrics and chart series are safely stored in the backend Data sheet, keeping the Dashboard purely presentational.
- **Formula Logic**: KPI card values are driven by simple direct references (`=Data!B2`) keeping them dynamically linked to the backend sheet. 
- **Visual Design**: 
  - **Sidebar**: Column A is narrowed (width 8) and flooded with dark blue (`#1E3A8A`), populated with large emoji icons mapped to `#SheetName!A1` hyperlinks.
  - **Canvas**: Filled with light gray (`#F3F4F6`), creating negative space between elements.
  - **Cards**: White (`#FFFFFF`) with a thin gray border (`#D1D5DB`) to simulate elevation. 
- **Charts/Tables**: Clean line and bar charts with legends and major tick marks removed to fit seamlessly into the modern, minimalist card aesthetic.
