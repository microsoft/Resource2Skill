...
> **Skill Name**: Executive Dashboard Layout

* **Tier**: archetype
* **Core Mechanism**: Creates a polished presentation canvas by disabling worksheet gridlines, building a full-width themed header, and reserving a left-hand sidebar for Slicers. It simulates a PivotTable-driven dashboard by aggregating data on a hidden calculation sheet and routing it to precisely positioned Stacked Column and Line charts on the main canvas.
* **Applicability**: Ideal for high-level KPI reporting and management dashboards. It sets up the exact structural foundation and "clean UI" needed for interactive dashboards, separating the presentation layer from the data layer.

### 2. Structural Breakdown

- **Data Layout**: Splits the workbook into a user-facing "Dashboard" sheet and a hidden "Calc" sheet (which acts as the aggregation/PivotTable layer).
- **Visual Design**: Turns off gridlines (`showGridLines = False`) for a clean software-like appearance. Uses a heavy filled header spanning columns A-N.
- **Charts/Tables**: Includes a Stacked Column chart (`overlap=100`) for categorical breakdowns, and two Line charts for time-series trends. Removes legends on the line charts to maximize data-ink ratio.
- **Theme Hooks**: Uses `theme["header_bg"]` and `theme["header_fg"]` for the top branding bar, and `theme["accent"]` for the sidebar filter header.
