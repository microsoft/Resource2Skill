...
> **Skill Name**: Themed Chart Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Builds a multi-sheet reporting workbook. It constructs a hidden data sheet with aggregated metrics, then creates a clean, gridline-free "Dashboard" presentation sheet with a styled header banner and multiple placed charts simulating an interactive dashboard layout.
* **Applicability**: When creating executive dashboards or visual summaries from raw data where presentation and layout structure matter more than displaying raw numerical tables.

### 2. Structural Breakdown

- **Data Layout**: Raw or aggregated metric data is populated on a backend sheet (`ReportData`) which is subsequently hidden using `ws.sheet_state = 'hidden'` to keep focus on the visual canvas.
- **Formula Logic**: None required for the shell; charts bind directly to the hidden backend data via `openpyxl.chart.Reference`.
- **Visual Design**: The dashboard sheet removes gridlines (`showGridLines = False`), applies a subtle solid background fill across the viewport, and merges top rows to create a dominant header banner. A mock visual "filter pane" is styled using cell borders to mimic Slicers.
- **Charts/Tables**: Integrates multiple openpyxl chart primitives (`LineChart`, `BarChart(type="col")`, `BarChart(type="bar")`) positioned strategically using cell anchors (e.g., `"B6"`, `"J6"`) to create a multi-pane grid.
- **Theme Hooks**: Consumes `header_bg` and `header_fg` for the title banner and filter headers, `bg` for the canvas background, and `border` for the mock slicer outlines.
