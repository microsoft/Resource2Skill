...
> **Skill Name**: Static Performance Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Generates a polished presentation dashboard by separating raw data aggregation (handled via Python and written to a hidden `ChartData` sheet) from visual layout. Places openpyxl charts in a structured grid with hidden gridlines and a unified theme header, mimicking the structure of an Excel-native dashboard.
* **Applicability**: Best for high-level summary reporting where data can be pre-aggregated into fixed categories (e.g., performance over time, segment breakdowns) and distributed as a clean, static, non-editable snapshot.

### 2. Structural Breakdown

- **Data Layout**: An intermediary `ChartData` sheet holds the aggregated pivot-style grids (e.g., Market x Product cross-tab, Month x Metrics). This sheet is set to `hidden` so the end-user only sees the dashboard.
- **Formula Logic**: Aggregations (SUMs and groupings) are resolved at generation time in Python using `collections.defaultdict`. 
- **Visual Design**: The main `Dashboard` sheet acts as a canvas. The gridlines are disabled (`sheet_view.showGridLines = False`), and an edge-to-edge merged header banner holds the report title.
- **Charts/Tables**: Multiple charts (`BarChart` stacked, `LineChart` for timeseries) reference the hidden `ChartData` sheet. They are anchored strategically (e.g., `B4`, `J4`, `J16`) to form a split-pane layout (one large chart on the left, two stacked vertically on the right).
- **Theme Hooks**: The dashboard banner fill color relies on the `primary` token from the provided theme palette. 
