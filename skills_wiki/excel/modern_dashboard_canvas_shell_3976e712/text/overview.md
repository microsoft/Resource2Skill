...
> **Skill Name**: Modern Dashboard Canvas Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Transforms a standard worksheet into a modern, web-app-style dashboard canvas. It hides gridlines, applies a soft background fill to the viewable area, and sets up a dark vertical navigation sidebar. It includes a programmatic helper to generate white "cards" (cell ranges with solid fill and perimeter borders) that act as clean visual containers for charts and metrics.
* **Applicability**: Use when generating top-level executive dashboards or summary reports where a polished, professional UI appearance is required, rather than a raw data grid.

### 2. Structural Breakdown

- **Data Layout**: Employs a spatial layout rather than a tabular one. Column A is reserved for the sidebar. The remaining columns are adjusted to standard widths to act as a flexible grid system for placing cards.
- **Formula Logic**: N/A (Pure presentation and layout structure).
- **Visual Design**: Uses a specific UI color palette (Light Gray `#F3F4F6` background, Dark Slate `#1F2937` sidebar, White `#FFFFFF` cards, and Subtle Gray `#E5E7EB` borders) to mimic modern web design principles.
- **Charts/Tables**: This shell prepares the exact layout zones (the "cards") where charts and KPI components should be anchored later in the generation process.
- **Theme Hooks**: Utilizes `bg_color` for the canvas, `primary_color` (or a dark variant) for the sidebar, and standard white/light-gray for card containers.
