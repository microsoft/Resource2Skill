...
> **Skill Name**: Dynamic Scenario Modeling with CHOOSE

* **Tier**: sheet_shell
* **Core Mechanism**: Stacks multiple scenario assumption blocks vertically and uses a master `CHOOSE` index to pull the active scenario into a single "Live Case" block. The main financial model then references only the Live Case, cleanly separating logic from scenario data and allowing rapid toggling.
* **Applicability**: Ideal for FP&A, budget forecasting, or any financial modeling where users need to rapidly switch between Base, Best, and Worst cases without duplicating the actual model calculations.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Top**: Consolidated Income Statement (Years 1-5).
  - **Middle**: Scenario toggle dropdown (linked to an index cell) and the "Live Case" assumptions block.
  - **Bottom**: Separated "Upper Case" and "Lower Case" assumption blocks containing the hardcoded variations.
- **Formula Logic**: 
  - `CHOOSE` acts as the router in the Live Case: `=CHOOSE($J$2, C21, C29)` pulls data from Row 21 (Scenario 1) or Row 29 (Scenario 2) depending on the dropdown value in `J2`.
  - Growth rates dynamically scale order volumes in out-years: `=C21*(1+D22)`.
- **Visual Design**: Uses theme-driven header fills, bolded subtotals (Gross Profit, Operating Profit) with top/double-bottom borders, and blue font color to explicitly denote hardcoded inputs versus formula-driven cells (black).
- **Charts/Tables**: Purely cell-based grid with standard financial formatting (`#,##0` for whole dollars, `0%` for margins/growth).
- **Theme Hooks**: Consumes `header_bg` for section bands, `highlight_bg` for the dropdown toggle, and `input_fg` (standard blue) for hardcoded assumption text.
