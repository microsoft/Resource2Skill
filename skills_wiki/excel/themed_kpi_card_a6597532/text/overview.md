...
*   **Tier**: component
*   **Core Mechanism**: Creates a visually distinct card containing a summary metric, a descriptive label, and an associated icon. The card's aesthetic (colors, shapes, fonts) is driven by the workbook's theme. The numeric value is represented as text directly from a specified cell, indicating where a dynamic formula link (like `=Sheet!A1`) would typically be set manually in Excel.
*   **Applicability**: Ideal for displaying key performance indicators (KPIs) in dashboard headers, summary sections, or as standalone metrics in reports. Provides at-a-glance insights with visual cues. Requires pre-calculated numeric values (e.g., from PivotTables) and a descriptive icon name.

### 2. Structural Breakdown

-   **Data Layout**: Assumes numeric KPI values are pre-calculated and reside in specific cells on a separate "Analysis" worksheet (e.g., `Analysis!B4`). A representative icon name and text label are provided as parameters.
-   **Formula Logic**: A text box within the card is conceptually linked to a source cell (e.g., `Analysis!B4`). Due to `openpyxl` limitations with direct formula linking in shapes, the formula string is inserted as literal text. For dynamic updates, users would manually copy this text into the shape's formula bar in Excel.
-   **Visual Design**:
    *   **Outer Shape (Accent Tab)**: A rounded rectangle shape (e.g., gold from the theme) serves as a visual tab on the left. It has no outline and optional shadow effects.
    *   **Inner Shape (Content Area)**: A larger, white rounded rectangle overlaps the accent tab, providing the main background for the KPI value and label. It also has no outline.
    *   **Icon**: Represented by a text placeholder (e.g., "Icon Placeholder") within the accent tab, styled with a primary theme color (e.g., purple). `openpyxl` does not support Excel's built-in icon library directly.
    *   **KPI Value**: Displays the numeric value (as literal text from `kpi_value_cell`) in a large, bold font (e.g., Aptos Narrow, 32pt) with a primary theme color, centered within the content area.
    *   **KPI Label**: Displays the descriptive text (e.g., "CALLS") in a smaller font (e.g., Aptos Narrow, 18pt) with a primary theme color, centered below the KPI value.
    *   **Shadows**: Outer shadow effect is applied to the main shapes to give a lifted appearance.
-   **Charts/Tables**: This component *consumes* data (pre-calculated KPI values) but does not generate charts or tables directly.
-   **Theme Hooks**: `accent_1` (for the outer tab), `text_fg_light` (for the inner content area), `accent_2` (for icon placeholder text), and `text_fg_dark` (for KPI value and label). `header_bg_dark` and `header_bg_light` are used for the general dashboard background in the example setup.
