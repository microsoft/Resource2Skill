...
> **Skill Name**: Clean Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Sets up a dedicated "Dashboard" presentation worksheet with hidden gridlines and a styled header banner. Generates multiple standard charts (like Stacked Bar and Line charts) tied to a separate, isolated "Data" sheet and anchors them cleanly into a dashboard grid layout.
* **Applicability**: Best used for creating visually appealing, app-like report views in Excel where messy data transformations happen on hidden/separate sheets and only polished visuals are presented to the end-user.

### 2. Structural Breakdown

- **Data Layout**: Raw data is isolated on a "Dashboard Data" worksheet, cleanly separated into contiguous data blocks for each specific chart.
- **Formula Logic**: N/A (Data is provided directly to chart references).
- **Visual Design**: Gridlines are disabled (`showGridLines = False`) to remove the spreadsheet feel. A prominent title banner spans `B2:M4` using merged cells, centered alignment, and theme-driven fill/font colors.
- **Charts/Tables**: Includes a Stacked Column chart (`type="col"`, `grouping="stacked"`) and a Line chart, arranged side-by-side using specific top-left cell anchors (`B6`, `H6`) and explicit chart dimensions.
- **Theme Hooks**: Uses the theme's `primary` color for the banner background and `header_fg` for the banner text.
