...
> **Skill Name**: Cell-Based Themed KPI Card

* **Tier**: component
* **Core Mechanism**: Translates the visual concept of shape-based KPI cards into a robust, programmable cell-grid layout. It merges a block of cells, applies deep themed background colors, and uses contrasting typography to create a standalone "card" layout with a title, a primary metric, and a distinct formatted "badge" area for secondary metrics (like percentages).
* **Applicability**: Use for executive dashboards or summary sheets where key metrics need to stand out visually above or beside tabular data. This cell-based approach is significantly more stable for automated generation than floating shapes.

### 2. Structural Breakdown

- **Data Layout**: A 2x3 block of cells originating at the `anchor`. Top row merged for the Title. Bottom two rows split into a main label/value column and a merged "badge" column for a secondary metric.
- **Formula Logic**: Directly assigns values rather than using shape formula links, ensuring data is native to the grid and recalculates reliably.
- **Visual Design**: Uses dark background fills (e.g., navy) with white text to mimic the cards in the video. The "badge" uses a slightly lighter or distinct fill color with a white border to simulate the overlapping circle effect. 
- **Charts/Tables**: None.
- **Theme Hooks**: Consumes `bg` (primary card background), `fg` (text color), and `badge_bg` (secondary accent color for the right-hand metric).
