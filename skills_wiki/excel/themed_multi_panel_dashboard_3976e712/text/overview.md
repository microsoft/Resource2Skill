...
> **Skill Name**: Themed Multi-Panel Dashboard
* **Tier**: archetype
* **Core Mechanism**: Generates a modern analytical dashboard by configuring a secondary "Inputs" sheet for raw data, and a primary "Dashboard" sheet. It uses border-boxed white cells against a shaded background to simulate UI "cards", and implements specific visual components (Line, Doughnut, and Radar charts) set to transparent backgrounds so they blend seamlessly into their respective panel cards.
* **Applicability**: Best for executive summaries or KPI scorecards where data separation and clean, padded visual containment (cards) significantly improves readability and structure.

### 2. Structural Breakdown

- **Data Layout**: Two-sheet setup: `Inputs` (holds flat tables for KPIs, time-series trends, and radar metric data) and `Dashboard` (presentation layer).
- **Formula Logic**: Dashboard cells link directly to Input aggregates (e.g., `=Inputs!B2`).
- **Visual Design**: Turns off gridlines and uses a solid gray background (`F2F2F2`) with white-filled cell ranges wrapped in thin gray borders to act as dashboard panels. A left navigation bar uses a bold theme color (`1F4E78`).
- **Charts/Tables**: 
  - **Doughnut charts**: Track KPI progress (Actual vs Target).
  - **Line chart**: Maps time-series sales trends.
  - **Radar chart**: Plots multi-axis qualitative scoring (Customer Satisfaction). 
  - *Note*: Chart objects are explicitly styled with `noFill=True` to eliminate standard white bounding boxes.
- **Theme Hooks**: Utilizes a primary dark accent color (corporate blue) for the navigation bar, card titles, and active chart series.
