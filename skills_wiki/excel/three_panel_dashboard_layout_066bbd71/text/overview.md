...
> **Skill Name**: Three-Panel Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a clean, gridless presentation layer with a merged banner header. Orchestrates three distinct charts (a primary categorical stacked bar, and two secondary time-series lines) by routing their data to a hidden backing sheet, keeping the visible dashboard pristine. 
* **Applicability**: Ideal for executive summaries that require a primary breakdown (e.g., product mix by region) alongside two supporting key performance trends (e.g., volume and profit over time). 

### 2. Structural Breakdown

- **Data Layout**: Raw chart data is sequentially written to a newly created, hidden worksheet (`{sheet_name}_Data`). The dashboard sheet contains only the visual artifacts.
- **Formula Logic**: Purely structural; uses `openpyxl.chart.Reference` to anchor chart series to the hidden data ranges dynamically based on row counts.
- **Visual Design**: Gridlines are disabled (`showGridLines = False`). A prominent unified banner spans the top rows (`A1:P2`) with centered, large text and a solid background fill. 
- **Charts/Tables**: 
  - Chart 1: Stacked `BarChart` (col type) placed on the left side, sized to span the full height of the two right-side charts.
  - Charts 2 & 3: `LineChart` objects placed vertically on the right side. Legends are removed for single-series line charts to maximize plot area.
- **Theme Hooks**: Utilizes `primary_bg` for the banner background and `primary_fg` for the banner text. Falls back to a standard dark blue/white combo.
