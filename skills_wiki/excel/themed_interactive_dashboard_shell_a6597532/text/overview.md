...
> **Skill Name**: Themed Interactive Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Sets up a two-tone dashboard canvas (dark header, pale body) using full-row background fills. Simulates "floating" KPI shape cards using styled, merged cell blocks with contrasting accent columns. Integrates a dense performance leaderboard relying on `DataBarRule` conditional formatting to embed bar charts directly inside the data grid.
* **Applicability**: Best used when generating executive summaries or top-level dashboards where raw data needs to be presented alongside high-impact visual KPIs, without requiring the user to manually configure shapes and pivot charts.

### 2. Structural Breakdown

- **Data Layout**: Places KPI summaries in rows 3-6 spaced across columns. The primary performance leaderboard begins at row 10 (column B through F), alongside a visual chart taking up the right flank (column H).
- **Formula Logic**: Aggregates provided raw data dynamically to populate the top KPI strip, simulating a PivotTable summary.
- **Visual Design**: Uses a custom dark purple (`#321E4B`) and pale purple (`#F2EFF5`) layout to break the grid into header and body sections. KPI cards use thick, colored left-borders (achieved via a filled narrow column) and white data areas.
- **Charts/Tables**: Applies yellow and purple gradient `DataBarRule` formatting to the table to visualize scale instantly. Generates a standard column chart stripped of major gridlines and legends to serve as a clean visual companion.
- **Theme Hooks**: Hardcodes the specific palette from the tutorial (Aspect theme variant) as defaults, but can easily be bound to `theme` tokens for header backgrounds and data bar gradient colors.
