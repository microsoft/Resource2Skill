...
> **Skill Name**: Executive Dashboard Shell Archetype

* **Tier**: archetype
* **Core Mechanism**: Builds a presentation-ready dashboard by separating raw data, hidden calculation/aggregation tables, and a polished front-end sheet. The presentation layer uses a full-width banner header, hides gridlines, reserves a shaded left-hand column for interactive controls (slicers/filters), and strategically aligns multiple charts (stacked column and line graphs) for visual impact.
* **Applicability**: Ideal for generating automated KPI reports, executive dashboards, or analytical tools where the final deliverable needs to feel like an application interface rather than a raw spreadsheet.

### 2. Structural Breakdown

- **Data Layout**: Segregates architecture into three layers: `Dashboard` (view), `Raw Data` (model), and `ChartData` (hidden calculations).
- **Formula Logic**: (In pure Excel, PivotTables power the aggregations. Programmatically, we simulate this by writing pre-aggregated data to the hidden `ChartData` sheet). 
- **Visual Design**: Gridlines disabled globally. A prominent header spans `A1:R3` using `theme.primary` fill with white text. A light-gray control panel is designated in columns `A:C`.
- **Charts/Tables**: Contains a primary `BarChart` (`grouping="stacked"`, `overlap=100`) for categorical breakdown, and two `LineChart`s for temporal trends. All pull from the hidden calculation sheet.
- **Theme Hooks**: Utilizes `primary` for the banner and major text highlights, `bg_light` for the sidebar area, and `text_light` for the header text.
