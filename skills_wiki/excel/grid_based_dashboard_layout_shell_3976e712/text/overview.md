...
> **Skill Name**: Grid-Based Dashboard Layout Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Transforms a blank worksheet into a structured dashboard canvas by disabling gridlines, applying a global background wash (light gray), and carving out crisp white "card" regions using cell fills and subtle borders. Includes a dedicated column for a dark-themed navigation sidebar.
* **Applicability**: Acts as the starting foundation for any interactive Excel dashboard. Using cell regions (rather than floating shapes as seen in the tutorial GUI) ensures that Python-generated charts and KPIs will snap cleanly to an indestructible, fixed-grid layout.

### 2. Structural Breakdown

- **Data Layout**: Employs an exact column-width geometry to create uniform KPI and Chart containers with consistent spacing (Spacers on columns B, G, L).
- **Formula Logic**: N/A (Structural shell).
- **Visual Design**: Hides native gridlines (`showGridLines = False`), uses high-contrast dark sidebar against a muted neutral wash, and uses thin gray borders to elevate the white KPI cards. 
- **Charts/Tables**: Defines bounding box regions (`make_card`) where charts and data tables should be anchored.
- **Theme Hooks**: Consumes `sidebar` (dark brand color), `wash` (neutral background), `card` (content area), `text`, and `border` colors.
