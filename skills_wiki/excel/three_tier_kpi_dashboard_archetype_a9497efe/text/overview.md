...
> **Skill Name**: Three-Tier KPI Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Implements a 3-sheet architecture (Data, Staging, Dashboard). Raw data resides on sheet 1, calculated KPI metrics on sheet 2, and the presentation layer on sheet 3 uses `INDEX/MATCH` linked to a Data Validation dropdown to populate large-font, conditionally formatted KPI cards dynamically.
* **Applicability**: Ideal for monthly executive reporting where raw data extracts are pasted in regularly. Hides computational complexity in a staging tab and provides a clean, interactive summary layout that highlights performance versus targets using color coding.

### 2. Structural Breakdown

- **Data Layout**: Separated into three distinct worksheets: `1) Data` (raw inputs), `2) Staging` (metrics calculated via cross-sheet formulas), and `3) Dashboard` (presentation).
- **Formula Logic**: Uses `INDEX(Staging_Range, 1, MATCH(Dropdown_Value, Month_Header, 0))` to dynamically retrieve specific KPI values based on the month selected in the dropdown.
- **Visual Design**: Uses massive font sizes (28pt bold) for the primary KPI numbers. Header bars use the theme's primary color with white text.
- **Charts/Tables**: Instead of charts, relies on visual "KPI Cards" comprised of merged cells with Data Validation dropdowns for interactivity.
- **Theme Hooks**: Consumes `primary` for section headers, `secondary` for card titles, and standard semantic colors (`good`/`bad`) for the Conditional Formatting logic.
