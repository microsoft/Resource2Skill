...
> **Skill Name**: Dynamic Scenario Income Statement

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a 5-year financial forecast using the `CHOOSE` function linked to a Data Validation dropdown to hot-swap assumption scenarios. Hardcoded inputs are segregated into scenario blocks, while the Live Case and Income Statement remain pure formula engines.
* **Applicability**: Ideal for financial models, budgeting tools, or any forecasting sheet requiring multiple scenario evaluations (e.g., Base/Best/Worst case) without duplicating calculation logic or cluttering the core statement.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Columns B-H**: Line item labels, Units, and Year 1 through Year 5 forecasting columns.
  - **Rows 3-14**: Core Income Statement calculation block.
  - **Rows 18-27**: Live Case assumptions (driven by `CHOOSE` formulas).
  - **Rows 28+**: Hardcoded scenario assumption blocks (Scenario 1 & 2).
- **Formula Logic**:
  - *Scenario Selector*: Data Validation list linking to `$K$2`.
  - *Dynamic Pull*: `=CHOOSE($K$2, D32, D42)` grabs the correct assumption based on the toggle.
  - *Forecast Growth*: `=D32*(1+E33)` compounds previous year orders by the new growth rate.
  - *Tax Floor*: `=MAX(0, D11 * D27)` ensures taxes don't trigger negatively on operating losses.
- **Visual Design**: 
  - Inputs are strictly colored blue to distinguish from formulas (black).
  - Financial totals utilize top and bottom borders (double bottom for Net Profit).
- **Theme Hooks**: 
  - `header_bg` / `header_fg`: Styles the top chronological header.
  - `highlight`: Background for the interactive Scenario toggle cell.
  - `input_fg`: Font color for hardcoded manual inputs.
