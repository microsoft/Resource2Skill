...
> **Skill Name**: Grid KPI Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Renders a visually separated grid of KPI cards. Each card acts as an atomic container featuring a unified title, a large-font primary value with dynamic target-based conditional formatting (Red/Green), and comparative sub-metrics (vs Target, vs Prior) stacked beneath it. 
* **Applicability**: Executive summaries, monthly reporting packages, and fast-glance operational dashboards where specific metrics require immediate visibility and RAG (Red/Amber/Green) coloring against predefined thresholds.

### 2. Structural Breakdown

- **Data Layout**: Configured via a list of dictionaries mapping sections to multiple KPI dicts. Renders a flexible grid where each KPI consumes a fixed 3-column span with a 1-column spacer padding between metrics.
- **Formula Logic**: Employs cell coordinates dynamically to link conditional formatting target boundaries to the visible "Vs. Target" sub-metric cells within the grid.
- **Visual Design**: Uses thematic fills for section separators, light fills for KPI headers, and large font sizes (size 24) for the main values. Conditional formatting overrides both cell `PatternFill` and `Font` properties.
- **Charts/Tables**: Purely cell-and-shape based; does not rely on native Excel charts, maximizing cross-compatibility and load speed. 
- **Theme Hooks**: Consumes `header_bg`, `header_fg`, `section_bg`, `kpi_bg`, `good_bg`, `good_fg`, `bad_bg`, and `bad_fg`.
