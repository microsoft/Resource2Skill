...
> **Skill Name**: Dynamic Performance Dashboard

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a multi-sheet, dynamic performance dashboard in Microsoft Excel. It starts by structuring raw data into an Excel Table, which serves as the source for multiple PivotTables. Each PivotTable is then used to generate a PivotChart visualizing specific business metrics (e.g., profit by market, units sold over time). The dashboard integrates these charts onto a dedicated sheet, with interactive elements like slicers and timelines (set up manually post-generation) for real-time filtering and data exploration.
*   **Applicability**: This skill is highly applicable for business intelligence professionals, analysts, or managers who need to visualize key performance indicators (KPIs) and track trends interactively. It's suitable for sales dashboards, financial summaries, project performance reports, or any dataset that benefits from multi-dimensional analysis and dynamic filtering. It assumes raw data is available in a clean, tabular format and is particularly powerful for recurring reports due to the automatic data refresh capabilities of Excel Tables and PivotTables.

### 2. Structural Breakdown

-   **Data Layout**:
    *   `Data` sheet: Contains the raw sales transaction data (Country, Product, Units Sold, Revenue, Cost, Profit, Date) formatted as an Excel Table. This ensures automatic expansion for new data and serves as the source for all PivotTables.
    *   Hidden sheets: Three separate sheets are created to host individual PivotTables and their associated PivotCharts, allowing for independent configuration and easier management. These sheets are hidden from the user in the final dashboard.
-   **Formula Logic**:
    *   The dashboard relies entirely on PivotTables for data aggregation and summarization, hence no direct formulas are present on the dashboard sheet itself. PivotTable calculations perform sums of `Units Sold` and `Profit`.
-   **Visual Design**:
    *   **Dashboard Sheet**: Gridlines and row/column headings are hidden for a clean, report-like appearance. A custom banner/header is included for branding ("Kevin Cookie Company Performance Dashboard").
    *   **Charts**: Charts are positioned and aligned using Excel's snapping and alignment tools (`Alt` key for snapping, `Shape Format > Align`). Chart titles are descriptive (e.g., "Profit by Market & Cookie Type", "Units sold each month", "Profit by month"). Redundant legend items and PivotChart field buttons are hidden to reduce clutter.
    *   **Theme Integration**: The dashboard's color scheme, fonts, and chart styles are managed through Excel's built-in themes (`Page Layout > Themes`), allowing for easy customization to match organizational branding.
-   **Charts/Tables**:
    *   **Profit by Market & Cookie Type**: A Stacked Column Chart showing `Sum of Profit` (Values) by `Country` (Rows) and `Product` (Columns). Countries and products are sorted by their respective grand totals (profit) from largest to smallest.
    *   **Units Sold Each Month**: A Line Chart showing `Sum of Units Sold` (Values) over `Date` (Rows, grouped by Months).
    *   **Profit by Month**: A Line Chart showing `Sum of Profit` (Values) over `Date` (Rows, grouped by Months).
    *   **Interactive Slicers (Manual Setup)**:
        *   One Timeline Slicer for the `Date` field, positioned on the left side of the dashboard.
        *   One Slicer for the `Country` field, positioned below the Timeline Slicer.
        *   One Slicer for the `Product` field, positioned below the Country Slicer.
        *   All slicers are manually connected to all three underlying PivotTables via `Report Connections` (right-click on slicer > `Report Connections`). This enables cross-filtering across all charts.
-   **Theme Hooks**: The skill consumes palette tokens for background colors, text colors, and chart series colors (implicitly managed by Excel's theme when charts are created and themes applied). Fonts for titles and labels are also theme-driven.
