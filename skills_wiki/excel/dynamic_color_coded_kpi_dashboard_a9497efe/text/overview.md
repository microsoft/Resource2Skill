...
> **Skill Name**: Dynamic Color-Coded KPI Dashboard

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a multi-sheet Excel workbook to present key performance indicators (KPIs) dynamically. It involves three interconnected sheets: a raw `Data` tab, a `Staging` tab for KPI calculations and target definition, and a user-facing `Dashboard` tab. The dashboard uses `INDEX MATCH` (or `VLOOKUP`) to retrieve real-time data based on a user-selected month, coupled with conditional formatting (green for good, red for bad) to instantly highlight performance against predefined targets and prior periods.
*   **Applicability**: This skill is highly applicable for financial controllers, business analysts, and managers who need to monitor enterprise health, track strategic goals, and provide concise, visually impactful reports. It requires monthly or periodic financial and operational data that can be structured into time series for calculation and comparison.

### 2. Structural Breakdown

-   **Data Layout**:
    *   `1) Data` Sheet: A flat table where raw financial and operational figures (e.g., Sales, COGS, AR Balance, FTE, OPEX Actual, Budgets) are stored. Categories are listed in column A, and monthly values are arranged horizontally in subsequent columns (e.g., Jan-20, Feb-20).
    *   `2) Staging` Sheet: Acts as an intermediate layer where KPIs are calculated by referencing the `Data` sheet. It mirrors the monthly structure for each KPI (e.g., DSO, DPO, CAC, Gross Margin, OPEX vs. Budget, CPFTE) and explicitly includes corresponding target values for each KPI and month.
    *   `3) Dashboard` Sheet: The presentation layer. KPIs are displayed in distinct "cards" grouped by category (e.g., "Working Capital Efficiency", "Sales KPIs", "Cost KPIs"). Each card prominently features the current month's KPI value, alongside its target and the prior month's actual value for easy comparison.
-   **Formula Logic**:
    *   **`2) Staging` Sheet**:
        *   Data Fetching: Simple cell references to pull raw data from the `1) Data` sheet (e.g., `='1) Data'!C4`).
        *   Number of Days: Uses `DAY(EOMONTH(Month_Cell,0))` to dynamically get days in a selected month.
        *   KPI Calculations: Standard arithmetic operations using linked data (e.g., `DSO = (AR_Balance / Credit_Sales) * #_of_Days`). `IFERROR` is used for robust handling of potential division-by-zero errors.
        *   Targets: KPI-specific target values are hardcoded or referenced per month/KPI.
    *   **`3) Dashboard` Sheet**:
        *   Month Selection: A data validation dropdown in cell `B4` allows users to select a month from the headers of the `1) Data` sheet.
        *   KPI Value Retrieval: `INDEX MATCH` is used to dynamically pull the calculated KPI values from the `2) Staging` sheet based on the selected month (`$B$4`) (e.g., `=INDEX('2) Staging'!$7:$7,MATCH($B$4,'2) Staging'!$3:$3,0))`).
        *   Prior Month Value Retrieval: Similar `INDEX MATCH` logic, but `EOMONTH($B$4,-1)` is used within the `MATCH` function to retrieve the value for the preceding month.
-   **Visual Design**:
    *   **Font**: Large, bold font (e.g., 36pt) for primary KPI values to ensure instant readability. Smaller fonts (e.g., 10-12pt) for labels and comparison metrics.
    *   **Color Coding (Conditional Formatting)**:
        *   KPI values on the `3) Dashboard` are conditionally formatted:
            *   Green fill/font for "good" performance (e.g., DSO <= Target, Gross Margin >= Target).
            *   Red/pink fill/font for "bad" performance (e.g., DSO > Target, Gross Margin < Target).
        *   Headers and sections use contrasting theme colors (e.g., dark blue for main title, gray for section headers) to organize the dashboard visually.
    *   **Layout**: KPIs are presented in clean, bordered "cards" arranged in a grid for clarity. Merged cells are used for titles and KPI values to enhance visual impact.
-   **Charts/Tables**: The dashboard primarily uses numerical cards, not graphical charts. The underlying `Data` and `Staging` sheets are simple, well-structured tables.
-   **Theme Hooks**: `header_bg`, `header_fg`, `section_bg`, `section_fg`, `accent_bg`, `accent_fg`, `good_fill`, `good_fg`, `bad_fill`, `bad_fg`, `text_fg` are used to ensure brand consistency and easy customization.
