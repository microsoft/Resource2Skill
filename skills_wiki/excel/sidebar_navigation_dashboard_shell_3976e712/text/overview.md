...
> **Skill Name**: Sidebar Navigation Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Creates a modern, interactive dashboard layout natively in the cell grid. It builds a dark fixed left sidebar with hyperlinked icons for multi-sheet navigation, and uses light gray sheet backgrounds with designated white, bordered "widget containers" to simulate shape-based report layouts without the brittleness of floating shapes.
* **Applicability**: Best for executive summaries, interactive reporting workbooks, or multi-tab financial models where users need a clear navigation menu and structured "drop-zones" for charts and KPIs.

### 2. Structural Breakdown

- **Data Layout**: Column A acts as the navigation sidebar. Columns B and U are narrow spacer columns to create margins. The remaining grid is split into specific row/col coordinates that act as widget containers.
- **Formula Logic**: Uses the `HYPERLINK` mechanism natively on cells (`#'SheetName'!A1`) to allow the user to click sidebar icons and jump between tabs.
- **Visual Design**: The global sheet view hides gridlines and uses a light gray solid fill. Specific widget ranges override this with a white fill and a thin gray perimeter border to create a "card" effect.
- **Charts/Tables**: This shell leaves the white container regions empty, acting as perfectly sized drop-zones for donut charts, line charts, or summary tables.
- **Theme Hooks**: Consumes `sidebar_bg` (dark), `dashboard_bg` (light gray), `widget_bg` (white), `primary_text` (dark gray), and `accent` (for headers).
