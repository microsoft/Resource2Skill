...
> **Skill Name**: Dashboard Layout Shell Archetype

* **Tier**: archetype
* **Core Mechanism**: Sets up a professional, multi-sheet reporting structure. It creates a raw data sheet formatted as an Excel Table, a hidden calculation sheet for aggregated metrics, and a presentation-ready "Dashboard" sheet with hidden gridlines, a branded header, and consistently sized and aligned charts (stacked bar and line charts).
* **Applicability**: Best used when generating automated end-of-month or executive reports where raw data needs to be separated from a clean, visual, gridline-free summary canvas.

### 2. Structural Breakdown

- **Data Layout**: Splits the workbook into three layers: `Data` (raw transactional data as a formatted Table), `CalcData` (hidden, pre-aggregated data for charts), and `Dashboard` (presentation layer).
- **Formula Logic**: Aggregations are handled in Python to populate the hidden `CalcData` sheet, replacing the manual PivotTables used in the UI, ensuring static, reliable chart rendering.
- **Visual Design**: The `Dashboard` sheet hides standard Excel gridlines (`showGridLines = False`). A solid-filled header row spans the top to create a banner effect, and chart legends/gridlines are stripped away for a modern, uncluttered look.
- **Charts/Tables**: Utilizes a Stacked Column chart (`col` type, `stacked` grouping) for categorical breakdowns (e.g., Profit by Market) and Line charts for trend analysis over time.
- **Theme Hooks**: The dashboard banner and text elements can easily consume `theme.primary_bg` and `theme.primary_fg` tokens to align with corporate branding.
