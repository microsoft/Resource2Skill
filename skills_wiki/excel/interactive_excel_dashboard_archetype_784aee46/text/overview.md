...
> **Skill Name**: Interactive Excel Dashboard Archetype

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a fully interactive data dashboard within an Excel workbook. It starts by converting raw tabular data into an Excel Table, then generates multiple pivot tables to aggregate key metrics (e.g., monthly trends, units sold by category, top performers). Corresponding pivot charts are created for visualization. Slicers are added for dynamic filtering, with connections established across all relevant charts. Finally, all charts and slicers are arranged on a dedicated dashboard sheet, enhanced with a title, optional logo, and themed styling for a professional presentation. Helper sheets containing raw data and pivot tables are hidden for a clean user experience.
*   **Applicability**: This skill is ideal for businesses and analysts needing to present complex datasets in an easily digestible and interactive format. It's suitable for sales performance reports, financial dashboards, inventory tracking, or any scenario requiring dynamic data exploration across different dimensions (time, categories, geographical regions). The input data should be structured in a tabular format with clear headers for dimensions and measures.

### 2. Structural Breakdown

-   **Data Layout**: Raw data is structured in columns: `Month` (date), `State` (text), `Product Category` (text), `Units Sold` (number), `Revenue (USD)` (currency), `Cost (USD)` (currency), `Profit (USD)` (currency). This raw data is converted into an Excel Table named "Sales".
-   **Formula Logic**: Primarily relies on Pivot Table aggregations. The dashboard itself uses no direct cell formulas, but its dynamic nature is driven by Pivot Table logic and Slicer filtering.
-   **Visual Design**:
    -   **Dashboard Sheet**: Hidden gridlines, background fill (white or transparent to show background color from theme).
    -   **Title Bar**: Rounded rectangle shape as a banner across the top, filled with a theme-defined background color (`header_bg`), containing a bold text title (font and color from theme `title_font` and `title_fg`). An optional logo is inserted.
    -   **Slicers**: Arranged vertically on the left side, with custom column count for 'Months' slicer, headers hidden, adjusted widths and heights for a consistent look. Theme-based slicer styles are applied.
    -   **Charts**: Charts are positioned within predefined sections of the dashboard. Borders are applied (color from theme `border_color`).
-   **Charts/Tables**:
    -   **Revenue Trend PivotTable**: `Month` (grouped by Years/Months) in rows, `Product Category` in columns, `Sum of Revenue (USD)` in values. On `Revenue Trend` sheet.
    -   **Revenue Trend Chart**: Line chart (e.g., `LineChart()`) displaying monthly revenue trends, split by product category.
    -   **Units Sold vs. Hoodies PivotTable**: `Years` (grouped by Years) in rows, `Product Category` in columns, `Sum of Units Sold` in values. On `T-shirts vs Hoodies` sheet.
    -   **Units Sold Chart**: Clustered column chart comparing units sold of hoodies vs. t-shirts across years.
    -   **Top 5 States by Profit PivotTable**: `State` in rows, `Sum of Profit (USD)` in values, filtered to show only the top 5 states by profit. On `Top 5 States` sheet.
    -   **Top 5 States Chart**: Clustered column chart showing profit for the top 5 states.
    -   **Slicers**: `Years`, `Months`, `State`, `Product Category`. Each slicer is connected to all relevant pivot tables/charts for cross-filtering.
-   **Theme Hooks**:
    -   `header_bg`: Used for the main title banner background.
    -   `title_fg`: Used for the main title text color.
    -   `border_color`: Used for chart and slicer borders, and potentially background shapes.
    -   `accent`: Used for specific chart series if overriding default theme colors.
    -   Font styles: Applied via theme font definitions.
