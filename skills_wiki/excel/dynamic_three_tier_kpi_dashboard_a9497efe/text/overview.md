...
> **Skill Name**: Dynamic Three-Tier KPI Dashboard

* **Tier**: archetype
* **Core Mechanism**: Constructs a multi-sheet architecture separating data presentation (`Dashboard`) from calculation/data (`Staging`). Uses Data Validation dropdowns to drive dynamic `INDEX/MATCH` lookups, feeding large-font KPI cards. Direction-aware Conditional Formatting evaluates actuals against targets to automatically shade metrics green or red.
* **Applicability**: Ideal for executive summaries, monthly operational reviews, and financial reporting where a pristine, interactive front-end is required to summarize complex underlying operational data. 

### 2. Structural Breakdown

- **Data Layout**: `Staging` sheet holds time-series metrics row-by-row with month columns. `Dashboard` uses a clean grid spacing with merged cell blocks for KPI widgets.
- **Formula Logic**: `=INDEX(Staging!$B$2:$D$5, MATCH("Metric", Staging!$A$2:$A$5, 0), MATCH($C$3, Staging!$B$1:$D$1, 0))` links the dropdown selection to the correct staging value.
- **Visual Design**: Gridlines disabled for a "software" feel. Uses large 24pt bold fonts for metric values. `CellIsRule` conditional formatting applies dynamic green/red fills.
- **Charts/Tables**: Cell-based widgets (simulating cards) constructed from merged regions with outer borders.
- **Theme Hooks**: Consumes `primary` for the main dashboard header and `accent` for the section sub-headers.
