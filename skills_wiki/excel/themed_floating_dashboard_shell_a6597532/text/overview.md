...
> **Skill Name**: Themed Floating Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a modern, interactive-style dashboard shell by utilizing a two-tone background (dark header, light body) and merging cells across the split to create the illusion of "floating" KPI cards. Pre-defines perfectly aligned layout panels for subsequent chart placement using a standardized column-width grid system.
* **Applicability**: Use for high-fidelity executive summaries or KPI views where visual impact and cohesive UI design are prioritized over raw data density. Provides an excellent canvas for injecting openpyxl charts.

### 2. Structural Breakdown

- **Data Layout**: Grid-based layout enforcing an 8-character column width (Cols C-S). Columns A and B act as left-margin spacers. Title and Subtitle snap to the left edge of the first layout panel.
- **Formula Logic**: N/A - pure structural architecture.
- **Visual Design**: Uses a solid dark header spanning rows 1-7 and a contrasting light body spanning rows 8-40. KPI blocks (Rows 6-9) bridge this gap with white fills and thick, accent-colored top borders to simulate depth/cards. Panel headers have a subtle bottom rule.
- **Charts/Tables**: Reserves empty, filled 7x12 block regions (`panels`) perfectly spaced out to accept drop-in openpyxl objects later.
- **Theme Hooks**: `primary` (header), `secondary` (body), `accent` (KPI trim, subtitle), `background` (cards), `text_light` (title), `text` (values).
