...
> **Skill Name**: Left-Nav Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Configures a foundational dashboard layout using cell fills and column widths instead of floating shapes (which are brittle). It establishes a narrow margin, a wide dark-themed left pane for slicers/KPIs, and a uniform light-themed grid area (snap-to grid) for aligning charts and tables. Gridlines are disabled to complete the application-like UI.
* **Applicability**: Use as the base canvas for any executive or summary dashboard. It provides a clean separation of controls (left) and data visualizations (right), ensuring consistent alignment across different screen sizes. 

### 2. Structural Breakdown

- **Data Layout**: 
  - Col A: Gap/margin (width 2)
  - Col B: Left Navigation & KPI Panel (width 30)
  - Col C: Inner divider gap (width 2)
  - Cols D through Z: Uniform grid for content (width 10)
- **Formula Logic**: Pure visual shell (serves as the anchor target for subsequent component insertions).
- **Visual Design**: Disables sheet gridlines. Applies solid `PatternFill` to column B to form the left panel, and a contrasting light fill to the main canvas. Titles are bolded with vertical/horizontal centering.
- **Charts/Tables**: Provides the grid anchor points where charts (spanning e.g., `D5:K15`) should be placed.
- **Theme Hooks**: `primary` (left pane background), `text_light` (left pane text), `bg_light` (main canvas background), `text_dark` (main canvas text).
