...
> **Skill Name**: Two-Sheet Scenario Planning Model

* **Tier**: archetype
* **Core Mechanism**: Separates input variables onto an "Assumptions" tab and links them dynamically to a "Model" (time-series forecast) tab. Uses cross-sheet absolute cell references for constant multipliers (like AOV or Margin) and relative compound growth formulas for month-over-month projections.
* **Applicability**: Ideal for financial forecasting, budgeting, or business scenario planning where key drivers (growth rates, pricing, margins) must be easily adjustable to instantly simulate impacts on a projected timeline.

### 2. Structural Breakdown

- **Data Layout**: 
  - `Assumptions` sheet: Two columns (Assumption Name, Value), grouped into distinct business units or product lines.
  - `Model` sheet: Rows represent line items (Orders, Revenue, Profit), columns represent time periods (Month 1 to Month N).
- **Formula Logic**: 
  - Base month references starting inputs: `=Assumptions!$B$3`
  - Growth months calculate sequentially: `=[Previous Month Cell] * (1 + Assumptions!$B$4)`
  - Value totals use constant absolute refs: `=[Current Month Orders] * Assumptions!$B$5`
- **Visual Design**: Accent fills for section headers to group product lines visually. Standardized formatting sets (Currency for dollars, Percentage for rates, Integers for volume).
- **Charts/Tables**: A multi-series `LineChart` anchored below the model grid tracking revenue generation across distinct product lines over time.
- **Theme Hooks**: Uses `header_bg` / `header_fg` for top-level column headers and summary rows, and `accent` for product-line subheaders and visual separation.
