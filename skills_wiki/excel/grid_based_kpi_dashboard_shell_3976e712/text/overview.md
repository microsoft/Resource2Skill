...
> **Skill Name**: Grid-Based KPI Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a robust dashboard layout using cell-based "cards" instead of fragile floating shapes. Formats grid backgrounds and borders to simulate floating panels, dynamically hiding a data backing-sheet, and precisely anchors standard openpyxl charts (Doughnut, Line, Radar) within the grid.
* **Applicability**: High-level executive reporting and KPI summaries. Ideal for replacing static shape-based designs with a robust, script-friendly grid approach. Includes visual fallbacks since complex 3D/Map charts (shown in the video) lack native openpyxl generation support.

### 2. Structural Breakdown

- **Data Layout**: Stores all backing data in a hidden `_DashboardData` sheet. Structured horizontally for charts.
- **Formula Logic**: Calculates "Remaining" gap and "% to Target" directly during data insertion to feed Doughnut charts correctly (which require Actual and Remaining as side-by-side series).
- **Visual Design**: Uses a global light-gray grid fill (`F3F3F3`) overlaid with white blocks and thin gray borders to create an elevated card effect. Column A is painted dark blue to simulate a sidebar. Gridlines are globally disabled.
- **Charts/Tables**: 
  - **KPIs**: `DoughnutChart` (Hole size 65%), border disabled.
  - **Trends**: `LineChart` (Style 13), smooth lines, legend at bottom.
  - **Categorical**: `RadarChart` (Filled type), legend disabled.
- **Theme Hooks**: `background` (gray fill), `surface` (white cards), `primary` (sidebar and accents), `text_primary` / `text_secondary` (KPI labels).
