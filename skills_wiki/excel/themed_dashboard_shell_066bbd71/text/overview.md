...
* **Tier**: sheet_shell
* **Core Mechanism**: Sets up a clean, BI-style dashboard layout by hiding native gridlines, spanning a branded title banner across the top, dedicating a distinct left-rail panel for slicers/filters, and arranging a primary/secondary grid of charts fed by a hidden data sheet.
* **Applicability**: Best used when generating executive summaries, KPI reports, or static dashboards. It visually separates data processing from presentation, creating an interactive "app-like" feel in Excel.

### 2. Structural Breakdown

- **Data Layout**: A hidden `_Data` helper sheet stores pre-aggregated categorical (e.g., market by product) and time-series (monthly) metrics to feed the charts cleanly.
- **Formula Logic**: Not strictly required (chart references directly target the raw arrays in the hidden sheet). 
- **Visual Design**: Worksheet gridlines are hidden globally (`ws.sheet_view.showGridLines = False`). A merged header spanning `A1:M3` uses the theme's primary color. A dedicated `A5:B20` region is styled with a subtle gray fill to host interactive Slicers. 
- **Charts/Tables**: A primary `BarChart` (stacked column) is anchored in the center to show categorical breakdowns. Two smaller `LineChart`s are anchored on the right to show trends over time. Legends are hidden on the line charts to maximize data ink.
- **Theme Hooks**: The title banner uses the theme's primary/accent color to establish corporate branding.
