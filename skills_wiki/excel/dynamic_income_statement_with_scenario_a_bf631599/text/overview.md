...
*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill constructs a standard income statement layout including historical actuals and future forecasts. It integrates dynamic forecasting for key revenue and expense drivers using the Excel `CHOOSE` function, which links to a scenario selector on a separate 'Cover' sheet. It applies industry best practices for formatting (distinguishing hardcoded inputs from calculated values and totals), uses cell protection, and includes grouping for better organization.
*   **Applicability**: Useful for building flexible financial models where different assumptions (scenarios) can be easily toggled to see their impact on financial performance. Applicable to any company requiring income statement forecasting and interactive scenario planning. It provides a structured, reusable template for common financial modeling tasks.

### 2. Structural Breakdown

-   **Data Layout**:
    *   **Years Header (Row 3)**: Columns D:K contain years, with custom formatting for "Actuals" (e.g., `2023A`) and "Estimates" (e.g., `2026E`).
    *   **Income Statement Section (Rows 4-13)**: Column B lists line items (Revenue, COGS, Gross Profit, etc.), while Columns D:K contain corresponding monetary values or calculations.
    *   **Income Statement Assumptions (Rows 16-22)**: Column B lists assumption drivers (e.g., Revenue Growth Rate, COGS as % of Revenue). Columns D:K contain historical calculated percentages and dynamic forecasted percentages.
    *   **Revenue Scenarios (Rows 25-28)**: Column B lists scenario names (Best Case, Base Case, Worst Case). Columns D:K contain the specific percentage values for each scenario year.
    *   **Cover Sheet**: Contains a scenario selector cell (e.g., C6) and company details.
-   **Formula Logic**:
    *   **Years**: `D3` (hardcoded start year), `E3` (`=D3+1`, dragged right).
    *   **Historical Calculations (Assumptions)**:
        *   Revenue Growth Rate (`D17` for 2024): `=(D4/C4)-1`
        *   COGS as % of Revenue (`D18` for 2023): `=D5/D4`
        *   Similar for other percentage-based assumptions.
    *   **Dynamic Forecasted Assumptions (e.g., `G17` for Revenue Growth Rate)**: `=CHOOSE(Cover!$C$6, Forecast!G$26, Forecast!G$27, Forecast!G$28)` where `Cover!$C$6` is the scenario index (1, 2, or 3) and `Forecast!G$26` etc. refer to the specific scenario values.
    *   **Forecasted Income Statement (e.g., `G4` for Revenue)**: `=F4*(1+G17)` (Previous year revenue * (1 + current year's dynamic growth rate)). Other line items are calculated based on these revenue forecasts and the dynamic assumptions (e.g., `G5` (COGS): `=G18*G4`).
    *   **Totals**: Standard arithmetic formulas (e.g., `G6` (Gross Profit): `=G4-G5`).
-   **Visual Design**:
    *   **Headers**: Dark blue background, white bold font.
    *   **Years Formatting**: Custom formats `#"A"` for actuals, `#"E"` for estimates.
    *   **Input Values (Manual)**: Vibrant blue font (e.g., historical actuals, dynamic assumption selector output).
    *   **Calculated Values (Formulas)**: Black font.
    *   **Subtotals/Totals**: Bold font, top border. Net Income row also has a yellow fill and a double bottom border.
    *   **Protection**: Formula cells and fixed text are locked. Input cells (blue font) are unlocked. The sheet is protected.
    *   **Grouping**: Rows containing assumptions and columns for forecast years are grouped for collapsibility, indicated by `+`/`-` buttons.
-   **Charts/Tables**: The income statement itself does not contain charts or tables. The cover page contains a Table of Contents with hyperlinks.
-   **Theme Hooks**:
    *   `header_bg`: Used for main headers (Years, Income Statement Assumptions, Scenario Analysis, General Settings, Table of Contents).
    *   `header_fg`: Used for header text.
    *   `accent_bg`: Used for the Net Income row fill.
    *   `input_fg`: Used for manual input values (vibrant blue).
    *   `text_fg`: Default text color (black).
