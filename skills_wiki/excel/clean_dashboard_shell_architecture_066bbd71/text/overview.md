...
> **Skill Name**: Clean Dashboard Shell Architecture

* **Tier**: sheet_shell
* **Core Mechanism**: Separates data and presentation by creating a hidden "backend" worksheet for aggregated data, and a clean "frontend" worksheet with hidden gridlines, a themed unified header, and precisely aligned summary charts. 
* **Applicability**: Best used when generating high-level summary reports or executive dashboards where the raw data should be abstracted away, and the focus is entirely on visual KPIs and trends.

### 2. Structural Breakdown

- **Data Layout**: Raw or aggregated data is injected into a separate worksheet (`Dashboard_Data`) which is subsequently hidden.
- **Formula Logic**: Chart references are cross-sheet (`data_ws` to `dashboard_ws`).
- **Visual Design**: Gridlines and row/column headers are disabled. A unified banner is created across the top using merged cells (`A1:P3`), filled with a theme-driven background color and stark white title text.
- **Charts/Tables**: Combines a large Stacked Column chart for categorical breakdowns (Profit by Market) and smaller, stacked Line charts for temporal trends (Units and Profit over time).
- **Theme Hooks**: Consumes `header_bg` and `title_fg` for the dashboard banner. Chart styles rely on native Excel preset indices.
