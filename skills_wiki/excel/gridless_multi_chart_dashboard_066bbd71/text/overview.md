...
> **Skill Name**: Gridless Multi-Chart Dashboard

* **Tier**: archetype
* **Core Mechanism**: Creates a presentation sheet with gridlines disabled and a bold thematic header. Behind the scenes, it writes summary tables to a hidden worksheet and builds multiple interconnected, cleanly-formatted charts (Stacked Column, Line) positioned onto the dashboard sheet.
* **Applicability**: Best for synthesizing categorical and time-series data into a single executive summary view. Ideal when you want a clean "app-like" visual feel in Excel rather than a standard spreadsheet interface.

### 2. Structural Breakdown

- **Data Layout**: Abstracted away onto a separate, hidden "ChartData" worksheet containing sequential summary tables.
- **Formula Logic**: Purely data-driven for the charts; aggregation is handled programmatically before insertion. 
- **Visual Design**: The dashboard relies on `ws.sheet_view.showGridLines = False` to create a blank canvas. A large merged region (`A1:P3`) acts as a persistent header banner with vertical centering and indents.
- **Charts/Tables**: 
  - Chart 1: `BarChart` with `grouping="stacked"` and `overlap=100` to create a proper stacked column.
  - Charts 2 & 3: `LineChart` with custom `y_axis.number_format` for currency/integers and `legend = None` to reduce visual clutter.
- **Theme Hooks**: The header banner's fill and font colors should consume the primary palette colors (e.g., `theme.primary` and `theme.text_on_primary`).
