...
> **Skill Name**: Dashboard Card Canvas Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Simulates a modern, web-app-style UI layout natively in the Excel grid. Because floating shapes (like rounded rectangles) are brittle to generate via code, this pattern disables gridlines, paints a global background "canvas" color, establishes a dark navigation sidebar, and carves out specific cell regions with a white background and subtle borders to act as "Cards" for KPIs and charts. 
* **Applicability**: Essential for building executive dashboards, summary sheets, or presentation layers where you want to hide the raw spreadsheet aesthetic and present data in distinct visual quadrants.

### 2. Structural Breakdown

- **Data Layout**: Utilizes a rigid column grid system. Column A acts as a navigation sidebar. Columns B, F, J, and N act as narrow "spacer" columns to create margins between cards.
- **Formula Logic**: None in the shell itself; serves as the target layout for downstream chart components.
- **Visual Design**: Uses a distinct palette—dark navy (`1C2A39`) for the sidebar, light gray (`F3F4F6`) for the negative space, and white (`FFFFFF`) for the cards. Gridlines are explicitly turned off (`ws.sheet_view.showGridLines = False`).
- **Charts/Tables**: Defines the bounding boxes where charts/tables will be injected (e.g., placing a trend line inside the 7-column wide card at `C10:I21`).
- **Theme Hooks**: Uses `sidebar`, `canvas`, `card_bg`, `card_border`, and `text_main` to drive the structural coloring.
