...
> **Skill Name**: Targeted KPI Card

* **Tier**: component
* **Core Mechanism**: Renders a 4x3 cell grid acting as a standalone KPI card. Merges the top row for a clean title and the middle row for an oversized metric. Applies dynamic `CellIsRule` conditional formatting to the value, turning it green or red by comparing it directly to the target cell embedded in the bottom row.
* **Applicability**: Best for executive dashboards and top-line summary sheets. Drop these components into a spaced grid layout (e.g., anchors at A1, F1, K1) to build a highly readable metrics layer.

### 2. Structural Breakdown

- **Data Layout**: 4 columns wide, 3 rows tall. Row 1: Title (Merged). Row 2: Value (Merged). Row 3: Target Label, Target Value, Prior Label, Prior Value.
- **Formula Logic**: Uses an absolute cell reference (e.g., `$B$3`) in the `CellIsRule` conditional format to evaluate the performance state across the merged value cell.
- **Visual Design**: 24pt bold font for the primary value, light gray header fills, and 9pt gray text for benchmark labels. Employs standard Excel stop-light fills (Green/Red) to denote status.
- **Charts/Tables**: N/A 
- **Theme Hooks**: Uses neutral grays (`#EFEFEF`, `#333333`) and standard status colors to ensure compatibility across themes.
