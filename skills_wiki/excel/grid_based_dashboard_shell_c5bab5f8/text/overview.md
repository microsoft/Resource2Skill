...
* **Tier**: sheet_shell
* **Core Mechanism**: Transforms a blank worksheet into a modern, web-app style dashboard interface. It hides gridlines and uses `PatternFill` and column sizing to create a fixed dark sidebar for KPIs and a contrasting lighter main area. "Cards" (containers for charts or tables) are generated by painting specific cell ranges white and wrapping them in subtle borders. 
* **Applicability**: Ideal for executive summaries, reporting overviews, or any scenario where you need to present multiple metrics and charts on a single screen. Using cell fills instead of floating shapes (as done in the video) makes programmatic generation much more robust and easier to align.

### 2. Structural Breakdown

- **Data Layout**: 
  - Column A: Left inner margin (Width 3)
  - Column B: Sidebar content area (Width 25)
  - Column C: Sidebar right margin (Width 3)
  - Column D: Main area left margin (Width 3)
  - Columns E-P: Main content grid (Width 10 each)
- **Formula Logic**: Not heavily formula-driven in the shell phase; designed as a layout destination for external pivot data or direct value injection.
- **Visual Design**: Disables worksheet gridlines. Applies contrasting themes (e.g., dark navy sidebar, light gray background, pure white cards with thin outline borders).
- **Charts/Tables**: Creates designated drop-zones ("Cards") where standard `openpyxl` charts or formatted ranges can be placed safely without misaligning.
- **Theme Hooks**: Utilizes a custom inner palette mapped from standard theme names, consuming `sidebar_bg`, `sidebar_fg`, `main_bg`, `card_bg`, `card_border`, and `text_main`.
