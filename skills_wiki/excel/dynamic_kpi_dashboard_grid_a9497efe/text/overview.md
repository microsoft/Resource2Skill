...
> **Skill Name**: Dynamic KPI Dashboard Grid

* **Tier**: sheet_shell
* **Core Mechanism**: Generates a presentation-ready dashboard grid using modular "KPI Cards". Each card merges cells for layout, displays a large primary metric, and applies `CellIsRule` conditional formatting that evaluates against an in-card target to dynamically highlight the metric green or red.
* **Applicability**: Perfect for executive summaries, financial controllership dashboards, and monthly performance reports where tracking multiple disparate metrics (ratios, currency, percentages) in a unified layout is required.

### 2. Structural Breakdown

- **Data Layout**: Arranged in 2-column wide, 4-row high "Cards". Top row is the title, second is the large value, third and fourth are secondary comparison metrics.
- **Formula Logic**: Conditional formatting uses absolute referencing to the Target cell within the same card (e.g., `$C$8`) ensuring the highlighting remains accurate as data updates.
- **Visual Design**: Leverages extreme typography (size 24 fonts) for the main KPI, muted colors for sub-labels, and bold fills for section grouping.
- **Charts/Tables**: Purely grid-based visual design relying on structural layout and conditional formatting cell fills, rather than native charts.
- **Theme Hooks**: Uses `primary` and `secondary` for section grouping headers, `card_header_bg` for KPI titles, and semantic colors (`good_bg`, `bad_bg`) for the CF rules.
