...
> **Skill Name**: Sidebar Navigation Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Constructs a multi-sheet workbook that behaves like an interactive application. It uses a fixed left-hand column on every sheet as a navigation sidebar, utilizing `=HYPERLINK()` formulas to switch seamlessly between sheets. The main dashboard sheet leverages colored cell regions and borders to simulate floating UI cards on a clean canvas.
* **Applicability**: Best for app-like Excel reports and dashboards where users need to frequently navigate between high-level summary views, raw data inputs, and settings/contacts tabs.

### 2. Structural Breakdown

- **Data Layout**: Column A is reserved exclusively as the navigation sidebar. Columns B through T act as the main content area.
- **Formula Logic**: Uses `=HYPERLINK("#'SheetName'!A1", "Display Text")` to create clickable navigation links to other sheets.
- **Visual Design**: Gridlines are disabled. The sheet background is filled with a light gray canvas color, while the sidebar uses a solid dark primary color. White cells with subtle gray borders create "cards" for modular content.
- **Charts/Tables**: The dashboard lays out structural bounds for 3 top-level KPIs, a wide trend chart, and a square satisfaction chart area.
- **Theme Hooks**: Consumes `primary` for the sidebar background, `accent` for the active navigation item, `bg` for the main sheet canvas, and `card` for the widget containers.
