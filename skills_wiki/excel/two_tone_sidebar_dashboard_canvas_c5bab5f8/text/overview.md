...
> **Skill Name**: Two-Tone Sidebar Dashboard Canvas

* **Tier**: sheet_shell
* **Core Mechanism**: Implements a modern, app-like dashboard layout by partitioning the worksheet grid with contrasting background fills (a dark sidebar vs. a light main canvas) instead of using fragile floating shapes. It organizes high-level KPI cards vertically in the frozen sidebar and mounts dynamic charts in the main canvas area, disabling gridlines for a clean UI.
* **Applicability**: Perfect for executive summaries, financial overviews, or operational dashboards where a unified visual interface is required. Best used when you want a robust, grid-anchored layout that won't break if the user resizes columns.

### 2. Structural Breakdown

- **Data Layout**: 
  - Columns A-C form the "Sidebar" (Col B holds the actual KPI data; A and C act as padding). 
  - Columns D-Z form the "Canvas". 
  - A dedicated hidden sheet (`_DashData`) is generated to hold chart source data, keeping the dashboard sheet strictly for the presentation layer.
- **Formula Logic**: Standard cell references are used. While the tutorial uses `GETPIVOTDATA` or direct cell linking (`=+Pivots!A4`), this shell pre-calculates and mounts static or direct-linked values to ensure cross-compatibility.
- **Visual Design**: 
  - Gridlines are disabled globally on the sheet.
  - The sidebar uses a dark theme color (e.g., deep green/blue) with white, bold, high-font-size text for KPI values.
  - The canvas uses a soft background (e.g., light green/gray) to make chart areas pop.
- **Charts/Tables**: Standard openpyxl Line and Bar charts are constructed, stripped of unnecessary legends, and mounted directly onto the canvas area.
- **Theme Hooks**: `sidebar_bg`, `sidebar_fg`, `canvas_bg`, `accent_primary`, `accent_secondary`.
