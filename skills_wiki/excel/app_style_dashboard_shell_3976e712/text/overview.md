...
> **Skill Name**: App-Style Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Hides the spreadsheet gridlines, establishes a solid-colored, frozen left navigation pane containing internal sheet hyperlinks, and sets a contrasting canvas background with isolated white "cards" to house KPI metrics. This translates floating shape-based UI design into a programmatic, robust grid layout.
* **Applicability**: Best for multi-page reporting workbooks where users need an executive, web-app-like navigation experience rather than a traditional raw data grid.

### 2. Structural Breakdown

- **Data Layout**: Column A acts as the fixed sidebar (width 22). Column B is a gutter. Columns C, E, and G act as wide layout containers for KPI cards, separated by padding columns D and F. 
- **Formula Logic**: Utilizes the `HYPERLINK` attribute on cells to navigate within the workbook (e.g., `#'Data Inputs'!A1`).
- **Visual Design**: Gridlines are hidden globally on the sheet. The sidebar uses a prominent primary theme color with centered, bold white text. The main canvas is filled with a light gray pattern, and KPI cards use a stark white fill bounded by subtle gray thin borders. 
- **Charts/Tables**: Implements placeholder text for Top-line metrics (Sales, Profit, Customers) utilizing size 20 fonts to emphasize hierarchy.
- **Theme Hooks**: Consumes a `primary` color for the sidebar background and KPI numerical values, a `canvas_bg` for the main dashboard body, and a `card_bg` for the metric containers.
