...
> **Skill Name**: Clean Dashboard Layout Archetype

* **Tier**: archetype
* **Core Mechanism**: Constructs a presentation-layer worksheet with hidden gridlines, a branded title banner, and carefully aligned floating charts. Isolates aggregate data in a hidden "backend" sheet to keep the dashboard canvas pristine.
* **Applicability**: Ideal for executive summaries, KPI reports, and final deliverables where the user needs a high-level, interactive-style view without exposing the raw data or calculation mechanics.

### 2. Structural Breakdown

- **Data Layout**: Splits the workbook into a frontend "Dashboard" sheet (gridlines off) and a backend "Summary_Data" sheet (hidden).
- **Formula Logic**: Aggregated table structures act as static stand-ins for PivotTables.
- **Visual Design**: Uses a solid-filled merged cell range (A1:P3) as a branded top banner, driven by theme colors. Indented left-aligned text for a professional look.
- **Charts/Tables**: One Stacked Column chart and two Line charts. Legends are removed from single-series line charts to maximize data-ink ratio (as demonstrated in the video).
- **Theme Hooks**: Primary color `primary` applied to the banner background; `text` color applied to the banner title.
