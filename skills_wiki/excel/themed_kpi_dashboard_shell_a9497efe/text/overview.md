...
* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a grid-based executive dashboard featuring grouped KPI "cards". Each card uses a 3-row layout (Title, Large Value, Context/Targets) with horizontal merging to create wide, readable blocks. Dynamic context is provided via conditional formatting (Green/Red) applied directly to the large value cell based on the target comparison. 
* **Applicability**: Best used for monthly management reporting, executive summaries, or financial controller dashboards where a dense set of metrics needs to be digested quickly at a glance.

### 2. Structural Breakdown

- **Data Layout**: Hierarchical grid layout. Group headers span across the dashboard (e.g., "Working Capital Efficiency"), followed by a row of 3x2-cell KPI cards. 
- **Formula Logic**: (In the shell approach, data is injected; in a full workbook, these cells would contain `INDEX/MATCH` pointing to a staging sheet). Conditional formatting rules handle the binary logic (`>= target` -> Green, `< target` -> Red).
- **Visual Design**: 
  - **KPI Titles**: Light grey background, bold font, top-aligned.
  - **KPI Values**: Large font (size 24+), centered, dynamic background (conditional formatting).
  - **Context Row**: Smaller font (size 10), split into two columns (Vs. Target, Vs. Prior Month).
  - **Borders**: Thin grey borders wrapping the entire KPI card to encapsulate the metric.
- **Charts/Tables**: Replaces traditional charts with high-impact "Big Number" typography and strict color coding.
- **Theme Hooks**: Consumes `primary` for group headers, `bg_light` for card title/context backgrounds, and standard status colors (`status_success`, `status_danger`) for conditional formatting rules.
