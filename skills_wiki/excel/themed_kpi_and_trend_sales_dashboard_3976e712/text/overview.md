...
> **Skill Name**: Themed KPI and Trend Sales Dashboard

*   **Tier**: `sheet_shell`
*   **Core Mechanism**: This skill constructs a visually structured Excel dashboard on a single sheet, integrating key performance indicators (KPIs) and multiple chart types. It establishes a consistent layout using merged cells (simulating shapes) for distinct sections and titles, dynamically displays KPI metrics (numeric values and donut charts), and embeds a multi-series line chart for trends and a radar chart for satisfaction metrics. The design emphasizes clear data hierarchy and theme-driven color consistency.
*   **Applicability**: Best suited for management reports, executive summaries, or operational dashboards where a comprehensive, single-view snapshot of performance is required. It's ideal for presenting pre-aggregated data in an organized, visually appealing, and dynamic manner, assuming raw data is prepared on separate input sheets.

### 2. Structural Breakdown

-   **Data Layout**: The dashboard sheet itself does not contain raw data. Instead, it pulls or visualizes data conceptually sourced from an 'Inputs' sheet (simulated internally in the code).
    -   **KPI Data**: Actual vs. Target for Sales, Profit, and Customers, used to calculate completion percentages.
    -   **Sales Trend Data**: Monthly sales figures for two comparative years.
    -   **Customer Satisfaction Data**: Categorical scores for various satisfaction metrics.
    -   *(Note: The video also shows a "Sales by Country" map chart, which `openpyxl` does not directly support for generation. This is represented by a text placeholder.)*
-   **Formula Logic**:
    -   KPI completion percentages are calculated as `Actual / Target`.
    -   Text boxes (simulated as formatted cells) for KPI actual amounts are dynamically linked to their source data.
-   **Visual Design**:
    -   **Overall Background**: White for the main content area, with a distinct dark blue sidebar (Column A).
    -   **Section Containers**: Merged cells are used to simulate shapes like rounded rectangles for titles, KPI boxes, and chart areas. These have a white fill and a thin grey border. (Shadow and rounded corners are not directly reproducible in `openpyxl` on cells.)
    -   **Titles**: Main dashboard title is large, bold, and in white text on a dark blue background. Section titles are dark blue and bold.
    -   **Navigation Sidebar**: Column A is filled with a dark blue theme color. Conceptually, it holds hyperlinked icons (not directly reproducible in `openpyxl`).
-   **Charts/Tables**:
    -   **Donut Charts (KPIs)**: Three `openpyxl.chart.PieChart` instances, configured as donut charts with a 65% hole size. One slice represents the "% Complete" (dark blue), and the other represents the "Remainder" (light blue). Integrated with formatted cells showing the dynamic percentage value. Chart area has no fill or border.
    -   **Line Chart (Sales Trend)**: An `openpyxl.chart.LineChart` comparing two years of monthly sales. Features custom line colors (red for current year, dark blue for previous year) and circular markers with white fill and matching borders. Y-axis range is adjusted for better visibility of trends. Chart area has no fill or border.
    -   **Radar Chart (Customer Satisfaction)**: An `openpyxl.chart.radar.RadarChart` displaying multiple customer satisfaction metrics. Uses a dark blue line with circular markers (white fill, dark blue border). Axes are hidden. Chart area has no fill or border.
    -   **Map Chart**: Not directly supported by `openpyxl` for generation. A descriptive text placeholder is included where the map chart would typically reside.
-   **Theme Hooks**:
    -   `title_bg`: Used for the main dashboard title background.
    -   `title_fg`: Used for the main dashboard title and subtitle text.
    -   `sidebar_bg`: Used for the navigation sidebar background.
    -   `section_bg`: Used for the background fill of KPI boxes and chart containers.
    -   `section_border`: Used for borders around section containers.
    -   `accent_blue`: Used for primary blue elements (e.g., line chart series 1, radar chart line, donut chart complete segments, KPI values).
    -   `accent_red`: Used for a contrasting red element (e.g., line chart series 2).
    -   `light_blue`: Used for secondary blue elements (e.g., donut chart remainder segments).
    -   `text_dark`: Used for section titles.
