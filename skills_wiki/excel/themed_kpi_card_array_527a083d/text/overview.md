...
> **Skill Name**: Themed KPI Card Array

* **Tier**: component
* **Core Mechanism**: Generates a visually distinct row of "KPI Cards" by formatting groups of merged cells to mimic floating dashboard shapes. It utilizes strong thematic background colors, large typography for primary metrics, and contrasting "badge" columns for secondary percentages (like market share), bypassing the limitations of dynamic shape data-linking in Python by recreating the exact visual pattern structurally.
* **Applicability**: Best used at the very top of executive dashboards, sales reports, or summary sheets to highlight 3-5 macro-level metrics immediately above the supporting data tables.

### 2. Structural Breakdown

- **Data Layout**: Each KPI card occupies a 3x3 grid of cells. Cards are spaced horizontally with 1 blank column between them.
- **Formula Logic**: Static values or formulas can be passed into the component. The component handles formatting them distinctly (e.g., `$#,##0` for primary revenue, `0%` for badges).
- **Visual Design**: The cards use a dark primary theme color for the main body and a lighter accent color for the secondary "badge" area. Font is white, bold, and scaled up (size 18) for readability.
- **Charts/Tables**: No standard charts; uses spatial cell arrangement as a geometric visualization.
- **Theme Hooks**: Consumes a standard dashboard palette with `bg` (primary card background), `accent` (badge background), and `text` (font color) tokens.
