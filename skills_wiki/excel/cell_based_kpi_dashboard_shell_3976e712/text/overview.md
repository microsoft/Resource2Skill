...
> **Skill Name**: Cell-Based KPI Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Disables sheet gridlines and applies a global background color to create a unified "canvas". Uses targeted cell merging, background fills, and thin borders to create floating "Card UI" panels that emulate a shape-based web layout. This approach avoids brittle floating shapes, allowing text and values to sit natively and safely in cells.
* **Applicability**: Use for generating top-level executive dashboards where a modern, clean, UI-driven layout is required. It establishes predefined "drop zones" for KPIs and charts.

### 2. Structural Breakdown

- **Data Layout**: Employs a strict column-width grid system (wide columns for content, narrow columns for padding).
- **Formula Logic**: Native cell anchoring for KPI values (values drop directly into the merged card regions). 
- **Visual Design**: Light gray canvas (`#F3F4F6`), dark blue navigation sidebar (`#1E3A8A`), and white KPI cards with subtle gray borders. 
- **Charts/Tables**: Defines standard layout panels: top row for high-level KPI cards, and a wide bottom section for trend and breakdown charts.
- **Theme Hooks**: Utilizes `canvas_bg`, `card_bg`, `sidebar_bg`, `border_color`, and primary/secondary text colors.
