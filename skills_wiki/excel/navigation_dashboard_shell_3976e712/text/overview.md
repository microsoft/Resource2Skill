...
> **Skill Name**: Navigation Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Recreates a modern web-app style dashboard layout in Excel without using fragile floating shapes. Sets up a grid by disabling gridlines, filling a narrow left column as a dark navigation sidebar with hyperlinks, and defines formatted cell ranges (white background, thin border) on a light-gray canvas to act as "cards" for injecting charts and KPIs.
* **Applicability**: Ideal for executive dashboards and high-level reporting summaries where presentation quality is paramount. It provides the structured container into which other visualization components are dropped.

### 2. Structural Breakdown

- **Data Layout**: Uses specific column widths to create a rigid grid. Column A is the sidebar. Columns B, F, J are narrow spacers. The remaining columns act as the card bodies.
- **Formula Logic**: Uses standard hyperlink references (`#'SheetName'!A1`) on sidebar text elements to create interactive navigation.
- **Visual Design**: 
  - Canvas: Light gray (`#F3F4F6`)
  - Sidebar: Dark slate (`#1F2937`) with white bold text
  - Cards: White (`#FFFFFF`) with a subtle gray border (`#E5E7EB`) to simulate elevation/separation.
- **Charts/Tables**: N/A (acts as a host/shell for charts).
- **Theme Hooks**: Can be adapted to consume theme tokens for `sidebar_bg`, `canvas_bg`, and `card_border`.
