...
> **Skill Name**: Data Linked Shape KPI

*   **Tier**: component
*   **Core Mechanism**: Create a visually distinct Key Performance Indicator (KPI) by inserting and formatting multiple geometric shapes (e.g., rounded rectangle and oval), linking their text content to specific data cells using formula fields, applying theme-based colors for fill and font, and finally positioning the shapes relative to a given anchor cell within an Excel worksheet.
*   **Applicability**: Ideal for executive dashboards, summary reports, or any visual display requiring prominent, data-driven metrics. It enhances readability and visual appeal, allowing for quick insights into key performance indicators without needing traditional charts. This component can be reused for different regions or metrics by simply changing the linked data cells and the KPI's title.

### 2. Structural Breakdown

-   **Data Layout**: Assumes a source table with region names, revenue figures, and market share percentages. The KPI directly references these cells for its dynamic values.
-   **Formula Logic**: Shape text is dynamically linked to source cells (e.g., `='Sheet1'!$B$2` for revenue) via `Field` objects within the shape's `TextBody` structure. The main title (e.g., region name) is static text.
-   **Visual Design**:
    *   **Main Shape**: A rounded rectangle serves as the KPI card background. Its fill color is derived from the theme's `header_bg` or accent dark, and text (title, revenue label, revenue value) uses the theme's `text_on_dark` color.
    *   **Value Text**: Displays the revenue value, linked to a cell, bolded, and left-aligned.
    *   **Title Text**: Displays the region name, centered, bold, and with a larger font size.
    *   **Secondary Shape**: An oval, for a secondary metric like market share, positioned in the bottom-right corner of the main shape. Its fill color is from the theme's `accent_fill` or `primary_bg`, and text (market share value) uses the theme's `text_on_light` or `header_fg` color.
    *   **Secondary Value Text**: Displays the market share percentage, linked to a cell, bolded, and centered within the oval.
    *   **Positioning**: Shapes are anchored to a specific cell, with relative offsets used to achieve the desired layout.
-   **Charts/Tables**: No charts or tables are generated by this component itself; it's a visual element *consuming* data from existing tables.
-   **Theme Hooks**: `header_bg`, `text_on_dark`, `accent_fill`, `text_on_light`.
