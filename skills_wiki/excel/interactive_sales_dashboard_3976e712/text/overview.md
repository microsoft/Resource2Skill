...
*   **Tier**: sheet_shell
*   **Core Mechanism**: Builds a visually structured and interactive single-sheet dashboard in Excel. It integrates various chart types (donut, line, radar, map) with dynamic textual KPIs and hyperlinked navigation elements. The reusability comes from applying a consistent theme and data-driven visual components within a pre-defined layout, all generated programmatically.
*   **Applicability**: Useful for executive summaries, performance tracking, or any report requiring a comprehensive overview of key metrics and trends on a single interactive view. Suitable for datasets with time-series data, categorical breakdowns (e.g., by country), and target-vs-actual comparisons, enabling quick navigation and visual analysis.

### 2. Structural Breakdown

-   **Data Layout**: The dashboard resides on a dedicated sheet. Supporting data for KPIs, sales trends, customer satisfaction, and geographical sales is organized on a separate "Inputs" sheet. Contact information is on a "Contacts" sheet. Column A of the dashboard (and other sheets) is reserved for a static, hyperlinked navigation panel.
-   **Formula Logic**: Dynamic labels for KPI values are created using text boxes linked directly to cells on the "Inputs" sheet (e.g., `=Inputs!D5`). Donut charts are driven by calculated percentages (e.g., `={actual/target, 1-actual/target}`).
-   **Visual Design**: The dashboard features a clean, professional look with rounded rectangle shapes as containers for titles and visuals, using white fills, no outlines, and subtle shadows. A dark blue vertical strip serves as a consistent navigation panel across all sheets, utilizing white icons and text for contrast. Titles for sections are in dark blue, bold font.
-   **Charts/Tables**:
    *   **Donut Charts (3)**: Display KPI completion percentages. Configured with a darker accent color for the completed portion and a lighter shade for the remainder. A dynamic text box in the center shows the actual KPI value.
    *   **Line Chart (1)**: Illustrates year-over-year sales trends. Uses two distinct line colors (red and dark blue) with matching circular markers, and a custom Y-axis range for better data visibility.
    *   **Radar Chart (1)**: Visualizes customer satisfaction scores across different metrics, using a dark blue line and matching circular markers.
    *   **Map Chart (1)**: Shows sales breakdown by country in South America, employing a blue color gradient to represent sales values.
-   **Theme Hooks**: `header_bg` (dark blue for nav panel), `header_fg` (white for nav text), `section_bg` (white for visual containers), `accent1` (dark blue for primary data visuals), `accent2` (red for comparative data), `light_accent1` (light blue for secondary data visuals), `shadow_color` (for shape effects), `text_color` (dark blue for titles).
