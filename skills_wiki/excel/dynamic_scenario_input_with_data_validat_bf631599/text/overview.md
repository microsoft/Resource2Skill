...
> **Skill Name**: Dynamic Scenario Input with Data Validation & Sheet Protection

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill establishes a dynamic input mechanism where a single cell (the scenario selector) on a control sheet drives calculations on other worksheets. It uses Excel's `CHOOSE` function to select different sets of assumptions. To ensure model robustness, it then employs Data Validation to restrict input values for the selector cell and combines unlocked input cells with Sheet Protection to prevent unauthorized changes to formulas or static data.
*   **Applicability**: This pattern is crucial for financial models, budgeting tools, and interactive dashboards where users need to explore different outcomes (e.g., best case, base case, worst case) without altering the underlying logic. It enhances model integrity, user-friendliness, and error prevention.

### 2. Structural Breakdown

-   **Data Layout**:
    *   **Control Sheet (e.g., "Cover")**:
        *   Contains a scenario selection cell (e.g., `C6`) for user input (e.g., 1, 2, or 3).
        *   Optionally includes instructions for scenario numbers as a cell comment.
    *   **Forecast Sheet (e.g., "Forecast")**:
        *   Contains the core financial calculations (e.g., income statement line items across years).
        *   Contains rows for different scenario assumptions (e.g., "Best Case," "Base Case," "Worst Case" revenue growth rates).
        *   The actual assumption cells used in calculations (e.g., `G17` for revenue growth rate) link dynamically to the scenario assumptions using the `CHOOSE` function.
-   **Formula Logic**:
    *   **Scenario Selection (e.g., on "Forecast" sheet, cell G17)**: `=CHOOSE(Cover!C6, Forecast!G26, Forecast!G27, Forecast!G28)` where `Cover!C6` is the scenario index, `Forecast!G26` is the Best Case assumption, `Forecast!G27` is the Base Case assumption, and `Forecast!G28` is the Worst Case assumption.
    *   Financial calculations (e.g., revenue forecast) refer to these dynamic assumption cells.
-   **Visual Design**:
    *   Manual input cells (e.g., `Cover!C6`, and the scenario assumption ranges on "Forecast") are typically formatted with a distinct font color (e.g., blue) to visually distinguish them from formulas.
    *   Calculated cells (formulas) typically use a standard font color (e.g., black).
-   **Charts/Tables**: Not directly addressed by this specific skill.
-   **Theme Hooks**: `input_fg` for manual input cells, `formula_fg` for calculated cells, `header_bg` and `header_fg` for sheet titles and headers.
