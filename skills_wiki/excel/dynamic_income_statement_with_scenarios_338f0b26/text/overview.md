...
> **Skill Name**: Dynamic Income Statement with Scenarios

* **Tier**: sheet_shell
* **Core Mechanism**: Builds a 5-year forecasted income statement entirely driven by an assumptions block. It employs a data validation dropdown to select scenarios (Base, Upper, Lower) and uses the `CHOOSE` formula to dynamically pull the selected scenario's hardcoded values into a "Live Case" proxy block. All P&L calculations are anchored solely to this Live Case, avoiding duplication of financial logic.
* **Applicability**: Essential for standard financial modeling, budgeting, and forecasting where multiple scenarios must be evaluated dynamically on a single view. Best used when evaluating unit economics and fixed/variable expense scaling over a multi-year horizon.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Rows 1-4**: Title, Scenario Selector, and timeline headers.
  - **Rows 5-24**: The calculation engine (Income Statement) referencing the Live Case.
  - **Rows 26-37**: The "Live Case" block, strictly populated with `=CHOOSE($J$2, Scen1, Scen2, Scen3)` formulas.
  - **Rows 39+**: Stacked blocks of hardcoded scenario assumptions.
- **Formula Logic**: 
  - **Dynamic Toggle**: `=CHOOSE($J$2, D40, D53, D66)`
  - **Volume Forecasting**: `PreviousYear_Orders * (1 + GrowthRate)`
  - **P&L Margin/Tax Safety**: `=IF(OpProfit<0, NA(), OpProfit*TaxRate)` and `=IFERROR(OpProfit-Tax, OpProfit)` to safely handle tax credits on losses.
- **Visual Design**: 
  - Blue font for hardcoded scenario inputs (standard financial modeling convention).
  - Bold black font with top/double-bottom borders for subtotals and totals.
- **Charts/Tables**: Clean structured ranges, perfectly primed for referencing by secondary charts.
- **Theme Hooks**: Consumes `primary` (header bars, title), `secondary` (Live Case header), and standard `#0000FF` blue for inputs.
