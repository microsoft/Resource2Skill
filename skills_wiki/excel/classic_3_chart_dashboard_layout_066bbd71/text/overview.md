...
> **Skill Name**: Classic 3-Chart Dashboard Layout

* **Tier**: archetype
* **Core Mechanism**: Constructs a standard reporting dashboard canvas. It hides gridlines, builds a unified banner header, sets up a primary stacked column chart on the left, and stacks two secondary trend line charts on the right. A formatted side-panel simulates space for slicers/filters.
* **Applicability**: Excellent for executive summaries or static KPI reporting sheets where pre-aggregated data needs a clean, highly structured visual hierarchy without the clutter of a standard spreadsheet grid.

### 2. Structural Breakdown

- **Data Layout**: Generates a hidden `Data` sheet to store the pre-aggregated summary tables (e.g., market categorization and time-series metrics).
- **Formula Logic**: Relies on static chart data references (`Reference`) to bridge the hidden data sheet to the visual `Dashboard` sheet.
- **Visual Design**: Turns off gridlines (`showGridLines = False`). Merges cells for a strong top header with a deep theme-driven background color and contrasting white text. Formats a left-hand column with subtle grey fills to mock up interactive slicer regions.
- **Charts/Tables**: Employs a primary stacked `BarChart` for categorical comparison, and two `LineChart`s for temporal trends. Trend chart legends are hidden (`legend = None`) to maximize the data-ink ratio.
- **Theme Hooks**: Configures the main header banner to consume the primary brand color, falling back to standard corporate blue.
