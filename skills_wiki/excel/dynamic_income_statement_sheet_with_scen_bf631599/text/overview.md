...
> **Skill Name**: Dynamic Income Statement Sheet with Scenario Analysis

*   **Tier**: `sheet_shell`
*   **Core Mechanism**: This skill generates a pre-formatted Income Statement sheet with dynamic year headers (actuals vs. forecasts), historical data (hardcoded), and forecasted figures. The forecasts are driven by assumptions linked to a scenario selector on a separate 'Cover' sheet using the `CHOOSE` function. It also applies conditional styling based on hardcoded vs. formula-driven cells, and utilizes row/column grouping for enhanced readability and navigability.
*   **Applicability**: This skill is ideal for financial professionals, business analysts, or anyone building robust financial models requiring scenario analysis. It's suitable for presenting historical performance alongside future projections, allowing users to quickly toggle between different business outlooks (e.g., Best Case, Base Case, Worst Case) directly from a control panel.

### 2. Structural Breakdown

-   **Data Layout**:
    *   **Header Row (Years)**: Row 3, starting from column D (e.g., 2023A, 2024A, ..., 2030E).
    *   **Income Statement Line Items**: Column B (e.g., Revenue, COGS, Gross Profit, ... Net Income).
    *   **Income Statement Values**: Columns D through K, starting from Row 4.
    *   **Assumptions Section Header**: Cell B16 ("Income Statement Assumptions").
    *   **Assumptions Line Items**: Column B (e.g., Revenue Growth Rate, COGS as a % of Revenue, ... Tax Rate) starting from Row 17.
    *   **Assumptions Values**: Columns D through K, starting from Row 17 (for historical calculations) and G through K (for forecast inputs).
    *   **Revenue Scenarios Header**: Cell B25 ("Revenue Scenarios").
    *   **Scenario Labels**: Column B (Best Case, Base Case, Worst Case) starting from Row 26.
    *   **Scenario Values**: Columns D through K (or G through K for forecasts) starting from Row 26.
-   **Formula Logic**:
    *   **Dynamic Years**: `D3` is `2023`, `E3` is `=D3+1`, dragged right.
    *   **Historical Growth Rates (Assumptions)**: `D17` (Rev Growth Rate) is `=(D4/C4)-1`. Dragged right for historical actuals.
    *   **Historical % of Revenue (Assumptions)**: `D18` (COGS % of Rev) is `=D5/D4`. Dragged right for historical actuals.
    *   **Forecasted Revenue**: `G4` is `=F4*(1+G17)` (Previous year * (1 + chosen Revenue Growth Rate)). Dragged right.
    *   **Forecasted Expenses**: `G5` (COGS) is `=G18*G4` (Chosen COGS % * Revenue). Similar for SG&A, R&D.
    *   **Subtotals**: `G6` (Gross Profit) is `=G4-G5`. Similar for Operating Income, Pre-Tax Income.
    *   **Net Income**: `G13` is `=G11-G12` (Pre-Tax Income - Taxes).
    *   **Scenario Linkage (for Forecasted Assumptions)**: `G17` (Rev Growth Rate) is `=CHOOSE(Cover!$C$6, G26, G27, G28)` where `Cover!$C$6` is the scenario selector (1, 2, or 3), and `G26, G27, G28` correspond to Best, Base, Worst case growth rates for that year.
-   **Visual Design**:
    *   **Year Header**: Dark blue fill (`header_bg_dark`), white bold font (`header_fg_light`). Actuals (e.g., 2023A) light blue fill (`accent_blue_light`), white bold font. Forecasts (e.g., 2026E) medium blue fill (`accent_blue_medium`), white bold font.
    *   **Section Headers**: Dark blue fill (`header_bg_dark`), white bold font (`header_fg_light`).
    *   **Manual Inputs**: Blue font color (`input_fg`), not bold.
    *   **Formula Outputs**: Black font color, not bold.
    *   **Key Subtotals (Gross Profit, Operating Income, Pre-tax Income)**: Bold font, top border (`thin`), bottom border (`thin`).
    *   **Net Income**: Bold font, top border (`thin`), bottom border (`double`), yellow fill (`warning_fill`).
    *   **Assumptions Input Cells**: Blue font color (`input_fg`), not bold.
    *   **Grouping**: Rows for assumptions (17-22) are grouped. Columns for forecast years (G-K) are grouped.
    *   **Sheet Protection**: All cells are locked by default. Only cells with manual inputs or assumption selections are unlocked. The sheet is then protected.
-   **Charts/Tables**: No specific charts or tables are generated by this sheet_shell, but the structure is conducive to adding them.
-   **Theme Hooks**: `header_bg_dark`, `header_fg_light`, `accent_blue_light`, `accent_blue_medium`, `warning_fill`, `input_fg`.
