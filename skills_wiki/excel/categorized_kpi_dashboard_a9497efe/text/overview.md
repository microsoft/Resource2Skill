...
> **Skill Name**: Categorized KPI Dashboard

* **Tier**: sheet_shell
* **Core Mechanism**: Generates a grid of self-contained KPI cards with large typography, categorized headers, and dynamic conditional formatting based on target thresholds. 
* **Applicability**: Ideal for executive summaries, balanced scorecards, or top-level metric reporting where high-density, at-a-glance visual impact is the priority over granular data tables.

### 2. Structural Breakdown

- **Data Layout**: Categories span across KPI cards; each individual KPI card is constructed as a 3-row by 4-column grid (Title row, large Value row, and a bottom row displaying side-by-side sub-metrics). 
- **Formula Logic**: Primarily a visual engine; applies `CellIsRule` conditional formatting directly to the main KPI value, evaluating it against static targets to apply green/red health indicators.
- **Visual Design**: Uses merged cells for centered large typography (size 24). The top and bottom rows of each card use a subtle light gray fill, creating an enclosed card effect when wrapped in thin borders.
- **Charts/Tables**: Bypasses traditional charts in favor of highly legible "big number" dashboard design patterns.
- **Theme Hooks**: Utilizes a bold primary header color for category dividers and standard success/danger tints for conditional formatting.
