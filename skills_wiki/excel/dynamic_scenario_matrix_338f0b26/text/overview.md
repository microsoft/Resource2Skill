...
> **Skill Name**: Dynamic Scenario Matrix

* **Tier**: sheet_shell
* **Core Mechanism**: Uses a Data Validation dropdown connected to `CHOOSE` formulas to instantly swap driver assumptions in a centralized "Live Case" block. This live block then acts as the single source of truth driving the financial output matrix above it, preventing messy nested `IF` statements.
* **Applicability**: Essential for robust financial forecasting, operational planning, and sensitivity analysis where multiple outcomes (e.g., Base, Upside, Downside) need to be presented cleanly in a single worksheet without VBA.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Rows 3-7**: The Financial Output (Revenue, COGS, Profit).
  - **Rows 9-13**: The "Live Case" Drivers (Volume, Price, Cost).
  - **Rows 15+**: Identically structured scenario blocks (Base, Upside, Downside) stacked underneath.
- **Formula Logic**: 
  - `CHOOSE($J$2, C16, C22, C28)` maps the integer selected in the dropdown cell (`J2`) to the corresponding scenario's row index.
- **Visual Design**: The Live Case block is highlighted with an accent background to visually distinguish active assumptions from the static scenario storage blocks. The Scenario selector dropdown uses a distinct highlight color.
- **Charts/Tables**: N/A
- **Theme Hooks**: Uses `header` for section titles, `accent` for the Live Case background, and `highlight` for the scenario dropdown cell.
