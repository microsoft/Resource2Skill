...
> **Skill Name**: Themed Interactive Sales Dashboard Layout

*   **Tier**: `sheet_shell`
*   **Core Mechanism**: Constructs a visually cohesive and interactive dashboard layout on a single Excel sheet, leveraging pre-existing PivotTables and slicers. It applies a consistent color theme, positions key performance indicator (KPI) elements (shapes, icons, linked text boxes), and arranges multiple charts and a detailed pivot table for data visualization and exploration.
*   **Applicability**: Useful for managers or analysts who need to present summarized and detailed performance metrics interactively. It's applicable for any tabular dataset where trends, comparisons, and drill-down capabilities are beneficial, such as sales, marketing, operations, or financial reports. The layout is designed for easy updates by refreshing underlying data.

### 2. Structural Breakdown

-   **Data Layout**:
    *   **Source Data**: A single Excel Table (e.g., `SalesData`) containing raw transactional data (e.g., sales agent name, date, calls, deals, values). This table should be dynamically extensible.
    *   **Analysis Sheet**: A hidden sheet containing multiple PivotTables.
        *   One PivotTable for overall KPI totals (Total Calls, Calls Reached, Deals Closed, Deal Value).
        *   One PivotTable for agent-specific KPIs (Name, Total Calls, Calls Reached, Deals Closed, Deal Value).
        *   Multiple PivotTables for chart data, summarizing metrics (Calls Reached, Deals Closed, Deal Value, Average Duration, Call Drop Rate) by 'Months'.
        *   A small PivotTable on this sheet linked to the 'Name' slicer to extract the currently selected agent's name for conditional formatting.
-   **Formula Logic**:
    *   **Excel Table**: The raw data is formatted as an Excel Table (Ctrl+T) to automatically expand with new data.
    *   **PivotTables**: Generate dynamic summaries. Grouping dates by 'Months' is crucial for time-series charts.
    *   **Text Box Links**: KPIs are displayed in text boxes linked to cells in the "Analysis" sheet's summary PivotTable (e.g., `='Analysis'!B4` for total calls).
    *   **Conditional Formatting (PivotTable Data Bars)**: Applied to numerical columns in the agent KPI table to visually represent performance distribution. Uses custom colors (e.g., yellow and light purple) and hides the values to show only the bars if desired.
    *   **Conditional Formatting (Slicer Selection Highlight)**: A formula-based conditional formatting rule is applied to the 'Name' column of the agent KPI table (e.g., `=SD10=Analysis!$A$11` where D10 is the top-left cell of the name column and Analysis!$A$11 is the cell containing the currently selected name from a dedicated pivot table on the analysis sheet linked to the slicer).
-   **Visual Design**:
    *   **Overall Theme**: "Aspect" theme is applied from Page Layout > Colors.
    *   **Background**: Top header section (rows 1-8) has a dark purple fill (`#5C256E`). The main dashboard body (rows 9-40) has a lighter purple fill (`#F2EFF5`).
    *   **Titles**: "Sales Dashboard" (Aptos Narrow, 36pt, white font). Subheading "Evaluating Sales Agent Performance" (Aptos Narrow, 16pt, yellow font).
    *   **KPI Layout**: Four distinct sections in the header. Each contains a rounded-top rectangular shape (gold/white fill, no outline), an SVG icon (e.g., phone, target, ribbon, money bag, all in dark purple), a large numeric value (32pt, purple font), and a text label (18pt, purple font), both linked to pivot table cells or manually entered.
    *   **Slicer**: Formatted with the dark purple theme style, adjusted height to show all items without scrolling.
    *   **Charts**: All chart areas have no fill and no line border. Vertical axes start at 0. Gridlines and field buttons are hidden. Data labels are added where appropriate (e.g., inside end for stacked bars). Trendlines are added to relevant charts (e.g., total sales). Colors for chart series are chosen from the theme palette (yellow, purple). Shadows are applied to all charts for a 3D effect.
    *   **Agent KPIs Table**: Uses a PivotTable style that aligns with the dashboard's purple theme. Headers are cleaned by removing "Sum of".
-   **Charts/Tables**:
    *   **KPIs**: Four text boxes for Total Calls, Calls Reached, Deals Closed, Deal Value.
    *   **Sales Agent KPIs Table**: PivotTable displaying Name, Total Calls, Calls Reached, Deals Closed, Deal Value, with conditional formatting data bars.
    *   **Sum of Calls Reached & Deals Closed**: Stacked Column Chart, showing monthly trends.
    *   **Total Sales**: Column Chart, showing monthly trends with a linear trendline.
    *   **Average Call Duration**: Column Chart, showing monthly trends.
    *   **Average Call Drop Rate**: Area Chart, showing monthly trends with a gradient fill.

-   **Theme Hooks**:
    *   `header_bg`: Dark purple (`#5C256E`)
    *   `body_bg`: Light purple (`#F2EFF5`)
    *   `accent_primary`: Gold/Yellow (theme's accent color 1 or 2)
    *   `accent_secondary`: Purple (theme's accent color corresponding to the dark purple)
    *   `text_dark`: Dark purple
    *   `text_light`: White/Yellow
