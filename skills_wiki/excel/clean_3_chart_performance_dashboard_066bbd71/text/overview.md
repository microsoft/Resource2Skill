...
> **Skill Name**: Clean 3-Chart Performance Dashboard

* **Tier**: sheet_shell
* **Core Mechanism**: Sets up a dedicated dashboard presentation sheet with gridlines disabled. Routes summary data to a hidden backend data sheet. Instantiates three distinct charts (a primary large stacked column chart, and two smaller stacked line charts), aligning them perfectly to a grid layout. Extraneous chart elements (like legends on single-series line charts) are removed to maintain a professional, un-cluttered look.
* **Applicability**: Ideal for generating management reports, KPI views, or executive summaries. Best when you have categorical dimension data (e.g., market share by product) and time-series trend data (e.g., monthly volume and profit).

### 2. Structural Breakdown

- **Data Layout**: Summary data is written to a secondary hidden worksheet (`_Data` suffix). The layout is tabular, allowing easy range references for charts.
- **Formula Logic**: No complex formulas; relies purely on structural data aggregation which is plotted via standard chart references.
- **Visual Design**: The presentation sheet has `showGridLines` set to `False`. The title uses a large, bold font, and placeholder blocks are drawn using solid fills and thin borders to represent slicer/filter zones.
- **Charts/Tables**: 
  - Chart 1: `BarChart` (`grouping="stacked"`, `overlap=100`) for dimensional breakdown.
  - Chart 2 & 3: `LineChart` for monthly trends. Legends are hidden (`legend = None`) to maximize the chart plot area.
- **Theme Hooks**: Can integrate standard theme palettes. The title font color, line chart series colors, and placeholder fills can be hooked directly to `theme.title_fg`, `theme.accent_1`, and `theme.bg_alt`.
