...
> **Skill Name**: Interactive Sidebar Navigation Shell

* **Tier**: archetype
* **Core Mechanism**: Generates a multi-sheet workbook structure with a persistent, themed sidebar pane on the left side of every sheet. Each sidebar icon is internally hyperlinked to its corresponding sheet, simulating a web-app navigation experience within Excel.
* **Applicability**: Best for full-scale dashboards, reporting suites, or complex financial models where users need to seamlessly jump between distinct sections (e.g., Dashboard, Inputs, Contacts).

### 2. Structural Breakdown

- **Data Layout**: Column A acts as the navigation pane (width 8), Column B as a narrow spacer. Rows 4, 8, 12, etc., contain the navigation links.
- **Formula Logic**: Uses internal `#'SheetName'!A1` hyperlinks attached to cells to handle cross-sheet navigation without macro reliance.
- **Visual Design**: Gridlines are hidden across all sheets. The sidebar uses a solid fill color from the theme. The active sheet is indicated by a highlighted block around its icon.
- **Charts/Tables**: Provides the macro-layout shell; content (charts/tables) goes into Column C onwards.
- **Theme Hooks**: Consumes primary brand color for the sidebar background, a lighter/active variant for the selected state, and high-contrast text color for icons.
