...
> **Skill Name**: ECommerce Scenario Planning Model

* **Tier**: archetype
* **Core Mechanism**: This skill constructs a two-tab Excel workbook designed for e-commerce financial forecasting. It establishes a dedicated 'Assumptions' tab for all input variables, which then dynamically drives monthly revenue, gross profit, and contribution margin calculations on a 'Model' tab. The model includes product-line specific forecasts, combined totals, sanity checks, and a visual line chart, enabling interactive scenario planning by simply adjusting input assumptions.
* **Applicability**: This skill is ideal for e-commerce businesses or financial analysts who need to build flexible, forward-looking financial models for internal planning. It is particularly useful for tracking product-line performance, understanding key drivers like order value and growth rate, and evaluating profitability (gross and contribution margins) over multiple periods.

### 2. Structural Breakdown

- **Data Layout**:
    - **Assumptions Tab**: Clearly labeled sections for 'ACCESSORIES', 'DEVICES', and 'GENERAL' inputs. Each product line includes fields for Average Order Value, Starting Monthly Orders, Monthly Order Growth Rate, and Gross Margin %. The 'GENERAL' section defines the Forward Forecast (Months) and Model Start Date (Month). An additional input for 'Marketing Spend (% of Total Revenue)' is included. Blue font is used for editable input cells.
    - **Model Tab**: Presents a 24-month forecast (dynamic based on assumptions). Structured into sections for 'ACCESSORIES', 'DEVICES', 'COMBINED TOTALS', and 'SANITY CHECK'. Each product line section details Average Order Value, Orders, Revenue, Gross Margin %, and Gross Profit. The 'COMBINED TOTALS' section aggregates total revenue, total gross profit, blended gross margin, marketing spend ($), and contribution margin ($).
- **Formula Logic**:
    - **Dynamic References**: All calculated values on the 'Model' tab dynamically reference corresponding input cells on the 'Assumptions' tab (e.g., `=Assumptions!$B$4`).
    - **Orders Calculation**: Month 1 orders reference 'Starting Monthly Orders' from 'Assumptions'. Subsequent months apply 'Monthly Order Growth Rate' (e.g., `={PrevMonthOrders}*(1+Assumptions!$B$6)`).
    - **Revenue Calculation**: `Average Order Value * Orders`.
    - **Gross Profit Calculation**: `Revenue * Gross Margin %` (with Gross Margin referencing the 'Assumptions' tab).
    - **Total Revenue**: Sum of individual product line revenues.
    - **Total Gross Profit**: Sum of individual product line gross profits.
    - **Blended Gross Margin %**: `Total Gross Profit / Total Revenue`.
    - **Marketing Spend ($)**: `Total Revenue * Marketing Spend (% of Total Revenue)` (percentage references 'Assumptions' tab).
    - **Contribution Margin ($)**: `Total Gross Profit - Marketing Spend ($)`.
    - **Sanity Check**: Compares the sum of individual product line revenues against the 'Total Revenue' cell, yielding "Match" or "Mismatch".
- **Visual Design**:
    - **Headers**: Main title on both tabs (e.g., "E-Commerce Scenario Planning Model") uses bold font with a dark background and light foreground, driven by theme.
    - **Input Cells**: Blue font for editable values on the 'Assumptions' tab (`palette.input_color`).
    - **Number Formats**: Currency (`$#,##0`), percentage (`0.0%`), and integer (`#,##0`) formats are applied consistently.
    - **Sanity Check**: Conditional formatting (e.g., green for "Match", red for "Mismatch") on the 'Matches Total Revenue?' row.
    - **Borders**: Subtle borders for readability of sections.
- **Charts/Tables**:
    - **Line Chart**: A line chart titled "Monthly Revenue & Contribution Margin By Product Line" is embedded in the 'Model' tab. It plots monthly 'Accessories Revenue', 'Devices Revenue', and 'Contribution Margin' over the forecast period, using distinct, theme-aligned colors (blue, orange, green).
- **Theme Hooks**: `header_bg`, `header_fg`, `text_color`, `input_color`, `accent_color`, `positive_color`, `negative_color`.
