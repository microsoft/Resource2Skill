...
> **Skill Name**: Split Panel Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Configures a worksheet as a modern dashboard canvas by disabling gridlines, defining a dark-themed left sidebar column for high-level KPIs, and painting a contrasting light-themed main staging area. A helper mechanism programmatically generates cleanly bordered "Card" regions within the grid to host charts and tables, mimicking the floating-shape layout technique commonly used by Excel dashboard designers.
* **Applicability**: Use for executive summaries, static dashboards, or staging areas for interactive reports. It replaces default Excel grid views with a structured layout, neatly separating at-a-glance metrics from detailed visualizations and mitigating the need for finicky floating shapes.

### 2. Structural Breakdown

- **Data Layout**: Leverages strict column width controls. Column B acts as the broad sidebar. Columns D through N act as the flexible main grid. Rows are uniformly sized, allowing block merging to act as distinct UI panels.
- **Formula Logic**: Static UI shell; leaves spaces intentionally blank or lightly merged so the developer can anchor pivots, formulas, or charts inside the predefined "Cards".
- **Visual Design**: Disables sheet gridlines. Paints background cells to simulate a UI frame (e.g., Slate 800 for sidebar, Slate 100 for canvas). Renders outer thin borders and white backgrounds over specific grid blocks to create a "floating card" effect. 
- **Charts/Tables**: Acts as the host container. The `create_card` boundaries provide exact cell coordinates (e.g., `D3:H14`) that should be passed as anchors to chart rendering functions.
- **Theme Hooks**: Consumes a primary dark color for the `sidebar_bg`, a high-contrast accent for `sidebar_accent` text, and secondary neutral grays for the `canvas_bg` and `card_border`. 
