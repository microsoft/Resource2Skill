...
> **Skill Name**: Card-Based Sidebar Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Creates a structural layout mimicking a modern web application dashboard. Converts the left-most column into a dedicated dark navigation sidebar with hyperlinked menu items, hides Excel gridlines, and uses grouped cell regions with white fills and thin borders to represent floating visual "cards" that house KPIs and charts. 
* **Applicability**: Use for high-level executive dashboards and reporting screens where a professional, UI-like layout is desired. Ideal for summarizing multiple charts and KPIs in a single clean view without relying on fragile floating shapes.

### 2. Structural Breakdown

- **Data Layout**: A 16-column grid system. Column A is the fixed sidebar (width 15). Columns C-P form the main content area, separated into three distinct column blocks (C-F, H-K, M-P) divided by narrow spacer columns (G, L).
- **Formula Logic**: Uses internal workbook `#` hyperlinks mapped to specific sheet targets to create a clickable navigation menu.
- **Visual Design**: Turns off native gridlines. Applies a light gray canvas background (`#F0F2F5`). Dashboard "cards" have a solid white background (`#FFFFFF`) framed by a subtle gray thin outer border to create a shadow/lift effect. 
- **Charts/Tables**: Leaves clean, pre-defined 4-column blocks at the top for KPI text/metrics, and larger multi-span blocks at the bottom intended for dropping in Line and Radar charts.
- **Theme Hooks**: Utilizes a dark background (`sidebar_color`) for the navigation pane and contrasting light backgrounds for the card canvas.
