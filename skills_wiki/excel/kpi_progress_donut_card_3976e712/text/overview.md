...
> **Skill Name**: KPI Progress Donut Card

* **Tier**: component
* **Core Mechanism**: Generates a self-contained dashboard KPI card featuring a main metric value and a progress Doughnut chart. It creates hidden data for the chart, structures the layout within a framed cell area, and specifically colors the chart slices using `DataPoint` overrides to highlight the completed percentage against a muted remainder slice.
* **Applicability**: Perfect for high-level executive dashboards tracking progress toward targets like sales quotas, completion rates, or budget utilization.

### 2. Structural Breakdown

- **Data Layout**: Renders text metrics locally within the card cells, while offloading chart data (Actual % and Remainder %) to an off-screen hidden column (20 columns to the right) to avoid cluttering the visual area.
- **Formula Logic**: Calculates `pct` and `remainder` mathematically in Python before injecting into the sheet, leaving the card itself completely static and snapshot-friendly.
- **Visual Design**: Uses merged cells and varied font sizing (20pt bold for the main metric, 9pt for sub-labels) to establish a clear visual hierarchy. Wraps the multi-cell card in a subtle, thin gray outer border with a white background fill.
- **Charts/Tables**: Utilizes a `DoughnutChart` with `holeSize=70`, stripped of titles and legends. Employs `DataPoint` index mapping to explicitly assign the theme's accent color to the progress slice and a light gray to the remaining slice.
- **Theme Hooks**: Consumes `accent_color` for the progress chart slice and sub-label text, standard `text_color` for values, and `muted_color` for the title.
