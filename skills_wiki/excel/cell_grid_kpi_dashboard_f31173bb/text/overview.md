...
> **Skill Name**: Cell-Grid KPI Dashboard

* **Tier**: sheet_shell
* **Core Mechanism**: Disables gridlines and uses background color fills coupled with targeted cell merges to create distinct "panels" or "cards". Data is written to hidden columns off-screen, and charts are anchored directly into the visual panels, achieving a modern shape-based dashboard aesthetic purely through cell styling.
* **Applicability**: Ideal for executive summaries and top-level metric dashboards where an overarching view of KPIs and standard charts (Bar, Column, Line) needs to be presented cleanly without complex VBA or floating shapes. 

### 2. Structural Breakdown

- **Data Layout**: Raw data is staged out of sight (Col AA onwards) and hidden to keep the canvas clean.
- **Formula Logic**: No complex formulas; directly maps static summarized data into visual components.
- **Visual Design**: 
  - **Canvas**: Light gray (`F3F4F6`) applied universally to simulate a web application background.
  - **Panels**: Crisp white (`FFFFFF`) with a subtle gray border (`D1D5DB`) to simulate drop-shadowed cards.
  - **Typography**: Dark slate for headers (`1F2937`) and values (`111827`), muted gray for KPI labels (`6B7280`).
- **Charts/Tables**: Employs `ColumnChart` and `BarChart` sized explicitly in centimeters to sit perfectly within their respective merged-cell panel boundaries.
- **Theme Hooks**: Uses a fallback dictionary mimicking `bg_canvas`, `bg_panel`, `text_title`, `text_label`, and `text_value`.
