...
> **Skill Name**: Interactive Themed Sales Dashboard Shell

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill constructs the foundational layout of an interactive sales dashboard within a single Excel worksheet. It creates a themed sidebar for basic navigation, distinct "shapes" (simulated via merged cells) as containers for various KPIs and charts, and embeds dynamic charts and linked text boxes with mock data. The reusability comes from its adaptable layout and theme-driven styling.
*   **Applicability**: This skill is ideal for business analysts or report developers needing to quickly set up a visually appealing and organized dashboard in Excel. It provides a structured shell that can be easily populated and customized for different data sets and reporting needs, particularly for tracking KPIs, trends, geographical breakdowns, and satisfaction metrics.

### 2. Structural Breakdown

-   **Data Layout**:
    *   Column A: Fixed width (8) for navigation elements.
    *   Columns B:M: Main dashboard content area. Column widths are adjusted for visual balance.
    *   Merged cells are used extensively to simulate distinct "shapes" for the main header, KPI boxes, sales trend chart, customer satisfaction chart, and sales by country chart.
    *   An auxiliary "Inputs" sheet is created to house the raw data for charts and KPI calculations, demonstrating a separation of data from presentation.
-   **Formula Logic**:
    *   KPI percentages (e.g., "85%"): Calculated as `Actual / Target` on the "Inputs" sheet and linked dynamically into the text boxes within the donut charts (e.g., `=Inputs!D7`).
    *   KPI actual amounts (e.g., "$2,544"): Linked dynamically from the "Inputs" sheet to cells within the respective KPI "shapes" (e.g., `=Inputs!D5`).
    *   Hyperlinks: Text links are added to cells in Column A to navigate to different sheets (Dashboard, Inputs, Contacts) or external resources (email).
-   **Visual Design**:
    *   **Background**: Column A is filled with `theme.sidebar_bg`. The main dashboard area (`B:M`) has a white background.
    *   **"Shapes" (Merged Cells)**: All content containers (header, KPIs, charts) are created using merged cell ranges, which are then formatted with borders and fills to give the appearance of distinct shapes.
        *   **Header**: Merged cells (B2:M3) with `theme.header_bg` fill and `theme.header_fg` text.
        *   **KPIs**: Merged cells (3x, e.g., B5:D9) with white fill and a thin `light_grey` border.
        *   **Charts**: Merged cells for line chart (B11:H22), map chart (K1:M16), and radar chart (I18:M22) with white fill and `light_grey` borders.
    *   **Fonts**: Calibri (Body) is used throughout. Titles are bold and larger (e.g., 24pt for main title, 14pt for section titles), KPI numbers are bold and large (e.g., 22pt).
    *   **Borders**: Thin solid `light_grey` borders are applied to the merged cell "shapes".
    *   **Shadows**: `openpyxl` does not directly support shape shadows; this visual element cannot be fully replicated.
-   **Charts/Tables**:
    *   **Line Chart (Sales Trend)**: Plots mock monthly sales data for two years (2021, 2022). Lines are colored `accent_blue_dark` and `accent_red` with circular markers. Y-axis minimum is set to 180 for better trend visualization. No chart title or axis titles.
    *   **Bar Chart (Sales by Country)**: As `openpyxl` doesn't natively create interactive geographical maps, a themed `BarChart` is used as a functional alternative to display sales figures per country in South America. Bars are `accent_blue_dark`.
    *   **Radar Chart (Customer Satisfaction)**: Visualizes mock customer satisfaction scores across multiple categories (Speed, Quality, Hygiene, Service, Availability). The line and markers are `accent_blue_dark`.
    *   **Donut Charts (KPI Completion)**: Three separate donut charts for Sales, Profit, and Customers. Each shows "percentage complete" vs "remainder" using `accent_blue_dark` and `accent_blue_light` respectively. The actual percentage is displayed as a bold, dark blue text box overlayed inside the donut.

-   **Theme Hooks**:
    *   `header_bg`: `#213F60` (Dark Blue)
    *   `header_fg`: `#FFFFFF` (White)
    *   `sidebar_bg`: `#213F60` (Dark Blue)
    *   `text_color_dark`: `#213F60` (Dark Blue)
    *   `accent_red`: `#DA4453` (Red)
    *   `accent_blue_dark`: `#4285F4` (Google Blue)
    *   `accent_blue_light`: `#ADD8E6` (Light Blue)
    *   `white`: `#FFFFFF` (White)
