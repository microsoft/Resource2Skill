...
> **Skill Name**: Multi-Panel Executive Dashboard

* **Tier**: archetype
* **Core Mechanism**: Constructs a clean, professional dashboard layout by hiding worksheet gridlines, applying a prominent theme-colored title banner, isolating driving data into a hidden calculation sheet, and cleanly arranging multiple configured charts (stacked column and trend lines) into a unified view.
* **Applicability**: Ideal for creating executive-level summary reports where the focus is strictly on data visualization and insights rather than tabular data entry.

### 2. Structural Breakdown

- **Data Layout**: Places aggregated summary data into a dedicated, hidden `"CalcData"` worksheet to keep the presentation layer uncluttered.
- **Formula Logic**: Charts dynamically pull from specific isolated ranges in the hidden calculation sheet using standard structural references.
- **Visual Design**: Hides native Excel gridlines (`showGridLines = False`), creates a wide merged banner `A1:P3` with centered, bold white text set against a dark primary theme color.
- **Charts/Tables**: Employs a `BarChart` (set to `stacked` with `overlap=100`) for segmented totals, alongside two `LineChart` objects with their legends hidden (`chart.legend = None`) for cleaner trend displays. 
- **Theme Hooks**: Utilizes `theme` variables to map `primary` (banner background) and `text` (banner text color) dynamically.
