...
> **Skill Name**: Progress Donut KPI Tracker

* **Tier**: component
* **Core Mechanism**: Constructs a two-slice Doughnut chart to visualize progress toward a target. It calculates the completion vs. remaining percentages, formats the "completed" slice with a prominent theme color and the "remaining" slice with a muted background color, removes the legend, and reduces the donut hole size for a thicker, modern UI ring.
* **Applicability**: Best for high-level executive dashboards displaying critical metrics like sales quota attainment, budget utilization, or project completion percentages in a card-like layout.

### 2. Structural Breakdown

- **Data Layout**: Writes "Completed" and "Remaining" labels and their calculated percentages into a compact 2x2 grid at the anchor cell.
- **Formula Logic**: Calculates progress natively in Python (`Completed = Actual / Target`, `Remaining = 1 - Completed`) and pushes the static percentages to the sheet for chart rendering.
- **Visual Design**: Hides chart legend, sets a square aspect ratio, and embeds the core percentage into the chart title (approximating a floating text box).
- **Charts/Tables**: `DoughnutChart` with `holeSize` adjusted down to 65% (from the default 75%) to make the progress ring thicker and more pronounced.
- **Theme Hooks**: Uses a primary brand color (`theme_colors["primary"]`) for the active progress slice and a muted/gray color (`theme_colors["muted"]`) for the remainder slice.
