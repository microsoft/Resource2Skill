...
> **Skill Name**: Data Bar Leaderboard

* **Tier**: component
* **Core Mechanism**: Formats a tabular dataset as a ranking leaderboard. Applies column-specific Data Bar conditional formatting to highlight relative performance metrics directly within the grid layout.
* **Applicability**: Best used in dashboards or summary reports to rank entities (like sales agents, products, or regions) across multiple KPIs. Input data should ideally be pre-sorted by the primary metric for maximum readability.

### 2. Structural Breakdown

- **Data Layout**: Renders a title, a themed header row, and a dynamic grid of data rows based on a provided column configuration.
- **Formula Logic**: None (relies on static pre-calculated values passed into the component, making it fast and lightweight).
- **Visual Design**: Uses a dark primary color for the header with white text. Column widths and text alignments are strictly controlled.
- **Charts/Tables**: Employs `DataBarRule` conditional formatting on numeric columns, allowing distinct colors for each metric to aid visual scanning across different scales.
- **Theme Hooks**: Uses a primary color for the header fill and title font, while consuming explicit bar colors defined in the column configuration to support multi-metric dashboards.
