...
> **Skill Name**: Clean Static Chart Dashboard

* **Tier**: archetype
* **Core Mechanism**: Generates a gridless presentation layer worksheet with a colored hero header. Constructs stacked column and line charts from a hidden data sheet, and anchors them into an aligned dashboard layout. 
* **Applicability**: Use this archetype to present a clean, C-level KPI dashboard. Since `openpyxl` cannot natively generate PivotTables and interactive Slicers from scratch, this pattern constructs the static layout equivalent for automated reporting, keeping raw data cleanly separated on a hidden sheet.

### 2. Structural Breakdown

- **Data Layout**: Two datasets (Trend and Category) are written to a hidden `Data_DoNotModify` sheet.
- **Formula Logic**: None (pure chart data references).
- **Visual Design**: The dashboard sheet (`Dashboard`) disables gridlines for a clean canvas. Rows 1-3 are merged across columns A-Q and filled with the primary theme color to create a bold title band.
- **Charts/Tables**: 
  - Chart 1: Line Chart for unit trends.
  - Chart 2: Line Chart for profit trends.
  - Chart 3: Stacked Column Chart for market and category breakdowns (`overlap=100`, `grouping="stacked"`).
- **Theme Hooks**: Consumes `primary` for the header band background and `text_on_primary` for the header text.
