...
> **Skill Name**: Scenario Planning Financial Model

* **Tier**: archetype
* **Core Mechanism**: Separates hardcoded business inputs into a dedicated 'Assumptions' tab and references them dynamically to build a 24-month rolling forecast on a 'Model' tab. Includes dynamic time-series calculations, combined margin aggregates, and an automated sanity-check row to catch calculation mismatches.
* **Applicability**: Strategic financial forecasting, business scenario planning, SaaS/E-commerce revenue modeling, or any time-series projection where multiple inputs need to be adjusted independently to simulate outcomes.

### 2. Structural Breakdown

- **Data Layout**: Two distinct worksheets. 'Assumptions' contains vertical parameter lists grouped by product line with a single column for hardcoded inputs. 'Model' contains a 24-column time-series matrix (Month 1 to Month 24) broken down by revenue lines, aggregate totals, and a sanity check.
- **Formula Logic**: 
  - Base month calculations strictly multiply volume assumptions (e.g., `=Assumptions!$B$3*Assumptions!$B$4`).
  - Rolling forecast months reference the previous month and apply growth factors (e.g., `=C3*(1+Assumptions!$B$5)`).
  - Built-in sanity check uses absolute match validation: `=IF(ROUND(B3+B6,2)=ROUND(B9,2), "✓ Match", "✗ Error")`.
- **Visual Design**: The model uses a classic financial architecture: bold headers, distinct shading for editable hardcoded inputs (e.g., light gray/yellow), standard number formatting (Currency vs. Percentages), and conditional formatting applied to the validation/sanity check row.
- **Charts/Tables**: A 24-month Line Chart plotting Revenue lines and Contribution Margin on the same axis to visualize long-term margin trends alongside product growth.
- **Theme Hooks**: Utilizes `header_bg` and `header_fg` for column titles, `input_bg` to highlight assumption cells, and `success`/`error` colors for the conditional sanity check formatting.
