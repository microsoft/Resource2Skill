...
> **Skill Name**: Interactive Performance Dashboard

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a fully interactive, multi-sheet Excel dashboard. It dynamically pulls data from a structured Excel Table, generates linked pivot tables and pivot charts on hidden sheets, and presents them on a user-friendly dashboard sheet. Interactivity is achieved through slicers and timelines (represented as placeholders due to openpyxl limitations) connected to all underlying pivot tables, allowing users to filter and analyze data on the fly.
*   **Applicability**: This skill is ideal for creating dynamic business reports such as sales performance dashboards, financial overviews, or operational summaries. It is suitable for presenting large datasets where users need to explore trends and insights across various dimensions (e.g., time, category, geography) without requiring direct Excel formula or pivot table knowledge.

### 2. Structural Breakdown

-   **Data Layout**: The raw data is organized in a single sheet named "Data", formatted as an Excel Table. Columns include categorical data like 'Country' and 'Product', quantitative data like 'Units Sold', 'Revenue', 'Cost', and 'Profit', and temporal data 'Date' (with an added 'Months' column for grouping). Auxiliary sheets for pivot tables and charts are created and subsequently hidden.
-   **Formula Logic**: The dashboard primarily relies on Excel's built-in PivotTable functionality for data aggregation (e.g., Sum of Profit, Sum of Units Sold). No complex custom formulas are explicitly shown within the dashboard or pivot tables, leveraging the aggregation power of pivots.
-   **Visual Design**: The dashboard sheet features a large merged header area with a company logo (conceptual placeholder) and the dashboard title, formatted with theme-driven background and font colors. Gridlines and row/column headings are hidden for a clean, report-like appearance. Charts are positioned and aligned using Excel's alignment tools. Slicer headers are conceptually hidden, and slicers are resized for aesthetic integration (represented by merged cells due to openpyxl limitations). Themes are used to control overall color schemes.
-   **Charts/Tables**:
    -   **Data Table**: A single Excel Table named "Data" on the "Data" sheet.
    -   **Pivot Tables**: Three pivot tables are generated on hidden sheets, acting as data sources, with their data directly prepared in code to simulate pivot table output:
        1.  "Profit by market and cookie": Rows by 'Country', columns by 'Product', values by 'Sum of Profit'.
        2.  "Units sold each month": Rows by 'Months', values by 'Sum of Units Sold'.
        3.  "Profit by month": Rows by 'Months', values by 'Sum of Profit'.
    -   **Pivot Charts**:
        1.  **Stacked Column Chart**: Visualizes "Profit by Market & Cookie Type", sorting countries by profit (largest to smallest) and stacking cookie types within each country.
        2.  **Line Chart**: Visualizes "Units Sold Each Month" to show trends over time.
        3.  **Line Chart**: Visualizes "Profit by Month" to show profit trends over time.
-   **Theme Hooks**:
    -   `header_bg_fill`: Applied to the main dashboard header background.
    -   `header_fg_font`: Applied to the main dashboard header font color.
    -   `sheet_bg_fill`: Applied as the default background for sheets (typically white).
    -   `chart_colors`: Used for chart series colors (implicitly managed by Excel's theme when charts are added).
