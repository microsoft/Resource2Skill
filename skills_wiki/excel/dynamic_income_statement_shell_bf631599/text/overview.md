...
> **Skill Name**: Dynamic Income Statement Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a 3-statement modeling style Income Statement that dynamically projects future periods based on a scenario toggle. It separates hardcoded historical inputs (styled with conventional blue font) from formulas, applies custom `"A"` (Actual) and `"E"` (Estimate) number formats to year headers, and uses Excel's `CHOOSE` function tied to a Data Validation dropdown to drive active forecast assumptions.
* **Applicability**: Best used for building financial models, valuation templates, or FP&A forecasting sheets where users need to cleanly separate historical data from projections and seamlessly toggle between Best, Base, and Worst-case scenarios.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Top**: Scenario toggle (dropdown cell).
  - **Upper grid**: Income statement line items (Revenue, COGS, Subtotals) spread across historical and forecast years.
  - **Lower grid**: Active Assumptions driving the forecast, followed by the specific Scenario Matrices (Best/Base/Worst inputs).
- **Formula Logic**: 
  - Subtotals: Gross Profit (`=Rev-COGS`), OpInc (`=GrossProfit-SGA`).
  - Active Assumptions: `=CHOOSE($B$2, Best_Cell, Base_Cell, Worst_Cell)` to fetch the correct growth rate based on the active scenario.
  - Projections: Prior Year * (1 + Growth Rate) or Current Year Revenue * Margin %.
- **Visual Design**: 
  - Headers: Dark background, bold white text. 
  - Subtotals: Top and bottom borders, bold text to clearly denote hierarchy.
  - Hardcodes: Blue font color (`#0000FF`) to indicate manual inputs, standardizing financial modeling best practices.
- **Charts/Tables**: Standard grid layout; no native Excel Tables used to maintain strict absolute/relative cell reference control across time series.
- **Theme Hooks**: `header_bg` (header background), `header_fg` (header text), `hardcode_fg` (blue manual inputs).
