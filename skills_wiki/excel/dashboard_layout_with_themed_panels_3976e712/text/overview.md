...
> **Skill Name**: Dashboard Layout with Themed Panels

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill constructs the structural foundation of an Excel dashboard by arranging and styling merged cell blocks as distinct, themed panels. It creates areas for a main title, key performance indicators (KPIs), trend charts, and geographical/satisfaction visuals, enabling a clean and organized presentation of data.
*   **Applicability**: Ideal for creating any visually segmented dashboard where information needs to be logically grouped and presented with a consistent aesthetic. It's suitable for executive summaries, operational reports, or sales dashboards across various industries.

### 2. Structural Breakdown

-   **Data Layout**: The skill itself defines the layout, not the data. It assumes underlying data exists in separate sheets (e.g., "Inputs") that will later populate visuals placed within these panels.
-   **Formula Logic**: Not applicable to the layout structure directly. Visuals placed within these panels (e.g., donut chart percentages) would link dynamically to data cells.
-   **Visual Design**:
    *   **Sheet Background**: White. Gridlines hidden.
    *   **Navigation Sidebar**: Column A is filled with a dark blue `header_bg` color, intended for hyperlinked icons (not implemented in this code).
    *   **Panels**: All content panels (main title, KPIs, charts) are created using merged cells. They have a white `panel_bg` fill and a thin grey `panel_shadow` colored border, mimicking the rounded rectangles with shadows shown in the video.
    *   **Titles within Panels**: Text uses a dark blue `text_color_dark` font. Main dashboard title is larger (size 18), bold. Subtitles and panel headers are smaller (size 11), bold (except for the main subtitle).
-   **Charts/Tables**: This skill creates the *containers* for charts and tables, but does not embed the charts or tables themselves.
-   **Theme Hooks**: `header_bg` (for sidebar), `panel_bg` (for panel backgrounds), `panel_shadow` (for subtle panel borders), `text_color_dark` (for all text).
