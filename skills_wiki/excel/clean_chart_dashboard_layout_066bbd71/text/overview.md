...
> **Skill Name**: Clean Chart Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Removes worksheet gridlines, constructs a prominent thematic header banner across the top rows, reserves a left-hand column for interactive controls (like slicers), and arranges multiple charts into an aligned grid layout powered by a hidden backing data sheet.
* **Applicability**: Best used when summarizing aggregated data into a single, presentation-ready "Dashboard" tab. This provides a polished, application-like feel for management reporting, removing the raw spreadsheet grid and focusing purely on KPIs and visualizations.

### 2. Structural Breakdown

- **Data Layout**: Creates a hidden `_Data` sheet to store the chart backing data, keeping the main dashboard sheet purely visual.
- **Formula Logic**: N/A (Focuses on structural layout and chart positioning).
- **Visual Design**: Gridlines disabled (`showGridLines = False`). Rows 1-4 merged for a title banner with a deep theme-colored background and white bold text. Slicer placeholders are formatted with light gray backgrounds to designate interactive zones.
- **Charts/Tables**: Includes a Stacked Column chart and two Line charts positioned at specific cell anchors (e.g., `D6`, `L6`, `L21`) to form a structured 2-column layout. Line charts have their legends removed to maximize plot space.
- **Theme Hooks**: Uses a primary theme color (fallback `#203764`, deep corporate blue) for the header background and matching section titles.
