...
> **Skill Name**: Tile-Based KPI Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a grid-based dashboard interface by disabling sheet gridlines, painting a continuous background color, and simulating a navigation sidebar. Creates discrete "cards" using merged cells with stark white fills and subtle borders to anchor high-level KPI text and embedded charts (Line Trend and Radar).
* **Applicability**: Best used for executive summaries, financial overviews, or regional performance tracking where a clean, modern UI is required to present multiple diverse metrics (trends, distributions, and single-value KPIs) on a single sheet.

### 2. Structural Breakdown

- **Data Layout**: A hidden/secondary data sheet (`_Data` suffix) stores the raw chronological sales data and categorical satisfaction metrics, preventing clutter on the presentation layer.
- **Formula Logic**: Pure structural layout; relies on direct value injection for KPIs to act as a presentation shell.
- **Visual Design**: Disables gridlines natively. Simulates a left-hand navigation pane using a dark contiguous column fill. Uses light gray (`F2F2F2`) for the dashboard background and pure white (`FFFFFF`) with thin gray borders for the metric "cards".
- **Charts/Tables**: Integrates an `openpyxl` `LineChart` for multi-year trend comparison (configured with explicit circular markers) and a `RadarChart` for multi-axis customer satisfaction mapping. 
- **Theme Hooks**: Utilizes `sidebar_bg` for the faux-navigation pane, `bg` for the canvas, `card_bg` for the content blocks, and relies on default chart styles which inherit workbook themes.
