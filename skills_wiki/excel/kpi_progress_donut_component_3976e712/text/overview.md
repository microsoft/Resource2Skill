...
> **Skill Name**: KPI Progress Donut Component

* **Tier**: component
* **Core Mechanism**: Creates a standalone, high-contrast Doughnut Chart to visualize completion percentage against a target. The technique explicitly customizes the donut's `holeSize` to 65% for a thicker ring, strips out the legend/borders for a clean "card" integration, and manually targets slice data points to enforce a primary vs. muted-grey color scheme.
* **Applicability**: Ideal for executive dashboards where individual KPIs (like Customer Satisfaction, % Sales to Target, or SLA Adherence) need to be visualized as clean, standalone progress rings without the clutter of standard Excel chart elements.

### 2. Structural Breakdown

- **Data Layout**: A hidden 2x2 grid is written at the anchor point (Achieved % and Remaining %) which the chart is placed directly over. 
- **Formula Logic**: Calculates the remainder natively as `1 - achieved_pct` before charting.
- **Visual Design**: The chart area background and border lines are set to `noFill=True` to seamlessly blend into dashboard backgrounds.
- **Charts/Tables**: `DoughnutChart` is utilized with specific `holeSize = 65` property. Slice 0 (Achieved) is hardcoded to a theme accent color, while Slice 1 (Remaining) is forced to a neutral light grey (`#D9D9D9`).
- **Theme Hooks**: Consumes the primary accent color (e.g., corporate blue) for the "Achieved" slice, contrasting against a standard neutral token for the remaining track.
