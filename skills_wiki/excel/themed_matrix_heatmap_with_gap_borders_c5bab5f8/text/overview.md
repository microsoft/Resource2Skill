...
> **Skill Name**: Themed Matrix Heatmap with Gap Borders

* **Tier**: component
* **Core Mechanism**: Extracts a 2D dataset (similar to a cross-tabulated Pivot Table) into a cleanly formatted matrix. It applies percentage number formats and dynamically overlays an openpyxl `ColorScaleRule` to generate a heatmap. Critically, it applies a thick, solid white border to every data cell to visually simulate "padding" or "gaps" between the heat blocks—a modern dashboard UI design trick.
* **Applicability**: Best used for visualizing cross-tabulated behavioral data, demographic splits, or time-series heatmaps (e.g., hour-of-day vs. day-of-week). Acts as a highly visual, presentation-ready alternative to default Pivot Table outputs or standard 100% stacked bar charts.

### 2. Structural Breakdown

- **Data Layout**: A merged header row for the title, a header row for column categories, a leading column for row categories, and a central `NxM` matrix of values.
- **Formula Logic**: Static extraction; the component receives raw computed matrix arrays (mimicking the tutorial's step of mapping cells outside a Pivot Table to avoid formatting lock-in).
- **Visual Design**: Bold, subtle-colored axes labels. The defining feature is the interior thick white cell boundaries (`Border(Side(style='thick', color='FFFFFF'))`) intersecting the conditional background fill.
- **Charts/Tables**: Replaces native charts entirely with a grid driven by Excel's native Conditional Formatting (Color Scales).
- **Theme Hooks**: The `max_color` of the color scale should consume the theme's primary accent color, while the `min_color` uses an ultra-light tint or plain white to ensure contrast.
