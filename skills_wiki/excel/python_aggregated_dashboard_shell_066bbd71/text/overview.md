...
> **Skill Name**: Python-Aggregated Dashboard Shell

* **Tier**: archetype
* **Core Mechanism**: Aggregates raw transaction data into a hidden calculation sheet using standard Python data structures (like `defaultdict`), then renders a multi-chart visual layout (dashboard shell) by pointing openpyxl Line/BarCharts at the aggregated ranges. Simulates the visual result of an interactive Pivot Chart dashboard without relying on unsupported PivotTable generation.
* **Applicability**: When generating automated, self-contained dashboard reports. Ideal for timeseries and categorical summary reporting where Excel is used strictly as a presentation layer and data aggregation is handled upstream in Python.

### 2. Structural Breakdown

- **Data Layout**: Raw data is ingested, grouped, and written to a hidden `ChartData` worksheet. Each chart receives its own discrete 2D range on this hidden sheet.
- **Formula Logic**: Replaced by Python pre-calculation upstream to guarantee compatibility and eliminate manual PivotTable refreshes.
- **Visual Design**: The `Dashboard` sheet hides gridlines (`ws.sheet_view.showGridLines = False`) to create a canvas feel. A bold, solid-fill header strip spans the top to frame the charts.
- **Charts/Tables**: Utilizes a `BarChart` configured as a stacked column (`grouping="stacked"`, `overlap=100`) for categorical breakdowns, and `LineChart` objects with hidden legends (`legend = None`) for timeseries trends.
- **Theme Hooks**: The dashboard title strip consumes `header_bg` and `header_fg` to align with organizational branding.
