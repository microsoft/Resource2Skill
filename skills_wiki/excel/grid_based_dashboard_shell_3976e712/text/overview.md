...
* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a modern UI dashboard directly in the Excel grid by disabling gridlines, applying a muted background color to the canvas, and creating high-contrast white "cards" using `PatternFill` and thin cell borders. Charts and KPI values are anchored strictly inside these card regions.
* **Applicability**: Ideal for executive summaries, KPI trackers, or any front-page dashboard where a polished, application-like visual hierarchy is required without relying on floating, brittle Excel shapes.

### 2. Structural Breakdown

- **Data Layout**: Metric values and chart data are sourced to a hidden `Dashboard_Data` sheet to keep the primary dashboard canvas clean and restricted to pure visualization.
- **Formula Logic**: Aggregated values are pushed directly to merged anchor cells within the card bodies.
- **Visual Design**: Uses a "canvas" background (light gray) with "cards" (white). Borders use a thin gray `Side` on the outer edges of the card ranges to create separation, and a subtle separator line sits below each card title. Gridlines are hidden.
- **Charts/Tables**: Line Chart for trends and Bar Chart for categorical breakdown (serving as a map fallback), placed cleanly within card bounds using `ws.add_chart()`. 
- **Theme Hooks**: Utilizes a muted slate gray for canvas (`bg_ui`), white for cards (`bg_primary`), dark slate for primary headers (`text_main`), and bold blues for KPI callouts (`accent1`).
