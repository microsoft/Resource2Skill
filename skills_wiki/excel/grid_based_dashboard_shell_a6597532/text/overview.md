...
* **Tier**: sheet_shell
* **Core Mechanism**: Simulates a modern web-app UI inside Excel using edge-to-edge grid painting. It disables native gridlines, applies a full-width colored header, and constructs KPI "cards" and chart panels using merged cell regions, contrasting backgrounds, and simulated top-border accents. 
* **Applicability**: Best for executive summaries and KPI dashboards where a sleek, modern visual layout is required. By avoiding floating shapes (which are brittle to maintain via scripts), this grid-based approach ensures maximum compatibility and programmatic ease when injecting charts or tables later.

### 2. Structural Breakdown

- **Data Layout**: Accepts a dynamic configuration of `kpis` (dictionaries with `label` and `value`) and `panels` (dictionaries with `title` and a target `range` string like "B10:H25").
- **Formula Logic**: N/A (Focuses entirely on presentation layer shell).
- **Visual Design**: Uses a custom theme dictionary. A light gray/purple background creates depth, while white regions with thin gray borders and bold accent stripes act as "cards".
- **Charts/Tables**: Acts as a host container, providing clearly demarcated placeholder regions (`panels`) for subsequent chart/table insertion.
- **Theme Hooks**: Consumes `primary` (header), `accent` (KPI stripes/subtitle), `bg` (canvas), `card_bg` (panels), and `text_light` (labels).
