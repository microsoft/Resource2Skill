...
> **Skill Name**: Grid-Aligned KPI Card

* **Tier**: component
* **Core Mechanism**: Simulates the visual appearance of floating, grouped shape-based KPI cards (as shown in the tutorial) using a structured 2x2 cell block. Uses merged cells for the title, cell background colors to mimic shape fills, and distinct inner borders to mimic an overlapping percentage badge, avoiding the brittleness of actual shapes in Python automation.
* **Applicability**: Use when you need to build top-level executive dashboards and KPI highlight strips. Grid-aligned cards are far more robust for automated Excel generation than actual floating shapes, preventing layout breakage and rendering issues across different Excel versions.

### 2. Structural Breakdown

- **Data Layout**: A 2-row by 2-column block of cells. Row 1 is merged for the KPI title. Row 2 contains the primary value (left) and the percentage badge (right).
- **Formula Logic**: Accepts direct values or formula strings (e.g., `"=Sheet1!B2"`) to link the card to the data model dynamically.
- **Visual Design**: Uses a dark primary fill (e.g., Navy) for the card body, with white text. The percentage cell uses a slightly lighter accent color and a thin white border to simulate a distinct, floating "badge".
- **Charts/Tables**: N/A - pure grid styling.
- **Theme Hooks**: Uses the theme's primary color for the card background (`bg_color`) and a secondary/accent color for the percentage badge (`badge_bg`).
