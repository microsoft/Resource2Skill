...
> **Skill Name**: Custom Shape KPI

*   **Tier**: component
*   **Core Mechanism**: This skill constructs a key performance indicator (KPI) visual by combining and styling two distinct `openpyxl` shapes: a main rounded rectangle and a smaller oval. The shapes are filled with static text (representing what would typically be dynamic data) and formatted with custom fonts and colors derived from a theme, and then placed at an absolute position on the worksheet.
*   **Applicability**: Ideal for creating visually striking and custom-branded KPI displays in dashboards or reports where static visual representation is sufficient, or where dynamic updates are handled by other means (e.g., VBA). It can represent any categorical and quantitative data point (e.g., Region, Revenue, Market Share).

### 2. Structural Breakdown

-   **Data Layout**: Assumes input data for `region_name`, `revenue_value`, and `market_share_value` will be provided as parameters. The shapes themselves do not link to Excel cells directly for dynamic content via `openpyxl`'s high-level API.
-   **Formula Logic**: N/A (shapes contain static text, not formulas via `openpyxl`).
-   **Visual Design**:
    -   **Main Rectangle**: Rounded corners, filled with `theme.header_bg` color, no outline. Text includes `region_name`, "Revenue", and `revenue_value`. Text is white (`theme.header_text_color`), bold, with varying font sizes (18pt for `region_name`, 14pt for others).
    -   **Market Share Oval**: Elliptical shape, filled with `theme.neutral_bg` color, no outline. Text displays `market_share_value` as a percentage. Text is black (`theme.text_color`), bold, 14pt.
    -   **Positioning**: Shapes are absolutely positioned in EMUs (English Metric Units) on the worksheet, with their top-left corner relative to the specified `anchor` cell.
-   **Charts/Tables**: N/A (the component itself is a standalone visual).
-   **Theme Hooks**: `header_bg` for the main shape's fill, `header_text_color` for its text, `neutral_bg` for the oval's fill, and `text_color` for the oval's text.
