...
> **Skill Name**: Scenario-Driven Financial Schedule

* **Tier**: component
* **Core Mechanism**: Constructs an Income Statement timeline with distinct custom number formats for historical (`0"A"`) vs. forecast (`0"E"`) years. It separates the financial schedule from the assumptions, using an inline `CHOOSE` formula linked to a data-validated scenario toggle (1=Best, 2=Base, 3=Worst) to dynamically drive forecast projections. 
* **Applicability**: Essential for FP&A, budgeting, and 3-statement modeling. Ideal whenever you need to seamlessly project future financials under varying assumptions without hardcoding or creating multiple duplicate sheets.

### 2. Structural Breakdown

- **Data Layout**: Vertically stacked schedule. Top section contains the Income Statement line items. Bottom section contains the model assumptions. The timeline is laid out horizontally across columns. 
- **Formula Logic**:
  - Subtotals: Standard arithmetic (e.g., Gross Profit `=Revenue - COGS`).
  - Forecast active assumption: `=CHOOSE($Toggle_Cell, Best_Value, Base_Value, Worst_Value)`.
  - Projection: `=Previous_Year_Value * (1 + Active_Growth_Rate)` or `=Revenue * Margin_Pct`.
- **Visual Design**: Employs dark theme-driven headers with bold white text. Subtotal rows receive bold fonts and top/bottom bounding borders. The scenario toggle cell is highlighted (light yellow, red bold text) to indicate it's a primary user control.
- **Charts/Tables**: Utilizes Data Validation (`"1,2,3"`) to build an in-cell dropdown for the scenario toggle. Also utilizes row grouping (`ws.row_dimensions.group`) to allow collapsing of the assumptions block.
- **Theme Hooks**: Consumes `header_bg` and `header_fg` for the primary section labels.
