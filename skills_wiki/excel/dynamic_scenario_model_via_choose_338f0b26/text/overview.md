...
> **Skill Name**: Dynamic Scenario Model via CHOOSE

* **Tier**: sheet_shell
* **Core Mechanism**: Uses a Data Validation dropdown to toggle an active scenario ID. A central "Live Assumptions" block uses the `=CHOOSE()` function to pull data from separate hard-coded scenario blocks based on the toggle. The main financial statements then exclusively reference the Live Assumptions, creating a fully dynamic toggleable model without duplicating formula logic.
* **Applicability**: Ideal for financial modeling, budgeting, or forecasting where you need to quickly toggle between Base, Upside, and Downside cases on a single dashboard.

### 2. Structural Breakdown

- **Data Layout**: Scenario assumption blocks are stacked vertically below the main statement. All blocks share identical horizontal column structures (Years 1-3) to ensure formulas can be dragged seamlessly (`Ctrl + R`).
- **Formula Logic**: `=CHOOSE($E$2, C16, C21)` links the active integer toggle in E2 to specific assumption rows.
- **Visual Design**: Strict financial modeling color convention applied: Hard-coded inputs are colored blue (`0000FF`), while dynamic formulas remain black. The main input toggle cell is highlighted with a pale yellow background (`FFF2CC`) to indicate interactivity.
- **Charts/Tables**: Standard unstructured range layout (no formal Excel Tables used, which prevents unwanted array expansion when using dynamic references).
- **Theme Hooks**: Primary header background uses standard `1F4E78` (theme mapped), with hard-coded values utilizing a standard absolute blue.
