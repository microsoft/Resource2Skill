...
> **Skill Name**: Static KPI Dashboard Layout

* **Tier**: archetype
* **Core Mechanism**: Creates a presentation layer by hiding gridlines and adding a full-width themed banner. Routes raw or aggregated data to a hidden calculation sheet, which feeds multiple `openpyxl.chart` objects (Stacked Column, Line) arranged symmetrically on the presentation sheet.
* **Applicability**: Use for delivering clean, executive-style static dashboards. Replicates the visual layout of interactive pivot dashboards (from the video tutorial) in a pure-Python, dependency-free Excel file since `openpyxl` cannot natively construct active Slicers or PivotTables from scratch. 

### 2. Structural Breakdown

- **Data Layout**: Input data is decoupled from presentation. Aggregated data tables are generated sequentially on a hidden `CalcData` worksheet to keep the presentation canvas clean.
- **Formula Logic**: No direct Excel formulas; relies on Python dictionaries to stage pre-aggregated metrics into exact ranges for the charts.
- **Visual Design**: Leverages `ws.sheet_view.showGridLines = False` for a white-space canvas. Includes a merged top header row with centered, bold, themed text functioning as the dashboard banner.
- **Charts/Tables**: One Stacked Column chart for categorical segmentation comparisons (Profit by Market), and two Line charts for trend analysis stacked vertically. Legends are explicitly removed (`lc.legend = None`) on single-series trend charts for optimal spacing.
- **Theme Hooks**: Banner background relies on `theme["bg"]` (fallback to `#1F4E78`), banner text uses `theme["fg"]` (fallback to `#FFFFFF`).
