...
> **Skill Name**: Scenario-Driven Income Statement Model

* **Tier**: sheet_shell
* **Core Mechanism**: Builds a 5-year forecasted financial model separating assumptions from the P&L output. Uses Data Validation for a scenario toggle and the `CHOOSE` function to dynamically pipe values from different hardcoded scenario blocks (Base Case, Downside) into a "Live Case" that drives the financial statements.
* **Applicability**: Essential for financial forecasting, budgeting, and scenario analysis where users need to evaluate multiple business cases (e.g., Optimistic vs. Pessimistic) through a single presentation-ready financial statement.

### 2. Structural Breakdown

- **Data Layout**: Column A holds line items, Column B holds unit indicators ($ or %). Columns C-G represent forecast Years 1 through 5. The sheet is stacked vertically: Income Statement at the top, Live Case assumptions in the middle, and hardcoded Scenario assumptions at the bottom. A scenario toggle is positioned prominently at the top right.
- **Formula Logic**:
  - Scenario Routing: `=CHOOSE($J$2, Case1_Cell, Case2_Cell)` maps selected scenarios to the Live Case.
  - Forward Growth: `=Prev_Year * (1 + Growth_Rate)` cascades order volume.
  - Error-Handled Tax: `=IF(OpProfit<0, "NA", OpProfit*TaxRate)` combined with `=IFERROR(OpProfit-Tax, OpProfit)` calculates Net Income gracefully even with string errors.
- **Visual Design**: 
  - Standard financial modeling convention: Hardcoded inputs use a blue font (`#0000FF`), while formulas remain black.
  - Subtotals and bottom lines use top borders and double-bottom borders.
  - The interactive scenario toggle is highlighted with a yellow background.
- **Charts/Tables**: Clean structured range layout without native Excel Tables, preserving strict relative referencing needed for complex financial models.
- **Theme Hooks**: Primary palette color drives the main header row and categorical section dividers.
