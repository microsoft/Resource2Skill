...
> **Skill Name**: In-Cell KPI Card

* **Tier**: component
* **Core Mechanism**: Translates the concept of floating shape-based KPI cards into an automation-friendly cell grid. Uses merged cells, contrasting solid fills, and targeted alignment to create a unified visual "card" component without relying on fragile floating objects.
* **Applicability**: Dashboards and summary sheets where high-level metrics (e.g., Revenue and Market Share) need to be highlighted. Cell-based cards are highly preferable over floating shapes for automated reporting because they resize and position predictably.

### 2. Structural Breakdown

- **Data Layout**: A 3x3 cell grid starting at the anchor. Row 1 merges across all 3 columns for the title. Rows 2-3 split into a 2-column primary metric area (left) and a 1-column secondary metric area (right).
- **Formula Logic**: Static values are injected by default, but these can be easily swapped for formulas linking to a summary data table (e.g., `=SUM(Data!B:B)`).
- **Visual Design**: Uses a dark primary fill for the main card body and a contrasting bright background for the secondary metric callout. A border matching the primary color frames the bright accent box.
- **Charts/Tables**: N/A
- **Theme Hooks**: Utilizes `bg` and `fg` for the main card, and `accent_bg` / `accent_fg` for the callout metric box.
