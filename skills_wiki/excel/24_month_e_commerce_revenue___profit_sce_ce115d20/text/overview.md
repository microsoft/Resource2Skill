...
> **Skill Name**: 24-Month E-Commerce Revenue & Profit Scenario Model

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a complete, dynamic 24-month financial planning model across two interconnected Excel worksheets: an 'Assumptions' tab for all changeable input variables and a 'Model' tab for monthly calculations, gross profit derivation, marketing spend, contribution margin, and a sanity check. It ensures all calculations in the 'Model' tab dynamically reference the 'Assumptions' tab, allowing for robust scenario analysis with a single input change.
*   **Applicability**: This skill is ideal for finance professionals, business analysts, or e-commerce managers who need to create flexible and data-driven revenue and profitability forecasts. It's particularly useful for businesses with multiple product lines, requiring detailed monthly projections, and the ability to simulate changes to key operational and financial assumptions to assess their impact on overall performance.

### 2. Structural Breakdown

-   **Data Layout**:
    -   **Assumptions Tab**: Organizes key input variables into distinct sections: 'ACCESSORIES', 'DEVICES', and 'GENERAL'. Each variable (e.g., Average Order Value, Monthly Order Growth Rate, Gross Margin, Marketing Spend %) is clearly labeled in column A, with its value in column B.
    -   **Model Tab**: Features 24 columns representing months. Rows are grouped into 'ACCESSORIES', 'DEVICES', 'COMBINED TOTALS', and 'SANITY CHECK' sections. Each section contains relevant metrics such as Orders, Revenue, Gross Profit, Total Revenue, Blended Gross Margin, Marketing Spend, and Contribution Margin. A separate section below contains calculated data for the accompanying chart.
-   **Formula Logic**:
    -   **Growth Calculation**: Monthly orders grow based on previous month's orders and monthly growth rates, referenced from the 'Assumptions' tab. (`={Previous Month Orders}*(1+{Monthly Order Growth Rate})`)
    -   **Revenue**: Calculated as `Orders * Average Order Value`, where Average Order Value is referenced from 'Assumptions'.
    -   **Gross Profit**: `Revenue * Gross Margin (%)`, with Gross Margin referenced from 'Assumptions'.
    -   **Combined Totals**: Sums individual product line revenues and gross profits. Calculates 'Blended Gross Margin (%)' as `Total Gross Profit / Total Revenue`.
    -   **Marketing Spend**: 'Marketing Spend ($)' is calculated as `Total Revenue * Marketing Spend (% of Total Revenue)`, with the percentage pulled from 'Assumptions'.
    -   **Contribution Margin**: `Total Gross Profit - Marketing Spend ($)`.
    -   **Sanity Check**: Explicitly verifies `(Accessories Revenue + Devices Revenue) == Total Revenue`.
-   **Visual Design**:
    -   Sheet titles are bold and larger.
    -   Section headers (e.g., 'ACCESSORIES', 'COMBINED TOTALS') are bold.
    -   Input cells in the 'Assumptions' tab have a distinct background fill and font color (e.g., light blue background, blue font) to visually distinguish inputs.
    -   Monetary values are formatted as currency, percentages as percentages, and order counts as integers.
    -   'Sanity Check' row uses conditional formatting to display "Match" in green and "Mismatch" in red.
    -   All main data areas in the 'Model' tab have thin borders.
-   **Charts/Tables**:
    -   A line chart titled "Monthly Revenue & Contribution Margin by Product Line" is included in the 'Model' tab.
    -   It displays three series: 'Accessories Revenue', 'Devices Revenue', and 'Contribution Margin' over the 24-month forecast period.
    -   Chart series are distinctly colored (e.g., blue for accessories, orange for devices, green for contribution margin).
    -   X-axis is labeled "Month", Y-axis is labeled "Amount ($)".
-   **Theme Hooks**: While explicit theme hooks (e.g., `theme.header_bg`) are not directly implemented with external helpers in this self-contained example, the code uses named color variables (`HEADER_BG_COLOR`, `INPUT_CELL_BG_COLOR`, `MATCH_COLOR`, etc.) that serve as clear points for future integration with a dynamic theme palette.
