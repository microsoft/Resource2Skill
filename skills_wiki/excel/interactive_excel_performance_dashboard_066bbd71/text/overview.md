...
> **Skill Name**: Interactive Excel Performance Dashboard

*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a fully interactive, multi-sheet dashboard within an Excel workbook. It takes raw tabular data, converts it into an Excel Table, generates multiple PivotTables and PivotCharts from this data, and then integrates Slicers and Timelines to provide dynamic filtering capabilities across all charts. The dashboard's appearance is refined by hiding gridlines and headings and by leveraging Excel's theme engine.
*   **Applicability**: This is ideal for creating robust, self-updating business intelligence dashboards from structured datasets (e.g., sales, operations, financial data) where users need to explore data dynamically without complex VBA or external add-ins. It's suitable for regular reporting and sharing insights within an organization, allowing for quick refreshes as new data becomes available.

### 2. Structural Breakdown

-   **Data Layout**:
    -   **Raw Data**: A sheet named "Data" containing structured, tabular data with headers: 'Country', 'Product', 'Units Sold', 'Revenue', 'Cost', 'Profit', 'Date'. This data is converted into an Excel Table (e.g., "SalesData").
    -   **PivotTable Sheets**: Multiple hidden sheets (e.g., "Profit by market and cookie", "Units sold each month", "Profit by month") each containing a single PivotTable linked to the main data table.
    -   **Dashboard Sheet**: A main sheet named "Dashboard" which displays the PivotCharts and interactive slicers.
-   **Formula Logic**: No direct formulas are explicitly shown as key logic, but PivotTables and PivotCharts automatically summarize and aggregate data based on selections. Excel automatically groups dates by month when 'Date' is added as a row field to a PivotTable.
-   **Visual Design**:
    -   **Header**: A merged cell region at the top of the "Dashboard" sheet for the company logo and "Performance Dashboard" title (e.g., dark blue background, white text).
    -   **Gridlines & Headings**: Turned off on the "Dashboard" sheet (`ws.sheet_view.showGridLines = False`, `ws.sheet_view.showRowColHeaders = False`).
    -   **Themes**: Excel themes (via "Page Layout > Themes") are used to apply a consistent color scheme and font style across the entire dashboard.
-   **Charts/Tables**:
    -   **Data Table**: Raw data is converted to an Excel Table (`Table(displayName="SalesData", ref="A1:G<last_row>")`) for automatic range expansion and PivotTable source.
    -   **PivotTables**:
        -   "Profit by Market & Cookie Type": 'Country' in rows, 'Product' in columns, SUM of 'Profit' in values. Values formatted as currency.
        -   "Units sold each month": 'Date' (grouped by month by Excel) in rows, SUM of 'Units Sold' in values. Values formatted with commas.
        -   "Profit by month": 'Date' (grouped by month by Excel) in rows, SUM of 'Profit' in values. Values formatted as currency.
    -   **PivotCharts**:
        -   "Profit by Market & Cookie Type": Stacked Column chart from corresponding PivotTable. Chart title is "Profit by Market & Cookie Type".
        -   "Units sold each month": Line chart from corresponding PivotTable. Chart title is "Units sold each month".
        -   "Profit by month": Line chart from corresponding PivotTable. Chart title is "Profit by month".
    -   **Slicers**: (Note: `openpyxl` does not directly support Slicers/Timelines or their connections via API. This functionality would be set up manually in Excel after workbook generation.)
        -   Timeline slicer for 'Date'.
        -   Slicer for 'Country'.
        -   Slicer for 'Product'.
        -   Slicers are linked to all PivotTables via "Report Connections" in Excel. Headers are removed and sized to fit content.
-   **Theme Hooks**: `header_bg`, `header_fg`, `accent_colors` (for chart series and slicer styles), `text_color`, `background_color`.
