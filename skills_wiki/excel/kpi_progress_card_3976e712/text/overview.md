...
> **Skill Name**: KPI Progress Card

* **Tier**: component
* **Core Mechanism**: Constructs a bounded 4x4 cell region to simulate a UI "card". It merges specific regions for a title and main value readout, then overlays a Doughnut chart formatted to look like a progress ring (65% hole size, primary color vs gray). Chart data is written into the cells directly beneath the chart area and hidden via font color matching.
* **Applicability**: Best used for executive dashboard summaries where you need to display an actual metric alongside progress toward a target (e.g., Sales vs Quota, Project Completion, Customer Satisfaction). It provides an elegant, modular visual block without relying on brittle floating shapes.

### 2. Structural Breakdown

- **Data Layout**: Consumes a 4-column by 4-row block. The progress percentages (Complete vs Remaining) are calculated in-memory and injected into the background cells behind the chart area to act as chart data sources.
- **Formula Logic**: Calculates completion ratio bounded to 1.0 (`min(actual / target, 1.0)`).
- **Visual Design**: Uses a white background fill with a thin gray perimeter border (`Side(style="thin")`) to create a "card" effect. Text is aligned and scaled hierarchically (Title size 11, Main Value size 20, Subtext size 9).
- **Charts/Tables**: `DoughnutChart` sized compactly (3.5cm x 2.5cm) to fit the right half of the card. Slice colors are explicitly overridden using `DataPoint` to match the theme color and remainder gray.
- **Theme Hooks**: Primary font and progress chart slice utilize the theme's core brand color (`corporate_blue` mapping to `203864`). 
