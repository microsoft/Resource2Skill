...
> **Skill Name**: Navigational Dashboard Workbook

* **Tier**: archetype
* **Core Mechanism**: Builds a multi-sheet workbook where the first column of every sheet acts as a fixed "web-app style" navigation sidebar. Uses in-cell hyperlinks to seamlessly switch between 'Dashboard', 'Inputs', and 'Contacts' tabs, keeping presentation charts cleanly separated from raw data tables. 
* **Applicability**: Perfect for complex financial models, management reports, or interactive dashboards where users need to navigate between a primary graphical summary and supporting data tabs without hunting through sheet tabs at the bottom.

### 2. Structural Breakdown

- **Data Layout**: Segregates data into three structural tabs: `Dashboard` (presentation), `Inputs` (raw data/tables for charts), and `Contacts` (reference data).
- **Formula Logic**: Uses standard Excel internal hyperlinks (`#'SheetName'!A1`) to bind sidebar cells to specific worksheet anchors.
- **Visual Design**: The first column (Column A) is widened and filled with a dark theme color across all sheets to simulate a fixed sidebar. "Active" or clickable cells are styled with bold, contrasting text and centered alignment.
- **Charts/Tables**: Implements a Radar Chart (for multivariate metrics like Customer Satisfaction) and a Line Chart (for time-series trends), placed directly onto the Dashboard sheet but referencing the `Inputs` sheet.
- **Theme Hooks**: Consumes `sidebar_bg`, `sidebar_fg`, and `accent` to color the navigation pane and style the KPI card boundaries.
