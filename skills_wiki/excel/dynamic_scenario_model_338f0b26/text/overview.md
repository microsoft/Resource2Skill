...
> **Skill Name**: Dynamic Scenario Model

* **Tier**: sheet_shell
* **Core Mechanism**: Drives financial projections by using a Data Validation dropdown and the `CHOOSE` function to pipe alternate sets of hard-coded assumptions into a centralized "Live Case" block. The output tables (like an Income Statement) strictly reference this Live Case, enabling immediate, whole-model scenario toggling.
* **Applicability**: Best used for financial statements, business case modeling, or multi-scenario forecasting where users need to switch between discrete assumptions (e.g., Base, Upside, Downside) dynamically.

### 2. Structural Breakdown

- **Data Layout**: Divided horizontally into calculation and input zones. Top section for the output (Income Statement), middle section for the active projection ("Live Case"), and stacked blocks at the bottom for distinct scenario inputs.
- **Formula Logic**: `=CHOOSE($G$1, <Scenario1_Cell>, <Scenario2_Cell>)` links the chosen scenario up to the Live block. Top-line calculations like Revenue simply multiply rows within the Live block (`=Orders * AOV`).
- **Visual Design**: Enforces classic financial modeling conventions: hardcoded input cells use a blue font color, while formulas and derived values remain black. The scenario toggle cell uses an accent fill (yellow) to indicate user interactivity.
- **Charts/Tables**: Clean structured ranges with consistent number formatting (`_($* #,##0_)` for accounting, `0%` for rates).
- **Theme Hooks**: Header backgrounds consume `header_bg` with `header_fg` text. The scenario toggle consumes the `accent` color.
