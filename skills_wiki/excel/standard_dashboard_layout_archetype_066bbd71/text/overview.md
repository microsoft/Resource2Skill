...
> **Skill Name**: Standard Dashboard Layout Archetype

* **Tier**: archetype
* **Core Mechanism**: Generates a presentation-grade dashboard sheet separated from the raw data sheet. It turns off gridlines, creates a prominent merged header banner, and positions a left-hand sidebar for interactive controls (slicers). Multiple charts (stacked column and line) are arranged neatly in the main content area.
* **Applicability**: Ideal for executive summaries, KPI tracking, and turning tabular raw data into an organized, visual "app-like" experience.

### 2. Structural Breakdown

- **Data Layout**: Separates concerns by keeping a hidden "Dashboard Data" sheet for chart sources and a clean "Dashboard" sheet for the presentation layer.
- **Formula Logic**: Driven by Pivot Tables/Summaries in the background (mocked as static data ranges in `openpyxl` since slicer/pivot caching is restricted).
- **Visual Design**: Disables worksheet gridlines (`showGridLines = False`). Uses a deep theme color for a large, merged top banner (A1:Q4) with bold, centered white text.
- **Charts/Tables**: Includes a Stacked Column chart (`type="col", grouping="stacked"`) for category breakdowns, and a Line chart for temporal trends. Both are positioned using specific cell anchors (e.g., `D6`, `K6`) to establish a clean grid flow.
- **Theme Hooks**: Consumes `header_bg` and `header_fg` for the main banner color, defaulting to a dark corporate blue if unspecified.
