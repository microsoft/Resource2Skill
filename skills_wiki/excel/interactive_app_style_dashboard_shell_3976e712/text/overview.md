...
> **Skill Name**: Interactive App-Style Dashboard Shell

* **Tier**: archetype
* **Core Mechanism**: Builds a multi-sheet interactive application structure featuring a persistent left-hand sidebar. Uses custom cell backgrounds, outer borders, and internal sheet hyperlinks to simulate a modern web application UI with "cards" and page navigation.
* **Applicability**: Best for multi-page reports, interactive dashboards, or tools where users need to navigate smoothly between a high-level visual summary and detailed data input/settings sheets. 

### 2. Structural Breakdown

- **Data Layout**: Multi-sheet setup ("Dashboard", "Inputs", "Contacts").
- **Formula Logic**: Internal cell references prefixed with `#` to create in-workbook hyperlinks (e.g., `#'Inputs'!A1`).
- **Visual Design**: Hides default gridlines. Uses a two-tone layout: a dark slate sidebar (Column A) and a light gray body (Columns B+). Highlights the active menu item by matching the background to the sidebar text color.
- **Charts/Tables**: Replaces native Shapes (which aren't fully supported via programmatic APIs) with pure cell-based UI "Cards" created by mapping a white background and thin outer borders to coordinate grids.
- **Theme Hooks**: Uses a clean, modern palette (slate-800 for sidebar, slate-100 for active text, gray-100 for background).
