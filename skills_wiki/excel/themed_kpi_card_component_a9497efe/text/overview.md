...
* **Tier**: component
* **Core Mechanism**: Builds a modular 3-row by 4-column stylized KPI block. Row 1 is a merged title with theme colors. Row 2 is a large-font main metric. Row 3 contains target and prior-period comparisons. Applies conditional formatting rules (`CellIsRule`) to the main metric to color it green/red dynamically based on the target cell.
* **Applicability**: Perfect for executive dashboards, scorecard sheets, or summary tabs where key metrics need immediate visual impact (good vs. bad) alongside context (target/prior period).

### 2. Structural Breakdown

- **Data Layout**: 4 columns wide, 3 rows tall. Row 1 (Title) and Row 2 (Main Value) are merged horizontally across the 4 columns. Row 3 is split into 4 individual cells for contextual labels and values.
- **Formula Logic**: Uses absolute cell references (e.g., `$C$4`) inside the conditional formatting rule to compare the Main Value cell against the Target Value cell dynamically.
- **Visual Design**: Uses a solid theme accent color for the title header. The sub-metrics use a light gray background for contrast. The main metric uses a very large font (size 24) and receives dynamic solid background fills (Light Green / Light Red) via conditional formatting. 
- **Charts/Tables**: N/A (Pure cell-based UI component).
- **Theme Hooks**: Consumes `accent` for the header background, `text` for header text, and relies on standard traffic-light colors (Green/Red) for the performance indicators.
