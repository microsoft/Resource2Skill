...
> **Skill Name**: Dynamic 3-Tier KPI Dashboard

* **Tier**: archetype
* **Core Mechanism**: Separates report architecture into three distinct layers (Data, Staging, Dashboard). The `Data` sheet holds raw time-series metrics. The `Staging` sheet uses `INDEX/MATCH` to dynamically pull the selected month's values and prior month's values based on a single dropdown. The `Dashboard` sheet acts purely as a presentation layer, utilizing merged cells, large typography, and conditional formatting to visualize the staged data.
* **Applicability**: Best used for executive summaries and monthly metric reporting where stakeholders need to toggle between different reporting periods quickly without altering the underlying datasets.

### 2. Structural Breakdown

- **Data Layout**: 
  - `Data`: Columns for categories, targets, and chronological months.
  - `Staging`: Rows for each metric; columns for current period, target, and prior period.
- **Formula Logic**: Uses dynamic two-dimensional lookups: `=INDEX(Data!$E$2:$Z$10, MATCH(A2, Data!$B$2:$B$10, 0), MATCH(Dashboard!$C$4, Data!$E$1:$Z$1, 0))` to sync the dashboard dropdown to the raw data.
- **Visual Design**: 
  - Grey background (`EAEAEA`) on the dashboard canvas to make white KPI cards "pop".
  - Hidden gridlines (`showGridLines = False`).
  - Conditional formatting (green/red) applied strictly on the Dashboard layer using `CellIsRule` tied to staging targets.
- **Charts/Tables**: Layout grid composed of 4-column wide blocks; top row for themed header, middle row for large KPI value, bottom row for benchmarking context (Target vs Prior).
- **Theme Hooks**: Uses `primary` for the main title and `secondary` for the individual KPI card headers.
