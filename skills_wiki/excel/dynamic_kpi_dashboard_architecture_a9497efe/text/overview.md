...
> **Skill Name**: Dynamic KPI Dashboard Architecture

* **Tier**: archetype
* **Core Mechanism**: Separates dashboard logic into three distinct layers: Raw Data, calculated Staging, and Presentation. Uses Data Validation for period selection and dynamic `INDEX/MATCH` formulas to automatically roll forward KPI cards without manual layout updates.
* **Applicability**: Ideal for executive reporting, monthly metric reviews, and scalable dashboards where underlying data grows over time but the visual footprint remains fixed and focused.

### 2. Structural Breakdown

- **Data Layout**: 
  - **Data Tab**: Raw transactional metrics (Rows = Metrics, Columns = Months).
  - **Staging Tab**: Computed KPIs, Targets, and Prior Period lookbacks.
  - **Dashboard Tab**: The visual front-end with a master control dropdown.
- **Formula Logic**: 
  - Staging formulas normalize metrics (e.g., `=(Data!B3/Data!B2)*30`).
  - Presentation layer uses `=INDEX(Staging!$B$2:$G$2, 1, MATCH($C$5, Staging!$B$1:$G$1, 0))` to bridge the selected month to the staging row.
- **Visual Design**: Themed categorical headers, large focal KPI fonts, and subtle sub-metric text. A neutral background makes the white KPI cards pop.
- **Charts/Tables**: Employs structural "cards" using merged cells and outer borders.
- **Theme Hooks**: Consumes `primary` for main headers, `accent` for categorical strips, `card_header` for KPI titles, and standard `good`/`bad` palettes for threshold formatting.
