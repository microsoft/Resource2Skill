...
> **Skill Name**: 3-Panel Performance Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Configures a presentation-ready worksheet by disabling gridlines, applying a bold top header, writing aggregated data to a hidden off-screen range, and instantiating three perfectly aligned charts (a primary categorical stacked bar, and two secondary time-series line charts) to emulate an interactive dashboard. 
* **Applicability**: Ideal for executive summaries and performance reporting where you need to display categorical breakdowns (e.g., profit by region/product) alongside trend data (e.g., monthly volume and revenue) in a clean, single-screen layout.

### 2. Structural Breakdown

- **Data Layout**: Raw aggregate data is written starting at column `AA` (which is then hidden) to prevent cluttering the visual dashboard area.
- **Visual Design**: Gridlines are disabled (`ws.sheet_view.showGridLines = False`). The top two rows are merged to create a prominent, themed header bar.
- **Charts/Tables**: 
  - **Chart 1 (Primary)**: Stacked Column chart placed on the left, covering a larger area to show multi-category composition.
  - **Charts 2 & 3 (Secondary)**: Line charts stacked vertically on the right to show time-series trends. Legends are removed to maximize the plot area, mirroring the video's aesthetic cleanup.
- **Theme Hooks**: The header background consumes the primary theme color (falling back to a corporate blue). Excel's native chart objects automatically map to the workbook's active XML theme palette.
