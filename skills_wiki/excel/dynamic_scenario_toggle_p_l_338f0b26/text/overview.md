...
> **Skill Name**: Dynamic Scenario Toggle P&L

* **Tier**: sheet_shell
* **Core Mechanism**: Drives a central "Live Assumptions" block by passing a control cell value into a `CHOOSE()` formula to dynamically pull from one of several hardcoded scenario blocks (e.g., Base, Downside, Upside). The top-level P&L relies exclusively on the Live Assumptions block, allowing the entire model to toggle instantly via Data Validation dropdown.
* **Applicability**: Essential for financial modeling, forecasting, and what-if analysis. Use when you have a defined set of outputs (like an Income Statement) and multiple discrete sets of inputs (scenarios) that need to be compared seamlessly without duplicating the calculation logic.

### 2. Structural Breakdown

- **Data Layout**: Organized vertically. Top: Income Statement (Rows 4-9). Middle: Live Assumptions driven by formulas (Rows 11-15). Bottom: Hardcoded Scenario blocks (Rows 17-33). Column F holds the scenario toggle dropdown.
- **Formula Logic**: Live inputs use `=CHOOSE($H$2, B18, B24, B30)`. The P&L computes derived metrics strictly from the live block (e.g., Revenue `=B12*B13`).
- **Visual Design**: Uses typical financial modeling conventions — hardcoded inputs are colored blue (`0000FF`), formulas remain black. Section headers are distinctly filled. The control dropdown is highlighted in yellow.
- **Charts/Tables**: Standard spreadsheet grid (no `Table` objects, as they interfere with free-form financial model structures).
- **Theme Hooks**: Consumes `header_bg` and `header_fg` for block separators to maintain a clean, readable hierarchy.
