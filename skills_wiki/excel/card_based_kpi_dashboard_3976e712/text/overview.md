...
> **Skill Name**: Card-Based KPI Dashboard

* **Tier**: archetype
* **Core Mechanism**: Generates a two-sheet architecture (`Inputs` for data, `Dashboard` for presentation). Uses targeted cell fills and borders over a contrasted canvas to simulate floating UI "cards". Embeds transparent charts (Doughnut, Line, Bar) directly over these cards to create a modern, web-like dashboard.
* **Applicability**: Perfect for executive summaries, KPI tracking, and standardizing tabular data outputs into a clean, presentation-ready visual format.

### 2. Structural Breakdown

- **Data Layout**: Two sheets. `Inputs` acts as the data layer containing raw structured tables (KPIs, Trends, Categories). `Dashboard` acts as the view layer.
- **Formula Logic**: Isolates presentation from data by projecting `openpyxl.chart.Reference` ranges from the `Inputs` tab onto the `Dashboard` tab.
- **Visual Design**: Uses a solid sidebar (Column A) for structural anchoring. Hides gridlines completely. Leverages a two-tone color strategy: a light grey background (`F3F4F6`) globally, with white (`FFFFFF`) rectangular ranges defined by thin grey borders to create the "card" containers.
- **Charts/Tables**: Employs `DoughnutChart` for KPI completion rings, `LineChart` for temporal trends, and `BarChart` for categorical distribution. All charts have `graphical_properties.noFill = True` to blend seamlessly into the white cards.
- **Theme Hooks**: Consumes `bg`, `card`, `sidebar`, `text`, `border`, and `accents` to drive the entire aesthetic.
