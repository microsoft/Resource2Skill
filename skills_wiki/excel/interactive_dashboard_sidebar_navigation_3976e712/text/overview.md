...
> **Skill Name**: Interactive Dashboard Sidebar Navigation

* **Tier**: archetype
* **Core Mechanism**: Establishes a multi-sheet interactive dashboard framework by creating a persistent, colored sidebar column across multiple worksheets. It utilizes internal Excel `HYPERLINK` references attached to text icons, allowing users to seamlessly click between "views" (e.g., Dashboard, Inputs, Contacts) to simulate a modern web-app navigation experience. 
* **Applicability**: Ideal for complex reporting workbooks, financial models, or automated dashboards that require separate tabs for front-end visuals and back-end data. It replaces standard Excel sheet-tab clicking with an intuitive, guided UI.

### 2. Structural Breakdown

- **Data Layout**: Column A is reserved entirely as the navigation sidebar (width 8). Columns C through Z form the clean, gridless canvas for placing dashboard widgets, tables, or charts. Rows 2-3 are designated for dynamic page headers.
- **Formula Logic**: Uses Excel's internal location syntax `#'SheetName'!A1` applied to the `.hyperlink` property to route clicks seamlessly between different worksheet tabs.
- **Visual Design**: The sidebar utilizes a persistent dark column fill. Emoji characters (🏠, 📊) act as icon buttons, formatted with a large, high-contrast font. The "active" tab state is highlighted by a slightly lighter fill color behind the current icon, reinforcing user context. Global gridlines are disabled.
- **Charts/Tables**: This archetype sets up the structural shell; charts and tables would be subsequently injected into the main canvas area (Column C+).
- **Theme Hooks**: Consumes `sidebar_bg` (dark primary color) and `sidebar_active` (lighter variant for selection highlight), along with `text_light` for the icon font contrast.
