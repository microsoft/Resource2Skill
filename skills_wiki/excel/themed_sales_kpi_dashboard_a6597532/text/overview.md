...
> **Skill Name**: Themed Sales KPI Dashboard

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a full-sheet presentation layer by painting the grid with zone colors (header/canvas) and building KPI cards via merged cells with accent fills. Integrates a metrics table driven by in-cell DataBars and a custom overlay column chart (100% series overlap) for metric comparison.
* **Applicability**: Best for executive summaries or performance dashboards where key metrics need to be highlighted in a visually separated "card" layout above detailed tabular data and trend charts.

### 2. Structural Breakdown

- **Data Layout**: Hidden trend data is written to columns `AA:AD` to act as the clean data source for charts, while the visible layout uses columns `B:P`.
- **Formula Logic**: Relies on direct cell values for rendering; in production, these would be `GETPIVOTDATA` or sum formulas pointing to raw data sheets.
- **Visual Design**: Uses a cohesive palette. The canvas is filled with a light background, overriding the default gridlines. Header block gets a deep primary color.
- **Charts/Tables**: Employs `DataBarRule` for in-cell bar charts on the agent table. Features an Overlay Column Chart achieved by setting `overlap=100` and `gapWidth=50`.
- **Theme Hooks**: `header_bg`, `canvas_bg`, `card_bg`, `accent_yellow`, `accent_purple`, `text_light`, `text_dark`, `text_muted`.
