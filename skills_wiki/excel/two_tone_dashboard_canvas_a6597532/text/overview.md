...
> **Skill Name**: Two-Tone Dashboard Canvas

* **Tier**: sheet_shell
* **Core Mechanism**: Establishes a sleek dashboard canvas by hiding gridlines and using contrasting fill colors to create a distinct header region (dark) and body region (light). KPI cards are constructed from grouped, merged cell ranges using a white fill and an accent-colored side strip, simulating modern UI shapes without relying on brittle floating objects.
* **Applicability**: Best for executive summaries and overview dashboards where visual polish is important. Using a finer cell grid (narrow columns) allows precise placement of KPI cards and chart anchors without the alignment issues common with Excel shapes.

### 2. Structural Breakdown

- **Data Layout**: Resizes columns 1-30 to a narrow uniform width (4.0) to act as a layout grid.
- **Formula Logic**: Static values injected into the layout; intended to be updated via Python or linked to analysis sheets.
- **Visual Design**: Dark purple header spanning rows 1-8, light background for rows 9+. Hides gridlines for a clean canvas.
- **Charts/Tables**: KPI "cards" are built from 6x4 cell blocks with outer thin borders, a left accent strip, and merged areas for centered values and labels.
- **Theme Hooks**: Consumes `header_bg` (primary), `body_bg` (primary_light), `accent`, and `text_light`.
