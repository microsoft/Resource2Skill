...
> **Skill Name**: Dynamic Scenario Model Toggle

* **Tier**: sheet_shell
* **Core Mechanism**: Uses Data Validation to create a scenario selector and the `CHOOSE` function to dynamically pull assumption values into a "Live Case" block. The financial statement formulas are built strictly off the Live Case, allowing users to toggle between scenarios instantly without duplicating calculation logic.
* **Applicability**: Essential for financial modeling, budgeting, and forecasting where multiple business scenarios (Base, Best, Worst) need to be toggled for variance analysis.

### 2. Structural Breakdown

- **Data Layout**: Income Statement calculations at the top, followed by a dynamically linked "Live Case" assumptions block, and finally the static/hard-coded scenario blocks below it.
- **Formula Logic**: `=CHOOSE($I$2, scenario1_ref, scenario2_ref)` maps the active scenario index from the toggle dropdown to the corresponding input block.
- **Visual Design**: Hard-coded assumptions are colored blue (a financial modeling standard), while dynamic references and calculations remain black.
- **Charts/Tables**: N/A
- **Theme Hooks**: Employs the `theme` dictionary's primary color for section header fills and the accent color to highlight the scenario toggle cell.
