...
> **Skill Name**: Static Performance Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a pristine dashboard interface by hiding spreadsheet gridlines, drawing a thematic full-width title banner, and orchestrating multiple pre-aggregated data charts (a stacked column and two line charts) in an aligned grid layout.
* **Applicability**: Ideal for executive summaries, static performance reports, and high-level KPIs where clean visual presentation matters more than raw data exploration. This pattern separates the data layer (hidden sheet) from the presentation layer.

### 2. Structural Breakdown

- **Data Layout**: Places aggregated chart data on a secondary hidden sheet (`{sheet_name}_Data`) to keep the main dashboard sheet clear of raw tables.
- **Formula Logic**: None (relies on pre-aggregated values acting as a stand-in for PivotTables).
- **Visual Design**: Hides gridlines (`ws.sheet_view.showGridLines = False`); creates a strong header banner (rows 1-3) using merged cells, primary theme colors, and contrasting aligned text.
- **Charts/Tables**: One Stacked Column chart (`overlap=100`, `grouping="stacked"`) and two Line charts placed at specific cell anchors (`B5`, `I5`, `I16`) to form a structured layout.
- **Theme Hooks**: Uses the `primary` hex for the banner background and `text_light` for the title font.
