...
> **Skill Name**: Cell-Grid Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Translates a typical floating-shape dashboard layout into a robust programmatic cell grid. It hides gridlines, sets a global canvas background color, and constructs discrete UI "panels" (cards) using white cell fills and thin outer borders.
* **Applicability**: Essential foundational shell for programmatic reports. Avoids the complexity and brittleness of manipulating VML/drawing shapes via Python by simulating a modern, clean card-based UI directly through native cell formatting.

### 2. Structural Breakdown

- **Data Layout**: Asymmetrical grid layout. Sidebar on Column A. Main content spans Columns B through M. Left block contains KPIs and trend charts; tall right block reserved for a map or list.
- **Formula Logic**: Purely layout and UI architecture.
- **Visual Design**: 
  - Canvas: `F3F3F3` fill, hidden gridlines.
  - Panels: `FFFFFF` fill, `D9D9D9` thin outer borders.
  - Typography: Soft gray (`595959`) for panel titles, bolded, using native cell indentation for padding instead of spaces.
- **Charts/Tables**: Creates precisely dimensioned empty placeholder panels ready for chart objects to be overlaid.
- **Theme Hooks**: Consumes `sidebar_color` (maps to the primary theme accent) and `bg_color` for the main canvas.
