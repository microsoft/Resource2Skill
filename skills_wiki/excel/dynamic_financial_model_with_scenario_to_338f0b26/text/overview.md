...
> **Skill Name**: Dynamic Financial Model with Scenario Toggle

* **Tier**: sheet_shell
* **Core Mechanism**: Stacks identical parameter blocks for a "Live Case" and multiple scenario states. Maps the live block's cells to the scenario blocks using the `CHOOSE()` function tied to a Data Validation dropdown. The top-level Income Statement references only the Live Case block, enabling a zero-VBA scenario toggle.
* **Applicability**: Best for financial models, capacity planning, and business forecasting where multiple predefined parameter states (Base, Bull, Bear) need to flow through a single reporting structure without duplicating the complex calculation logic.

### 2. Structural Breakdown

- **Data Layout**: 
  - Rows 4-13: Dynamic Income Statement.
  - Rows 15-21: Live Case Assumptions (formula-driven).
  - Rows 23-29: Scenario 1 / Base Case (hardcoded/calculated).
  - Rows 31-37: Scenario 2 / Downside Case (hardcoded/calculated).
  - Top-right (J2): Data Validation toggle cell.
- **Formula Logic**: 
  - `CHOOSE($J$2, C24, C32)` loops across the live assumptions block to pull parameters dynamically based on the active scenario.
  - `#VALUE` avoidance: Uses `=IF(C10<0, "NA", C10*C21)` for taxes (to show NA for losses), and wraps net income in `=IFERROR(OpProfit-Tax, OpProfit)` to safely subtract text values.
- **Visual Design**: Block headers use `accent_bg`, column headers use `header_bg`. Key profit metrics are bolded with top/bottom borders. The active toggle cell is highlighted with bright yellow fill.
- **Charts/Tables**: N/A - driven heavily by structured accounting formatting (accounting formats, text NA handling).
- **Theme Hooks**: `header_bg`, `header_fg`, `accent_bg`, `accent_fg`, `text_primary`.
