...
> **Skill Name**: Themed Dashboard Canvas with Cell-based KPI Cards

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a visually separated dashboard canvas using continuous row fills (banded header vs. body). Instead of using brittle floating shapes, it builds robust KPI cards by merging 3x3 cell blocks, filling the left column with an accent color for a mock "ribbon" effect, and layering large fonts for values. Finally, it applies `DataBarRule` conditional formatting to the data table to create in-cell bar charts.
* **Applicability**: Ideal for executive summaries, sales dashboards, or high-level report landing pages. The cell-based KPI cards guarantee cross-platform compatibility and precise alignment without dealing with Excel shape positioning logic.

### 2. Structural Breakdown

- **Data Layout**: 
  - Header band (Rows 1-8): Houses the dashboard title and subtitle.
  - KPI Cards (Rows 5-7): Each card spans 3 columns and 3 rows. The first column holds the icon, the next two hold the value and label.
  - Data Table (Row 10+): Displays detailed metrics underneath the cards.
- **Formula Logic**: Purely layout and formatting-driven; intended to be populated with aggregated data directly.
- **Visual Design**: Uses a custom theme palette to separate the header (`primary`) from the body (`primary_light`). KPI cards use `accent` for icons and white for text areas. Hides sheet gridlines to emphasize the "canvas" feel.
- **Charts/Tables**: Instead of floating charts, leverages Conditional Formatting Data Bars (`DataBarRule`) directly in the detail table to visualize relative performance across rows.
- **Theme Hooks**: Consumes `primary`, `primary_light`, `accent`, `text_light`, and `text_dark`.
