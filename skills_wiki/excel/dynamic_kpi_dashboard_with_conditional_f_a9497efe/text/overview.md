...
> **Skill Name**: Dynamic KPI Dashboard with Conditional Formatting

* **Tier**: sheet_shell
* **Core Mechanism**: This skill constructs a visually clear and dynamic Key Performance Indicator (KPI) dashboard sheet. It leverages `INDEX MATCH` formulas to fetch pre-calculated KPI values and comparative data (target, prior month) from a 'Staging' sheet, which in turn derives its data from a 'Data' sheet. `Conditional Formatting` is extensively used to apply intuitive color-coding (green/red) based on metric performance against targets, making the dashboard highly actionable.
* **Applicability**: This skill is ideal for businesses and finance professionals who need to monitor key operational and financial metrics over time. It's suitable for reports that require dynamic viewing of monthly performance, easy comparison to targets and historical data, and immediate visual indication of performance health. Requires a structured 'Data' input sheet and a 'Staging' sheet for KPI calculations, which are mocked in the provided code for demonstrating the dashboard sheet shell.

### 2. Structural Breakdown

- **Data Layout**:
    - The `Dashboard` sheet presents KPIs in a grid layout, grouped by categories (e.g., "Working Capital Efficiency", "Sales KPIs", "Cost KPIs"). Each KPI block consists of a merged cell for the KPI name, a large merged cell for the current value, and smaller cells below for "Vs. Target" and "Vs. Prior Month" labels and values.
    - A single cell (e.g., `D4`) acts as a month selector (dropdown) to control which month's data is displayed.
    - The `Data` tab (mocked) is a simple table with categories in column A and months as headers in row 1, containing raw numerical data.
    - The `Staging` tab (mocked) is a similar table, pulling data from the `Data` tab and performing intermediate calculations to derive KPI values and storing static target values for each KPI.

- **Formula Logic**:
    - **Month Selector (`D4` on Dashboard)**: Data validation list referencing a range of dates (formatted as "MMM-YY") from the `Staging` sheet's header row.
    - **KPI Value Display (e.g., `B8` for DSO on Dashboard)**: `=INDEX('2) Staging'!$C$<KPI_Staging_Row>:$N$<KPI_Staging_Row>,MATCH($D$4,'2) Staging'!$C$3:$N$3,0))` - Dynamically fetches the KPI value from the `Staging` sheet based on the selected month (`D4`) and the KPI's row.
    - **Vs. Target Value (e.g., `C9` for DSO on Dashboard)**: `=INDEX('2) Staging'!$C$<Target_Staging_Row>:$N$<Target_Staging_Row>,MATCH($D$4,'2) Staging'!$C$3:$N$3,0))` - Fetches the static target value for the KPI from the `Staging` sheet.
    - **Vs. Prior Month Value (e.g., `E9` for DSO on Dashboard)**: `=INDEX('2) Staging'!$C$<KPI_Staging_Row>:$N$<KPI_Staging_Row>,MATCH(EOMONTH($D$4,-1),'2) Staging'!$C$3:$N$3,0))` - Fetches the prior month's data using `EOMONTH` within the `MATCH` function.
    - **KPI Calculation on Staging Sheet (e.g., DSO)**: `=IFERROR(ROUND((<AR_Balance_Cell>/<Credit_Sales_Cell>)*<Days_in_Month_Cell>,0),0)` - (Example for DSO using linked cells from the `Data` tab). Similar formulas are used for DPO, CAC, Gross Margin, Sales vs. Budget, OPEX Actual vs. Budget, and CPFTE.
    - **# of days in month (Staging Sheet)**: `=DAY('1) Data'!<Month_Date_Cell>)`.

- **Visual Design**:
    - **Fonts**: Large font sizes (e.g., 24pt) for main KPI values, smaller (e.g., 9-12pt) for labels and comparative values. Bold text for titles and labels.
    - **Colors**: Theme-based background colors for main headers (e.g., orange for dashboard title, specific colors for KPI categories like gray, blue, red). White text on colored headers. Yellow fill for underlying data/staging cells.
    - **Borders**: Thin, black borders around all KPI blocks and individual cells within them.
    - **Conditional Formatting**: Applied to main KPI values. Rules are defined to color the cell background green for "good" performance (e.g., DSO less than target, Sales vs. Budget >= 100%) and pink/red for "bad" performance (e.g., DSO greater than target, Sales vs. Budget < 100%).

- **Charts/Tables**: The dashboard itself is structured as a highly visual table of KPI metrics rather than containing traditional charts.

- **Theme Hooks**: `header_bg`, `header_fg`, `accent_bg`, `accent_fg`, `good_performance_bg`, `bad_performance_bg`, `neutral_bg`.
