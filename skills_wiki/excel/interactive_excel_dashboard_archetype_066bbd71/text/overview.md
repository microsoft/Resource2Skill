...
*   **Tier**: archetype
*   **Core Mechanism**: This skill constructs a multi-sheet interactive dashboard in Microsoft Excel. It takes raw tabular data, converts it into an Excel Table, generates multiple PivotTables and PivotCharts, and integrates them into a user-friendly dashboard with interactive slicers and timelines. The dashboard automatically updates when new data is added to the source table and is styled dynamically with Excel themes.
*   **Applicability**: This skill is ideal for creating dynamic business intelligence reports for presenting key performance indicators (KPIs) and trends. It's suitable for sales data, financial performance, project tracking, or any tabular dataset where users need to explore data across different dimensions (e.g., region, product, time) and visualize trends without needing to manually update charts or formulas.

### 2. Structural Breakdown

-   **Data Layout**: Source data resides on a dedicated 'Data' sheet, formatted as an Excel Table. PivotTables are generated on separate, hidden sheets for organization. The main 'Dashboard' sheet consolidates the PivotCharts, Timeline, and Slicers in a visually organized manner.
-   **Formula Logic**: Primarily leverages Excel's built-in PivotTable functionality for data summarization and aggregation. The pivot tables automatically update to reflect changes in the source data table upon refresh.
-   **Visual Design**:
    -   **Header**: A merged cell spanning the top of the dashboard, containing a company name and dashboard title, with a solid background fill and contrasting font color.
    -   **Sheet View**: Gridlines and row/column headings are hidden on the 'Dashboard' sheet for a clean, report-like appearance.
    -   **Slicers/Timeline**: These interactive filter elements are positioned on the left side of the dashboard, allowing users to dynamically filter data across all connected charts. Slicer headers are hidden to streamline the interface.
    -   **Charts**: Charts (Stacked Column, Line) are positioned on the right side of the dashboard. They include descriptive titles, relevant axis titles, and hidden field buttons for a clean presentation. Numeric axis labels are formatted (currency/comma style, no decimals).
    -   **Themes**: The entire workbook's aesthetic (color schemes, chart styles, fonts) is driven by Excel's native themes, which can be easily swapped via the 'Page Layout' tab.
-   **Charts/Tables**:
    -   **Data Table**: An Excel Table (`TableStyleLight1`) on the 'Data' sheet, automatically expanding with new data.
    -   **PivotTables**: Three pivot tables (one for each chart) are created on hidden worksheets, pulling data from the main data table.
    -   **Profit by Market & Cookie Type Chart**: A Stacked Column Chart displaying profit distribution by country (rows) and cookie type (columns), linked to its corresponding pivot table.
    -   **Units Sold each month Chart**: A Line Chart showing the trend of units sold over time (months), linked to its corresponding pivot table.
    -   **Profit by month Chart**: A Line Chart illustrating profit trends over time (months), linked to its corresponding pivot table.
-   **Theme Hooks**: `header_bg`, `header_fg`, `text_fg`, `accent_bg`, `accent_fg`, `chart_line` (for line charts). The skill applies a comprehensive theme to the workbook, impacting all visual elements.
