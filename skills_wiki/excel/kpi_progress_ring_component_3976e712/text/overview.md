...
> **Skill Name**: KPI Progress Ring Component

* **Tier**: component
* **Core Mechanism**: Stacks KPI values (actual, target, and percentage) vertically in a styled text block, calculates `% Complete` and `% Remainder` in adjacent hidden cells, and renders a legend-less Doughnut chart with a 75% hole size to create a clean "progress ring" visual next to the metrics.
* **Applicability**: Best used in executive dashboards or summary sheets where tracking a specific metric against a target is required. Fits perfectly into grid or card-based dashboard layouts.

### 2. Structural Breakdown

- **Data Layout**: Renders text vertically at the anchor column. Places charting calculation data slightly below the text block and hides it using white font color to maintain a clean dashboard look.
- **Formula Logic**: Calculates progress using `Actual/Target` and sets the remainder slice using `=1-[Complete Cell]`.
- **Visual Design**: Uses varied font sizes (large 24pt for absolute values, 14pt for percentages) and gray italics for the target context.
- **Charts/Tables**: Employs a `DoughnutChart` sized as a compact square (4x4). Sets `holeSize = 75` to make it a thin ring, removes the legend, and disables shape outline borders (`line.noFill = True`) for seamless integration.
- **Theme Hooks**: Uses a dark primary accent (`1F4E78`) for main values and percentages, and standard grays (`595959`, `808080`) for contextual labels.
