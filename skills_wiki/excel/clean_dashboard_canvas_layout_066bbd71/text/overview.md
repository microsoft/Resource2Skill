...
> **Skill Name**: Clean Dashboard Canvas Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Transforms a standard worksheet into a professional reporting dashboard canvas. This is achieved by disabling gridlines and row/column headers, creating a dominant, visually distinct header strip across the top, and arranging multiple charts (e.g., stacked column, line charts) in a clean, snapped-to-grid layout while isolating raw data onto a separate, hidden sheet. 
* **Applicability**: Essential for any automated reporting script where the end deliverable is a high-level executive dashboard. Isolates the visual presentation from the underlying data tables, preventing users from accidentally breaking data while reviewing metrics.

### 2. Structural Breakdown

- **Data Layout**: Raw data is appended to a separate `<SheetName>_Data` worksheet which is subsequently hidden (`ws.sheet_state = 'hidden'`). This keeps the main dashboard sheet purely visual.
- **Formula Logic**: Purely chart-driven; `openpyxl.chart.Reference` maps ranges from the hidden data sheet directly into the dashboard charts.
- **Visual Design**: 
  - `ws.sheet_view.showGridLines = False`
  - `ws.sheet_view.showRowColHeaders = False`
  - Deep theme-colored merged header spanning the dashboard width (`A1:R2`) with vertically/horizontally centered white text.
- **Charts/Tables**: 
  - Chart 1: A `BarChart` configured with `grouping="stacked"` and `overlap=100` for a stacked column composition.
  - Chart 2 & 3: `LineChart` objects with their legends disabled (`legend = None`) to maximize the data-ink ratio.
  - Dimensions are explicitly set (`chart.height`, `chart.width`) and anchored to specific cell coordinates to create a predictable grid structure.
- **Theme Hooks**: 
  - `header_bg`: Primary fill color for the dashboard title banner.
  - `header_fg`: Font color for the dashboard title banner.
