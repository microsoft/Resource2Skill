...
> **Skill Name**: Dynamic KPI Dashboard Sheet

*   **Tier**: sheet_shell
*   **Core Mechanism**: This skill constructs a single, presentation-focused KPI dashboard sheet. It features large, bold numbers with conditional color-coding (green/red) to indicate performance against targets, dynamic data retrieval for current and prior periods based on a user-selected month (using `INDEX MATCH` and `EOMONTH`), and clear section headers for readability.
*   **Applicability**: Ideal for executive summary reports, operational dashboards, or any scenario requiring a high-level, easily digestible overview of key performance indicators. It assumes pre-calculated data is available in a 'Staging' sheet, which itself sources from a 'Data' sheet.

### 2. Structural Breakdown

-   **Data Layout**:
    -   Cell `D4` contains the selected month (date formatted `MMM-YY`), controlled by a Data Validation dropdown list.
    -   KPI sections (e.g., "Working Capital Efficiency", "Sales KPIs", "Cost KPIs") are presented as merged cells spanning columns and rows.
    -   Each KPI is displayed in a block of merged cells (e.g., `B7:F9` for DSO).
    -   The main KPI value uses a large font in a central merged cell.
    -   Smaller labels ("Vs. Target", "Vs. Prior Month") and their corresponding values are placed below the main KPI value.
-   **Formula Logic**:
    -   **Month Selection (D4)**: Data Validation list pointing to a range of dates (e.g., `='Staging'!$B$3:$N$3`).
    -   **Current KPI Value (e.g., D8 for DSO)**: `INDEX(Staging!$B$7:$N$7, MATCH($D$4, Staging!$B$3:$N$3, 0))`
        -   `Staging!$B$7:$N$7` is the row on the 'Staging' sheet containing DSO values for each month.
        -   `$D$4` is the selected month from the dashboard dropdown.
        -   `Staging!$B$3:$N$3` is the row on the 'Staging' sheet containing month headers.
    -   **Target KPI Value (e.g., C9 for DSO)**: `INDEX(Staging!$B$8:$N$8, MATCH($D$4, Staging!$B$3:$N$3, 0))`
        -   `Staging!$B$8:$N$8` is the row on the 'Staging' sheet containing DSO target values for each month.
    -   **Prior Month KPI Value (e.g., F9 for DSO)**: `INDEX(Staging!$B$7:$N$7, MATCH(EOMONTH($D$4,-1), Staging!$B$3:$N$3, 0))`
        -   `EOMONTH($D$4,-1)` dynamically calculates the last day of the prior month, allowing the `MATCH` function to find the correct column.
-   **Visual Design**:
    -   **Fonts**: Large (e.g., 36-48pt) for primary KPI numbers; regular (e.g., 10-12pt) for labels and comparison values. All in 'Montserrat' or similar sans-serif font.
    -   **Headers**: Main dashboard title (e.g., `A1:F2`) uses a distinct `header_bg` (e.g., orange) with `header_fg` (white). KPI category headers (e.g., `A5:F5`) use `category_bg` (e.g., gray) with `category_fg` (white).
    -   **Borders**: Thin borders define individual KPI blocks and comparison cells.
    -   **Conditional Formatting**: Applied to current KPI values.
        -   **DSO (Days Sales Outstanding)**: Green if value <= Target; Pink/Red if value > Target (lower is better).
        -   **DPO (Days Payables Outstanding)**: Green if value >= Target; Pink/Red if value < Target (higher is better).
        -   **Sales vs. Budget%**: Green if value >= 100%; Pink/Red if value < 100%.
        -   **Gross Margin**: Green if value >= Target; Pink/Red if value < Target.
        -   **OPEX Actual vs. Budget**: Green if value <= 100%; Pink/Red if value > 100%.
        -   **CAC (Customer Acquisition Cost)**: Green if value <= Target; Pink/Red if value > Target (lower is better).
        -   **CPFTE (Cost Per Full Time Employee)**: Green if value <= Target; Pink/Red if value > Target (lower is better).
    -   **Alignment**: Center-aligned for numbers and labels within KPI blocks.
-   **Charts/Tables**: No direct charts or `Table` objects are used on the dashboard. The visual impact is achieved through structured KPI cards and conditional formatting.
-   **Theme Hooks**: `header_bg`, `header_fg`, `category_bg`, `category_fg`, `primary_bg`, `primary_fg`, `good_indicator_bg`, `bad_indicator_bg`, `neutral_bg`, `accent_1_bg`, `accent_2_bg`, `accent_3_bg`.
