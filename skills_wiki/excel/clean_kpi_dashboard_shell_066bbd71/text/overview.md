...
> **Skill Name**: Clean KPI Dashboard Shell

* **Tier**: archetype
* **Core Mechanism**: Separates raw data logic from presentation by creating a dedicated "Data" sheet and a "Dashboard" sheet. The dashboard hides gridlines, uses a prominent merged header block, and organizes multiple styled charts (Stacked Columns and Lines) into a clean, unified view to serve as a high-level UI.
* **Applicability**: Best used when you need to present multiple key metrics (e.g., sales, profit by region, units over time) to leadership. This structure replaces messy, data-heavy worksheets with an interactive-feeling, clean visual report.

### 2. Structural Breakdown

- **Data Layout**: A hidden or secondary `Data` sheet holds summary blocks (or PivotTables). The primary `Dashboard` sheet holds only visual elements (Charts, Shapes, Headers).
- **Formula Logic**: Standardized data references mapping the `Data` sheet ranges into the `Dashboard` charts.
- **Visual Design**: Gridlines are disabled (`ws.sheet_view.showGridLines = False`). A main title spans the top of the dashboard using merged cells, deep background fill, and white bold text.
- **Charts/Tables**: Implements a stacked column chart (100% overlap) for categorical drill-downs (Profit by Market & Cookie) and clean line charts (with legends removed) for time-series trends, aligned onto the grid.
- **Theme Hooks**: The title header background and chart styles can be hooked into standard corporate palettes.
