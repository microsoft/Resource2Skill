...
> **Skill Name**: Themed Sales Performance Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Constructs a split-background dashboard canvas using row fills, simulates "floating" KPI cards using merged cell clusters with thick accent borders, and embeds a conditional-formatting-driven performance table (using `DataBarRule`) alongside standard charts.
* **Applicability**: Best for executive summaries, sales agent tracking, or any high-level metric dashboard where a mix of aggregate KPIs, individual performance rankings, and trend charts need to be displayed cleanly on a single page.

### 2. Structural Breakdown

- **Data Layout**: 
  - Rows 1-8: Dark header banner.
  - Rows 9-40: Light canvas background.
  - Columns C-M (Row 5-7): Horizontal KPI strip.
  - Columns B-F (Row 10+): Agent Performance Table.
  - Columns H-M (Row 10+): Charting area.
- **Formula Logic**: Directly embeds values for the layout shell; in a dynamic scenario, these cells would house `=GETPIVOTDATA()` or `SUMIFS()` connecting to an analysis sheet.
- **Visual Design**: Uses a two-tone background to anchor the header. KPI cards use a white fill with a thick, colored left-border acting as a visual divider between the (simulated) icon and value.
- **Charts/Tables**: Applies inline `DataBarRule` conditional formatting to the performance table to create horizontal, in-cell bar charts. Includes a Bar Chart for aggregate trends.
- **Theme Hooks**: Consumes `primary` (header background), `bg_light` (canvas background), `accent` (KPI borders, chart series), and `text_light` (header text).
