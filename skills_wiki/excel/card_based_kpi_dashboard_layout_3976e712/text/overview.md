...
> **Skill Name**: Card-based KPI Dashboard Layout

* **Tier**: sheet_shell
* **Core Mechanism**: Simulates a modern UI layout by removing gridlines, painting the sheet background light gray, and coloring distinct cell ranges white to form "cards". Embeds formatted KPI values and styled charts directly into these floating card regions while storing the chart data out of sight.
* **Applicability**: Best used for executive summaries or high-level dashboards where visual separation of KPIs, trends, and categorical breakdowns is needed on a single view.

### 2. Structural Breakdown

- **Data Layout**: The dashboard components sit in columns B through M. Chart data (categories and values) is written to hidden columns (e.g., column T and U) to keep the dashboard layer pristine.
- **Formula Logic**: Calculates "Percentage Achieved" and "Percentage Remaining" dynamically from raw KPI values and target values to feed the doughnut charts.
- **Visual Design**: 
  - `F3F3F3` background fill creates negative space. 
  - `FFFFFF` fill creates the isolated cards. 
  - A prominent left sidebar painted with the primary theme color anchors the dashboard.
- **Charts/Tables**: Integrates `DoughnutChart` for KPI completion tracking, `LineChart` for monthly trends, and horizontal `BarChart` for geographical breakdown. Legends are removed to maximize data-ink ratio.
- **Theme Hooks**: Consumes `primary` for the sidebar and title font color, falling back to corporate blues.
