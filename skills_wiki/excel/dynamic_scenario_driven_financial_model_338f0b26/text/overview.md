...
> **Skill Name**: Dynamic Scenario-Driven Financial Model

* **Tier**: sheet_shell
* **Core Mechanism**: Build a dynamic P&L model using `CHOOSE` and Data Validation. Multiple scenario assumption blocks (Best Case, Worst Case) are hardcoded. A single "Live" assumption block dynamically pulls values based on the active scenario dropdown. The Income Statement formulas are exclusively driven by this Live block, elegantly separating model logic from raw data inputs.
* **Applicability**: Perfect for budgeting, forecasting, or equity valuation where users need to instantly toggle between "Base", "Bull", and "Bear" business conditions without overwriting cells.

### 2. Structural Breakdown

- **Data Layout**: Columns for `[Line Item, Unit, Year 1... Year 5]`. Vertically stacked sections: Income Statement (top), Live Assumptions (middle), and Scenario Assumption blocks (bottom).
- **Formula Logic**: 
  - Validation dropdown in `K3` (values `1` or `2`).
  - Live Assumptions use `=CHOOSE($K$3, Scen1_Cell, Scen2_Cell)` to fetch the right data.
  - Compounding growth: `=Prev_Yr_Vol * (1 + Curr_Yr_Growth)`.
- **Visual Design**: Hardcoded inputs use a distinct blue font color (`#0000FF`) to indicate editability (a strict financial modeling convention). Scenario dropdown is highlighted. Standard financial accounting borders (single top, double bottom for Net Profit).
- **Theme Hooks**: `primary` for main headers and the "Live Assumptions" banner; `bg` for header text.
