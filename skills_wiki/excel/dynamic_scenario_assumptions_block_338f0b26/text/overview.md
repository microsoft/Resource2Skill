...
> **Skill Name**: Dynamic Scenario Assumptions Block

* **Tier**: component
* **Core Mechanism**: Stacks a "Live Case" assumption block above multiple scenario assumption blocks. It links a Data Validation dropdown to a target cell, and uses the `CHOOSE` formula to dynamically populate the Live Case block based on the selected scenario index. 
* **Applicability**: Highly applicable to financial models, business cases, and forecasting tools where users need to toggle between base, upside, and downside scenarios seamlessly without breaking downstream formulas. Constraints: Assumes a consistent number of assumptions across scenarios. Limited to 254 scenarios (Excel's limit for `CHOOSE` arguments).

### 2. Structural Breakdown

- **Data Layout**: Places the Scenario Selector at the designated anchor cell. The "Live Case" block is structured a few rows below, followed by dynamic spacing and individual blocks for each hardcoded scenario.
- **Formula Logic**: Uses `=CHOOSE($col$row, scenario1_ref, scenario2_ref, ...)` to evaluate the selector index and route the corresponding scenario's hardcoded cell into the live driver model.
- **Visual Design**: Adheres to financial modeling standards—hardcoded inputs use blue fonts (`0000FF`), while dynamic/calculated cells remain black (and bolded for the live case). The Data Validation selector cell uses a yellow fill to indicate an interactive input.
- **Charts/Tables**: Standard spreadsheet range layout without structured table formatting to preserve formula dragging compatibility.
- **Theme Hooks**: Background fills for block headers provide visual hierarchy, utilizing soft blues and neutral grays (simulating standard formatting presets).
