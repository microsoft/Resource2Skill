...
> **Skill Name**: Themed 3-Chart Dashboard Canvas

* **Tier**: sheet_shell
* **Core Mechanism**: Transforms a standard worksheet into a clean visual canvas by hiding gridlines, establishing a bold typographic header, and programmatically anchoring multiple charts (a primary stacked bar chart and two secondary trend lines) into a unified grid layout. Backing data is written to hidden columns to keep the canvas clean.
* **Applicability**: Best used when generating static, high-level executive summary dashboards. *Note: While the tutorial demonstrates interactive Slicers and Timelines via PivotTables, openpyxl is best suited for generating the static layout, visual styling, and chart arrangements of the final rendered dashboard using pre-aggregated summary data.*

### 2. Structural Breakdown

- **Data Layout**: Pre-aggregated summary data (Bar Chart data, Line Chart 1 data, Line Chart 2 data) is written to "off-screen" columns (e.g., columns `AA` onwards) and grouped/hidden so the user only sees the dashboard canvas.
- **Formula Logic**: None required; relies on pre-aggregated data passed via arguments or uses sensible defaults.
- **Visual Design**: Sheet gridlines disabled (`showGridLines = False`). The main title uses an oversized, bold font pulling from the primary theme color.
- **Charts/Tables**: 
  - 1 Stacked Column Chart (Primary metric by Category & Sub-category) anchored to the left.
  - 2 Line Charts (Time-series trends) stacked vertically on the right. Legends removed for cleaner look.
- **Theme Hooks**: Uses a theme palette dictionary for the primary title color (`primary`) to ensure it matches the corporate branding layout.
