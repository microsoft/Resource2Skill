...
> **Skill Name**: Static Chart Dashboard Archetype

* **Tier**: archetype
* **Core Mechanism**: Assembles a clean, presentation-ready dashboard by disabling gridlines, rendering a themed title banner, and anchoring multiple charts (stacked bar, lines) in a grid layout. Raw data for the charts is isolated on a secondary, hidden worksheet to emulate a PivotCache architecture.
* **Applicability**: Best used when generating automated high-level reporting workbooks where end-users need to consume metrics visually without seeing the underlying raw datasets. Replaces the need for complex, fragile PivotTables in programmatic Excel generation by pre-aggregating data and charting it directly.

### 2. Structural Breakdown

- **Data Layout**: A hidden `ChartData` worksheet stores the chart backing data sequentially in blocks, cleanly separating raw numbers from the presentation layer.
- **Formula Logic**: No complex formulas required; the pattern relies on precise `Reference` ranges pointing from the Dashboard's charts to the hidden data sheet.
- **Visual Design**: `ws.sheet_view.showGridLines = False` creates a blank canvas. A merged cell range (e.g., `A1:R3`) serves as a prominent title banner filled with the primary theme color.
- **Charts/Tables**: Utilizes a Stacked `BarChart` for category vs. segment comparisons, and `LineChart` objects (with `legend = None`) for time-series trends, positioned explicitly at cell anchors (e.g., `B6`, `K6`).
- **Theme Hooks**: The dashboard banner consumes a primary background color (falling back to a standard corporate blue) and a contrasting text color.
