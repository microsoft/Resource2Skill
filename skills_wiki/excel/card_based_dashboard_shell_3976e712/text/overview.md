...
> **Skill Name**: Card-Based Dashboard Shell

* **Tier**: sheet_shell
* **Core Mechanism**: Emulates a modern UI "card" layout using cell formatting instead of floating shapes (which are brittle to generate programmatically). Disables gridlines, paints a light gray canvas, and creates distinct white rectangular regions with thin borders to act as widget containers. Includes a colored sidebar for a navigation menu.
* **Applicability**: Ideal for executive summaries, KPI dashboards, and high-level reports where a clean, web-like layout is preferred over a raw data grid. Provides structured "drop-zones" for charts and metric callouts.

### 2. Structural Breakdown

- **Data Layout**: UI Shell. No raw data is present; it establishes designated ranges (cards) for anchoring charts and KPIs later.
- **Formula Logic**: None (purely structural and visual).
- **Visual Design**: Gridlines disabled. Background filled with a soft gray (`F3F4F6`). Sidebar filled with the primary theme color. Cards filled with white (`FFFFFF`) with a subtle outline (`D1D5DB`). Card headers are bolded, sized up, and vertically centered.
- **Charts/Tables**: Provides the bounding boxes (cards) where visual components (Donut charts, Line charts, Maps) should be placed.
- **Theme Hooks**: Uses the primary theme color for the sidebar and main dashboard title. Relies on neutral grays for the canvas background and card borders.
