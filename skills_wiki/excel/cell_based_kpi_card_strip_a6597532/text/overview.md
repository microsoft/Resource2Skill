...
> **Skill Name**: Cell-based KPI Card Strip

* **Tier**: component
* **Core Mechanism**: Renders a row of horizontal dashboard KPI cards using merged cells instead of floating shapes. Uses a 2x2 cell matrix per card to simulate a colored accent bar on the left (spanning two rows for an icon) and stacked text on the right (Value over Label).
* **Applicability**: Use at the top of a dashboard or report to highlight key metrics. Works best when worksheet gridlines are disabled (`ws.sheet_view.showGridLines = False`) and the underlying worksheet background contrasts with the card color.

### 2. Structural Breakdown

- **Data Layout**: Renders horizontally starting from `anchor`. Each card uses 2 columns and 2 rows, followed by a 1-column spacer.
- **Formula Logic**: Static values injected from a list of `kpi_data` dictionaries.
- **Visual Design**: Simulates a sleek shape-based UI using `PatternFill` and `Font` contrast. Uses a dark/accent fill for the left strip and a white fill for the data block.
- **Charts/Tables**: N/A
- **Theme Hooks**: Uses `accent` for the icon bar, `card` (usually white) for the text background, and `val`/`lbl` for typography colors.
