...
> **Skill Name**: Procedural Sugar-Coated Candy (Geometry Nodes Scatter)

* **Core Visual Mechanism**: Using Geometry Nodes to procedurally scatter and instance small objects (sugar crystals) across the surface of a base mesh (a gummy candy/torus). The signature of this technique is the non-destructive layering of detail using the `Distribute Points on Faces`, `Instance on Points`, and `Join Geometry` nodes.
* **Why Use This Skill (Rationale)**: Manually placing hundreds of tiny objects is impossible and destructive. This procedural pattern allows for organic, randomized scattering. By using `Random Value` nodes configured to `Vector` (for independent X/Y/Z rotation in radians) and `Float` (for uniform scaling), the instanced objects look naturally settled and chaotic, mimicking real-world physics and distribution.
* **Overall Applicability**: This is the foundational pattern for any surface scattering in 3D: sugar on candy, sprinkles on a donut, moss/pebbles on a rock, or trees on a terrain. 
* **Value Addition**: It adds micro-detail and macro-realism to an otherwise plain surface without permanently altering the base topology, allowing you to tweak density and variation on the fly.

### 2. Technical Breakdown

* **Step A: Geometry & Topology**
  - **Base Mesh**: A Torus (acting as the gummy candy), subdivided to create a smooth, high-poly surface.
  - **Instance Mesh**: A tiny Cube (acting as the sugar crystal).
  - **Modifier Stack**: A `Subdivision Surface` modifier is applied first to round out the base mesh, followed by the `Geometry Nodes` modifier which executes the scatter logic.
* **Step B: Materials & Shading**
  - **Candy Base**: A transmissive Principled BSDF material. Base color is typically a vibrant red `(0.8, 0.1, 0.1)`. High transmission (`0.8+`) and moderate roughness (`0.2`) create a gummy, translucent effect.
  - **Sugar Crystals**: A glassy Principled BSDF material. Pure white `(1.0, 1.0, 1.0)`, high transmission (`0.9`), and low roughness (`0.1`) to mimic refractive crystalline sugar.
* **Step C: Lighting & Rendering Context**
  - **Lighting**: Works best with strong backlighting or rim lighting to emphasize the subsurface/transmissive qualities of the gummy and the specular glints of the sugar.
  - **Render Engine**: Cycles is strongly recommended for physically accurate light refraction through the thousands of tiny transmissive crystals.
* **Step D: Animation & Dynamics**
  - Completely procedural and real-time. The GN modifier recalculates instantly if the base mesh is animated or deformed.
...
|---|---|---|
| Base & Crystal Geometry | `bpy.ops.mesh.primitive_*` | Provides the necessary starting mesh data for the node tree. |
| Surface Scattering | Geometry Nodes | The exact workflow demonstrated in the tutorial, providing non-destructive procedural instancing. |
| Random Variation | GN `FunctionNodeRandomValue` | Allows independent control over instance Rotation (Vector) and Scale (Float) driven by a procedural seed. |

> **Feasibility Assessment**: 100% reproduction. The code below programmatically builds the exact Geometry Nodes tree demonstrated in the video, right down to using `math.tau` for a full 360-degree radian rotation randomization.
