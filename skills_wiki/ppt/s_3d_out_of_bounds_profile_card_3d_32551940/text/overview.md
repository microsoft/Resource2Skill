# 3D Out-of-Bounds Profile Card (3D人物破局穿插排版)

## Analysis

Here is the skill extraction and reproduction code based on the provided tutorial.

### 1. High-level Design Pattern Extraction

> **Skill Name**: 3D Out-of-Bounds Profile Card (3D人物破局穿插排版)

* **Core Visual Mechanism**: The defining visual idea is **breaking the geometric boundary**. By layering a cutout portrait (transparent background) over a base geometric shape (like a circle or ellipse) and allowing the subject's head/shoulders to extend *beyond* the shape's top border while the bottom remains contained, it creates a powerful optical illusion of 3D depth and forward momentum.
* **Why Use This Skill (Rationale)**: Standard rectangular photos feel flat, passive, and constrained. The "pop-out" effect breaks the rigid grid of standard presentation design, drawing the viewer's eye immediately to the subject. It feels dynamic, energetic, and magazine-like, projecting authority and approachability simultaneously.
* **Overall Applicability**: Perfect for Executive/Team bio slides, Keynote speaker introductions, Award winner announcements, Portfolio covers, and "About the Author" sections.
* **Value Addition**: Transforms a standard bio slide into a high-end, bespoke graphical layout. It elevates the perceived production value of the presentation from "amateur" to "professional agency."

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  * **Cutout Portrait**: The focal point. Must have a transparent background (alpha channel).
  * **Geometric Anchor**: A solid shape (circle, ellipse, or brush stroke) placed behind the portrait.
  * **Color Logic**:
    * Slide Background: Clean White `(255, 255, 255, 255)` or Light Gray `(245, 247, 250, 255)`.
    * Anchor Shape: Deep Corporate Navy `(28, 53, 94, 255)` or Brand Accent Color.
    * Primary Text: Dark Indigo/Black `(20, 30, 40, 255)`.
    * Accent Elements (Lines/Icons): Matches the Anchor Shape `(28, 53, 94, 255)`.
  * **Text Hierarchy**: Huge background watermark -> Bold prominent Name -> Subtle Job Title -> Structured, bulleted biographical text.

* **Step B: Compositional Style**
  * **Spatial Feel**: Asymmetrical balance. The left side (40% width) holds the heavy visual anchor (the portrait card), while the right side (60% width) breathes with negative space and structured typography.
  * **Layering**: Bottom = Watermark Text/Decorations -> Middle = Geometric Anchor -> Top = Portrait.

* **Step C: Dynamic Effects & Transitions**
  * **Morph Transition (平滑过渡)**: The tutorial hints at changing the base shapes (from circles to ellipses to brush strokes) across slides. Using PowerPoint's native Morph transition between these layouts creates a fluid, highly engaging animation.
...
|---|---|---|
| **Base Shape & Typography Layout** | `python-pptx` native | Ideal for precise grid positioning, text formatting, and drawing standard geometric shapes (circles, lines). |
| **Out-of-Bounds Portrait** | PIL (Pillow) | The tutorial uses PPT's manual boolean shape intersections. To replicate this automatically without a human, we use PIL to dynamically generate a transparent cutout portrait and scale it precisely so it overflows the native PPTX background shape. |
| **Watermark Text** | `python-pptx` native | Background oversized text with light grey color creates the professional depth shown in the tutorial. |

> **Feasibility Assessment**: 90%. The code flawlessly reproduces the 3D pop-out layout, typography hierarchy, and color scheme. Because we cannot rely on external APIs (like remove.bg) for automatic background removal in this script, the script includes a built-in PIL generator that creates a highly accurate dummy "cutout portrait" silhouette to prove the technique works immediately out of the box.
