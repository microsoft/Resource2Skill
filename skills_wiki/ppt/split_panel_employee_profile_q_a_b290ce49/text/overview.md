# Split-Panel Employee Profile & Q&A

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Split-Panel Employee Profile & Q&A

* **Core Visual Mechanism**: A stark, highly structured two-column layout. The left column (roughly 1/3 of the slide) serves as a visual anchor, split horizontally between a light neutral background for a profile portrait and a vibrant, solid-color block for the person's core identity (name, title, key quote). The right column (2/3 of the slide) is a clean, white canvas dedicated to a structured Q&A, using the same vibrant color from the left panel as an accent for numbered circular bullet points.
* **Why Use This Skill (Rationale)**: This design solves the problem of presenting a lot of text (an interview/Q&A) without overwhelming the viewer. By confining the heavy text to a clean white space and balancing it with a bold, colorful, and highly visual left panel, the slide remains engaging. The numbered circles draw the eye down the page, establishing a clear reading order.
* **Overall Applicability**: Perfect for "Employee Spotlight" series, team introductions, customer testimonials, or creating a standardized layout for speaker bios in conference decks.
* **Value Addition**: Transforms a standard bulleted text slide into a magazine-style editorial layout. The strict color coordination between the left info block and the right number nodes creates a cohesive, branded feel that can be easily updated per person or department.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  - **Left Panel (Top)**: Light grey background `(235, 235, 235)`. Contains the profile image. In the original, these are transparent cutouts; we will approximate this cleanly with a circular-cropped portrait.
  - **Left Panel (Bottom)**: Solid accent color (e.g., Purple `(110, 80, 160)` or Blue `(0, 120, 180)`). Contains white text.
  - **Right Panel**: Solid white background `(255, 255, 255)`.
  - **Number Nodes**: Perfect circles using the accent color, containing white numbers.
  - **Text Hierarchy**:
    - **Name**: Large, bold, uppercase, white.
    - **Title**: Medium, regular, white.
    - **Quote**: Small, italic, white.
    - **Questions**: Medium, bold, dark slate/brand color `(70, 110, 130)`.
    - **Answers**: Medium, regular, dark grey `(50, 50, 50)`.

* **Step B: Compositional Style**
  - **Vertical Split**: Left panel occupies exactly 33% of the width (~4.5 inches). Right panel occupies 67% (~8.8 inches).
  - **Horizontal Split (Left)**: The left panel is split roughly 50/50 vertically between the photo area and the info area.
  - **List Spacing**: The right side distributes 5 items evenly across the vertical space, ensuring breathing room between questions.

* **Step C: Dynamic Effects & Transitions**
  - This is a static layout pattern designed for rapid reading and uniformity across multiple slides. Page curl or simple push transitions between different employees work best here.
...
|---|---|---|
| **Layout & Split Panels** | `python-pptx` native shapes | Standard rectangles perfectly achieve the color blocking. |
| **Profile Picture** | `PIL/Pillow` | To ensure the profile photo looks professional regardless of the source image, we use PIL to crop it into a perfect circle with a transparent background before inserting it over the grey panel. |
| **Numbered Q&A List** | `python-pptx` native | Circles with text inside, placed next to formatted text boxes, are easily handled by native APIs. |
