# Dynamic Wave Split-Screen Layout

## Analysis

Here is the extracted design skill and the corresponding implementation code.

### 1. High-level Design Pattern Extraction

> **Skill Name**: Dynamic Wave Split-Screen Layout

* **Core Visual Mechanism**: This design utilizes a custom organic S-curve (wavy) shape to split the slide vertically. By filling the right-side wavy shape with a high-quality photograph and layering it over a slightly offset, solid-colored duplicate shape, it creates a subtle "drop shadow" or overlapping paper aesthetic.
* **Why Use This Skill (Rationale)**: The organic curve breaks the rigid, boxy feeling of standard PowerPoint templates, adding a sense of modern fluidity and dynamism. The offset solid color creates depth (2.5D effect) without relying on artificial soft drop-shadows, resulting in a clean, flat-design friendly aesthetic.
* **Overall Applicability**: Perfect for corporate title slides, business proposal covers, and feasibility reports where you want to balance heavy text (left side) with evocative imagery (right side) while maintaining a professional, modern corporate identity.
* **Value Addition**: It elevates a standard text-and-picture slide into a polished, agency-level layout. It ensures the text remains highly readable on a clean background while integrating the image seamlessly into the overall geometry.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  * **Image Masking**: A custom organic wavy polygon that acts as a clipping mask for the background image. The image remains unrotated and upright.
  * **Accent Layering**: A duplicate of the wavy shape, filled with a solid accent color and shifted slightly on the X/Y axes to sit beneath the image layer.
  * **Color Logic**:
    * Primary Text & Logo: Navy Blue `(0, 85, 164, 255)`
    * Accent Shape & Title Highlight: Golden Yellow `(255, 192, 0, 255)`
    * Background: Pure White `(255, 255, 255, 255)`
    * Body Text: Dark Grey `(100, 100, 100, 255)`
  * **Text Hierarchy**: Small stylized "logo" badge -> Bold primary title -> Massive bold secondary title -> Subtle dividing line -> Wrapped body paragraph.

* **Step B: Compositional Style**
  * **Split Ratio**: The canvas is split roughly 45/55. The left 45% is dedicated entirely to text layout (negative space), and the right 55% is occupied by the visual elements.
  * **Alignment**: All text is strictly left-aligned, establishing a strong, invisible vertical grid line starting at about `X = 1.0 inch`.

* **Step C: Dynamic Effects & Transitions**
  * **Transition**: Works beautifully with PowerPoint's "Morph" or "Fly In" from the right for the shapes, and a "Fade" or "Wipe" for the text.
...
|---|---|---|
| **Wavy Organic Shape** | `python` Math + `python-pptx` FreeformBuilder | Since `python-pptx` lacks a native Bezier API, we calculate the cubic Bezier curve coordinates mathematically in Python, generating a high-resolution polygon. This is much more reliable and editable than injecting raw XML. |
| **Upright Picture Fill** | `python-pptx` native `user_picture` fill | By drawing the shape accurately without relying on slide rotation (as seen in the manual tutorial), we bypass the notorious PowerPoint bug where image fills rotate with the shape. The image stays perfectly upright. |
| **Layered Depth** | `python-pptx` shape rendering order | Drawing the yellow accent shape first, followed by the image shape, naturally perfectly stacks them on the z-axis. |
| **Typography & Layout** | `python-pptx` text frames | Excellent for precise coordinate placement, font sizing, and color control. |

*Feasibility Assessment*: **100% reproduction.** The code algorithmically recreates the exact S-curve geometry, performs the image fill cleanly without distortion, offsets the accent layer, and sets up the precise typography.
