# Standardized Orthogonal Flowcharting

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Standardized Orthogonal Flowcharting

* **Core Visual Mechanism**: The visual signature relies on standardized geometric shapes (Terminators, Processes, Decisions) arranged on a strict grid, connected by 90-degree orthogonal "elbow" lines. The aesthetic is driven by uniformity—equal spacing, consistent color fills, and centered, high-contrast typography.
* **Why Use This Skill (Rationale)**: Flowcharts leverage established cognitive schemas. By using universal symbols (diamonds for questions, rectangles for actions) and restricting connections to right angles, the design minimizes cognitive load, allowing the viewer to process complex algorithms or business processes step-by-step without visual distraction.
* **Overall Applicability**: Ideal for SOPs (Standard Operating Procedures), hiring pipelines, software algorithm documentation, user journey maps, and troubleshooting guides. 
* **Value Addition**: Transforms dense bullet points into a scannable, logical narrative. It forces the presenter to clarify their logic and makes identifying bottlenecks or decision points immediate for the audience.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  - **Shapes**: Rounded rectangles (Terminators/Begin-End), standard Rectangles (Process steps), and Diamonds (Decisions).
  - **Color Logic**: 
    - Shape Fill: Professional standard blue `(68, 114, 196)`.
    - Shape Outline: Darker blue or absent `(47, 85, 151)`.
    - Text: Pure white `(255, 255, 255)` for contrast.
    - Connectors: Dark blue or dark gray `(89, 89, 89)`.
  - **Text Hierarchy**: Body text inside shapes is concise (1-4 words), formatted as 16pt-18pt sans-serif (Calibri/Arial), bolded, and middle-centered. Branching labels ("Yes", "No") are smaller (12pt-14pt) and placed adjacent to connector lines.

* **Step B: Compositional Style**
  - **Spatial Feel**: Vertical gravity. The primary path flows straight down the center of the canvas. Secondary or exception paths branch out horizontally to the right or left, and either terminate or loop back.
  - **Proportions**: Shapes are universally sized (e.g., 2 inches wide by 0.8 inches high) to maintain rhythm. Vertical spacing between shapes is identical (e.g., 0.5 inches), acting as a visual metronome for the process.

* **Step C: Dynamic Effects & Transitions**
  - Usually static, but can be animated using the "Wipe" (from Top) or "Fade" transition, revealing one shape and its corresponding connector line at a time to walk the audience through the process.
...
|---|---|---|
| Flowchart Shapes (Process, Decision, etc.) | `python-pptx` native (`MSO_AUTO_SHAPE_TYPE`) | PowerPoint natively supports standard flowchart geometries, allowing for fully editable shapes. |
| Elbow Connectors | `python-pptx` native (`add_connector`) | Provides standard right-angle pathing and programmatic anchoring to specific shape connection sites (top, bottom, left, right). |
| Layout & Spacing | Python Math | By calculating exact XY coordinates based on a central axis, we recreate the snap-to-grid alignment shown in the tutorial perfectly. |

> **Feasibility Assessment**: 100%. The tutorial relies entirely on standard PowerPoint shape and connector tools. `python-pptx` is fully capable of generating this exact layout, complete with anchored elbow connectors and styled shapes, ensuring the result is completely editable by the user.
