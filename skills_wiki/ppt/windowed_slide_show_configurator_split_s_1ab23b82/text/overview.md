# Windowed Slide Show Configurator (Split-Screen Optimized)

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Windowed Slide Show Configurator (Split-Screen Optimized)

* **Core Visual Mechanism**: This is a structural, file-level feature rather than a traditional canvas design. By default, PowerPoint slide shows hijack the entire screen. This skill modifies the internal XML properties (`presProps.xml`) to force the presentation to open in "Browsed by an individual (window)" mode. When combined with an instructional mockup slide, it visually and technically facilitates a side-by-side split-screen workflow.
* **Why Use This Skill (Rationale)**: During remote meetings (Zoom, Teams, Google Meet), presenters with a single monitor often struggle to see their audience/chat while sharing full-screen slides. Configuring the file to launch in a resizable window allows the user to snap the presentation to one half of the screen and their meeting application to the other.
* **Overall Applicability**: Essential for presentations designed for webinars, online teaching, remote product demos, and single-monitor virtual meetings.
* **Value Addition**: Automating this step saves the user from navigating through nested PowerPoint ribbon menus. It guarantees the slide deck is immediately "Zoom-ready" upon delivery.


### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  To make the generated slide visually represent the tutorial's goal, the code generates an infographic-style desktop mockup:
  - **Background (OS Desktop)**: Solid Windows 10 Blue `(0, 120, 215)`.
  - **Window Elements (Mockups)**: Rounded rectangles representing application windows.
    - Video Call Window (Left): White body with a light gray title bar `(240, 240, 240)`.
    - PowerPoint Window (Right): White body with a PowerPoint-brand orange-red title bar `(196, 62, 28)`.
  - **Text Hierarchy**: Bold sans-serif headings for the window titles, and clear sequential instructions for snapping the windows.

* **Step B: Compositional Style**
  - The canvas is divided perfectly in half horizontally (50/50 split).
  - The mockups have generous padding (margins) to simulate the feel of a floating desktop environment.
  - The right-hand PowerPoint mockup contains the actual instructions on how to use the generated file.

* **Step C: Dynamic Effects & Transitions**
  - **Hardware/OS level**: Snap Assist (Windows feature) dragged to the edge of the screen.
  - **File level**: When the user presses F5 to present, instead of an animation taking over the screen, the slide show simply plays within the bounds of its native window frame.
...
|---|---|---|
| Modifying Slide Show Type | `lxml` XML injection | The `python-pptx` library does not expose an API for `<p:showPr>` (Slide Show Setup properties). We must manually iterate the package parts, find `presProps.xml`, and inject the `<p:browse>` XML element directly. |
| Split-Screen Visual Mockup | `python-pptx` native | Simple overlapping shapes (rectangles and text boxes) are sufficient to draw an infographic desktop interface. |

> **Feasibility Assessment**: 100% — The code successfully alters the internal `.pptx` XML so that the resulting file opens in Windowed Mode natively, and draws a representative infographic on the first slide.
