# Dynamic Device Mockup Composer

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Dynamic Device Mockup Composer

* **Core Visual Mechanism**: This pattern simulates a professional presentation "Slide Master" technique (Merge Shapes -> Intersect) to create a perfect device screen placeholder. It pairs a custom-cropped, perfectly rounded image with a crisp, vector-based "bezel" overlaid on top. This is flanked by a symmetrical, dual-aligned layout of icon-based text blocks.
* **Why Use This Skill (Rationale)**: Inserting raw rectangular screenshots often looks unpolished. Embedding them into a realistic device frame instantly anchors the digital content in reality, providing context and scale. The symmetrical layout draws the viewer's eye directly to the central focal point (the screen content).
* **Overall Applicability**: Ideal for app showcases, UI/UX portfolio reviews, software feature introductions, and modern pitch deck product slides.
* **Value Addition**: Transforms a standard screenshot into a premium, styled graphic asset. It brings the high-end "Apple keynote" aesthetic to standard PowerPoint slides without requiring external design software.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  - **The Mockup Centerpiece**: A combination of two layers. A base image precisely cropped to a 9:19.5 aspect ratio with calculated rounded corners, topped by a hollow vector rectangle (the bezel) and a top pill shape (the notch/camera housing).
  - **Color Logic**:
    - Background: Minimalist White `(255, 255, 255)` or very light gray to let the device pop.
    - Bezel/Hardware: Dark Charcoal/Almost Black `(30, 30, 30)` for high contrast.
    - Text: Primary Titles in True Black `(0, 0, 0)`, Secondary text in Medium Gray `(120, 120, 120)`.
  - **Text Hierarchy**: Large bold overarching title -> Symmetrical layout titles (Bold, Medium) -> Symmetrical body text (Regular, Small).

* **Step B: Compositional Style**
  - **Symmetry**: The device anchors the exact center (`X=6.66"` on a standard widescreen).
  - **Opposing Alignments**: Text blocks on the left are right-aligned (with icons pushing towards the center); text blocks on the right are left-aligned. This creates a funnel effect pointing at the phone.
  - **Proportions**: The phone occupies roughly 20% of the slide width but nearly 75% of the slide height, making it a dramatic, vertical focal point.

* **Step C: Dynamic Effects & Transitions**
  - As seen in the tutorial, the most effective animation for this layout is a grouped "Fly In" from the bottom for the text blocks, triggered *after* the phone frame appears. *(Note: Code below focuses on the final static visual layout, as programmatic animation sequencing in PPTX is highly volatile).*
...
|---|---|---|
| Perfectly rounded screen image | PIL/Pillow | `python-pptx` cannot natively apply precise corner radii to picture fills programmatically without deep XML manipulation that often breaks image scaling. Pre-processing the image mask is 100% reliable. |
| Device Bezel & Geometry | `python-pptx` + `lxml` | Native shapes ensure crisp, resolution-independent bezels. `lxml` is used to adjust the internal XML `adj` attribute, syncing the vector corner radius perfectly with the PIL image mask. |
| Symmetrical Typography | `python-pptx` native | Standard text frame APIs handle opposing alignments perfectly. |

> **Feasibility Assessment**: 100%. The code dynamically recreates the exact visual output of the tutorial's Slide Master technique, automatically generating the device bezel and safely cropping any input image to fit perfectly inside the mockup without manual intervention.
