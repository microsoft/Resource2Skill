# Modern Neumorphic Gantt Timeline

## Analysis

Here is the extracted skill strategy and reproduction code based on the provided PowerPoint tutorial transcript and visuals.

### 1. High-level Design Pattern Extraction

> **Skill Name**: Modern Neumorphic Gantt Timeline

*   **Core Visual Mechanism**: The defining visual idea is the "Track and Fill" metaphor. It uses full-width, lightly shaded, recessed rounded rectangles as "background tracks" for each project phase. Vibrantly colored, pill-shaped rectangles overlap these tracks to represent the actual duration of the task. 
*   **Why Use This Skill (Rationale)**: From a cognitive design perspective, the recessed track provides an immediate visual affordance of the *total possible timeline*, while the colored fill clearly dictates current progress or scheduling. The "pill" shape (fully rounded corners) softens the rigid nature of traditional Gantt charts, making executive presentations feel more modern, approachable, and dashboard-like.
*   **Overall Applicability**: Ideal for high-level project roadmaps, quarterly business reviews (QBRs), status updates, and strategic planning decks where exact daily granularity is less important than overall phase visualization.
*   **Value Addition**: Transforms a standard, boring table-based or Excel-pasted Gantt chart into a bespoke, app-like UI component. The neumorphic inner shadow on the background tracks gives the slide depth without clutter.

### 2. Visual Breakdown

*   **Step A: Core Visual Elements**
    *   **Background Tracks**: Very light gray `(235, 238, 240)` rounded rectangles with an **inner shadow** to simulate a recessed groove.
    *   **Active Bars**: Vibrant, varied colors for different phases (e.g., Coral `(238, 108, 77)`, Teal `(61, 90, 128)`, Navy `(41, 50, 65)`). These are pill-shaped (100% border radius) and sit *inside* or exactly on top of the tracks.
    *   **Data Labels**: Month markers (Jan, Feb, Mar) rotated 90 degrees vertically for space efficiency. Phase labels left-aligned in a clean Sans-Serif font (like Century Gothic or Segoe UI).

*   **Step B: Compositional Style**
    *   **Layout**: 
        *   Left 15% width: Row labels (Project Names).
        *   Top 15% height: Time scale (Months).
        *   Remaining 85x85% area: The Gantt tracks.
    *   **Spacing**: Generous vertical whitespace between the tracks (padding is roughly equal to the height of the track itself).

*   **Step C: Dynamic Effects & Transitions**
    *   **Animation (Manual PPT requirement)**: The tutorial utilizes a "Stretch" or "Wipe" entrance animation from the left for the active colored bars to simulate progress loading. *(Note: Code generates the static final layout; animations require manual application or complex XML injection).*
...
|---|---|---|
| **Grid & Shape Layout** | `python-pptx` native | Best for precise placement of rounded rectangles and text boxes across a calculated grid. |
| **Recessed Track (Inner Shadow)** | `lxml` XML injection | `python-pptx` does not have a native API for inner shadows. We must inject the `<a:innerShdw>` DrawingML element directly into the shape properties. |
| **Perfect Pill Shapes** | `python-pptx` native | We can manipulate the `shape.adjustments[0].value` to force the rounded rectangles into a perfect pill shape, regardless of their width. |

> **Feasibility Assessment**: **95%**. The code perfectly recreates the layout, the pill shapes, the colors, and the lxml-driven inner shadows for the recessed tracks. The only missing element is the manual entrance animation (Wipe/Stretch).
