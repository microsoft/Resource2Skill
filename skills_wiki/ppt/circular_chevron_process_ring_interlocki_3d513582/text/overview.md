# Circular Chevron Process Ring (Interlocking Flow)

## Analysis

# Agent Skill Distiller: PPTX Design Strategy Extract

## 1. High-level Design Pattern Extraction

> **Skill Name**: Circular Chevron Process Ring (Interlocking Flow)

* **Core Visual Mechanism**: A cyclic, 6-segment diagram forming a complete ring. Instead of standard flat pie/doughnut slices, each segment features an outward-pointing chevron arrow on one side and an inward-pointing notch on the other. This creates an interlocking, puzzle-like visual effect that drives the eye clockwise.
* **Why Use This Skill (Rationale)**: The circular layout eliminates hierarchy—no step is "first" or "last" in a continuous loop. The chevron cutouts provide innate directional flow (implied motion) without needing separate, cluttered arrow shapes. 
* **Overall Applicability**: Ideal for repeating business cycles (e.g., Agile development loops, supply chain life cycles, continuous improvement processes, customer journeys) where 4 to 8 equal phases need to be presented on a single slide.
* **Value Addition**: Transforms basic lists or flat charts into a premium, highly cohesive infographic. The interconnected nature of the graphics subliminally reinforces the idea that all steps are dependent on one another.

## 2. Visual Breakdown

* **Step A: Core Visual Elements**
  - **Shapes**: Custom interlocking chevron arcs (doughnut slices altered with directional points).
  - **Color Logic (6-Part Palette)**: High-contrast, vibrant jewel tones.
    - Deep Purple: `(89, 44, 130, 255)`
    - Crimson Red: `(186, 20, 76, 255)`
    - Orange: `(235, 126, 31, 255)`
    - Mustard Yellow: `(249, 187, 14, 255)`
    - Slate Blue: `(66, 110, 169, 255)`
    - Navy Blue: `(45, 52, 112, 255)`
  - **Text Hierarchy**: 
    - *Center Label*: Bold, tracked-out central title anchoring the graphic.
    - *Segment Labels*: Step numbers or icons placed directly on the colored arc.
    - *External Details*: Title and body text placed radially outside the ring, aligned relative to their respective segments.

* **Step B: Compositional Style**
  - **Proportions**: The graphic sits strictly in the center, occupying ~65% of the slide height. 
  - **Spatial Feel**: Plenty of negative space around the outside for text. A distinct central void (the doughnut hole) prevents visual heaviness.
  - **Layer Interaction**: Shapes overlap adjacently to form the chevrons, unified by a subtle drop shadow that lifts the entire ring off the flat background.

* **Step C: Dynamic Effects & Transitions**
  - The tutorial relies on manual boolean operations ("Merge Shapes -> Subtract"). In automation, we bypass this manual GUI process entirely by mathematically calculating the vertices of the interlocking arcs.
...
|---|---|---|
| **Interlocking Chevron Arcs** | `python-pptx` FreeformBuilder + Trigonometry | Native `python-pptx` cannot draw interlocking pie slices. Instead of inserting static images, calculating the polygon vertices via `math.cos`/`math.sin` and drawing them as native Freeform shapes keeps them perfectly crisp and editable. |
| **Shape Shadows** | `lxml` XML injection | Standard `python-pptx` cannot apply shadows. We inject `<a:outerShdw>` into the shape properties to give the ring depth. |
| **Radial Text Alignment** | `python-pptx` Layout Math | Calculates text box anchoring points radially outside the ring based on the median angle of each segment. |

> **Feasibility Assessment**: 95%. The code generates perfect, editable interlocking chevron arcs natively in PowerPoint, applies the drop shadow, and lays out the text radially. The only missing 5% is the inclusion of specific custom SVG icons, which are replaced by editable text numbers.
