...
> **Skill Name**: Neumorphic Soft UI Styling

* **Core Visual Mechanism**: Neumorphism (or Soft UI) creates depth not through contrasting colors or hard borders, but through dual-toned drop shadows on a monochromatic surface. The shape fill matches the canvas background exactly. A light shadow (white) is cast towards the top-left, and a dark shadow (cool grey) is cast towards the bottom-right. This creates the illusion that the UI elements are extruded from or indented into the background itself.
* **Why Use This Skill (Rationale)**: This style dramatically reduces cognitive load and visual noise. By removing sharp borders and high-contrast bounding boxes, the viewer's eye is naturally drawn purely to the content (text, icons, charts) rather than the containers holding them. It feels tactile, premium, and highly modern.
* **Overall Applicability**: Perfect for tech presentations, digital product pitches, dashboard visualizations, and "future-forward" corporate decks. It works exceptionally well for infographic slides where process steps or metrics need to be compartmentalized without looking cluttered.
* **Value Addition**: It elevates a standard text-and-box slide into a bespoke, app-like experience. It demonstrates high design maturity and attention to detail, making the presentation feel like an interactive software interface.
...
* **Step A: Core Visual Elements**
  - **Color Logic**: Monochromatic low-contrast base with soft, cool tones.
    - **Background & Shape Fill**: Soft cool gray `(238, 240, 245)`
    - **Dark Shadow**: Muted slate gray `(200, 205, 215, 255)`
    - **Light Shadow**: Pure white `(255, 255, 255, 255)`
    - **Accent Color**: Soft Teal/Mint `(133, 193, 184)` (Used for titles and icons)
    - **Body Text Color**: Slate gray `(122, 133, 153)`
  - **Text Hierarchy**: Very clean. Accent colors are used for primary numbers/icons and high-level headings. Body text is smaller, highly tracked (letter spacing), and uses low-contrast grays rather than pure black.
  - **Shapes**: High-radius rounded rectangles (pills) and perfect circles. Sharp corners are entirely avoided.

* **Step B: Compositional Style**
  - Extreme "white space" (negative space). Elements are given plenty of room to breathe so the shadow blurs don't overlap aggressively.
  - Symmetrical, grid-based layouts. Cards are identical in size and perfectly aligned.
  - Floating pills are often used as top-anchored breadcrumbs or category tags (e.g., "INFOGRAPHIC STYLE").

* **Step C: Dynamic Effects & Transitions**
  - Smooth pan/fade transitions. In PowerPoint, this is best achieved using the "Morph" transition, allowing the soft cards to glide across the screen seamlessly.
...
|---|---|---|
| **Neumorphic Dual Shadows** | `PIL/Pillow` | PowerPoint's native rendering engine does not natively support assigning *multiple* offset blur shadows (light and dark simultaneously) to a single shape without complex overlapping. PIL allows us to programmatically generate pixel-perfect Neumorphic masks with precise Gaussian blurs and composite them into a transparent PNG to use as a slide asset. |
| **Slide Layout & Text Content** | `python-pptx` native | Standard text boxes layered on top of the PIL-generated Neumorphic backgrounds provide crisp, editable text rendering. |

> **Feasibility Assessment**: 95%. The core visual style (the extruded soft UI shapes) is reproduced perfectly using programmatic image generation. Inner shadows (indents) are omitted in this specific code block for simplicity, but the main extruded cards, pills, and exact color palettes are fully functional.
