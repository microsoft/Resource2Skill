# 3D Isometric Stacked Layers Infographic

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: 3D Isometric Stacked Layers Infographic

* **Core Visual Mechanism**: This pattern generates a visually striking 3D isometric stack using geometrically precise 2D polygons. By dividing an isometric block into three distinct faces (Top, Left, Right) and applying a lighting-based color adjustment (brightest on top, darker on the sides), it creates a convincing 3D illusion entirely out of flat vector shapes. Rotated text is perfectly aligned to the angled edges to reinforce the 3D perspective.
* **Why Use This Skill (Rationale)**: The isometric perspective breaks the monotony of flat 2D slides, making the information hierarchy spatially intuitive. Stacking visually communicates architectural layers, sequential progression, or tech stacks (e.g., "front-end to back-end").
* **Overall Applicability**: Ideal for Tech Stack Diagrams, Business Process Frameworks, Maturity Models, and Layered Architecture presentations.
* **Value Addition**: It elevates a simple bulleted list into an engaging, "app-icon-like" 3D graphic. Because it's constructed with native PPTX vector shapes rather than 3D models, it remains crisp at any scale and is lightweight.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  * **Layer Blocks**: Composed of 3 polygons (a diamond for the top, and two parallelograms for the sides).
  * **Color Logic**: A base hue is assigned to each layer. The Top face uses the base color, the Left face is darkened by 15%, and the Right face is darkened by 30% to simulate a light source coming from the top-left.
  * **Typography**: Flat description text floats cleanly beside the stack, while an index number (e.g., "01") is mathematically rotated (approx. 26.5°) to lie perfectly flat against the left isometric plane.

* **Step B: Compositional Style**
  * **Central Axis**: The blocks are centrally aligned and stacked bottom-to-top (rendered in reverse order so higher blocks visually overlap lower ones).
  * **Negative Space Space**: A precise gap between blocks creates a "floating" exploded-view effect, making each layer distinct.

* **Step C: Dynamic Effects & Transitions**
  * *Code achievable*: The 3D geometry is perfectly baked into the vectors.
  * *PPTX manual*: Applying a "Wipe" (from Top or Bottom) or "Fly In" (from Bottom) animation to each group sequentially enhances the exploded-view build-up.
...
|---|---|---|
| **Isometric 3D Blocks** | `python-pptx` (FreeformBuilder) | Drawing custom polygons using `build_freeform` ensures the blocks are high-quality, editable vector shapes rather than static images. |
| **Simulated Lighting** | Python Math | Dynamically scaling RGB values (e.g., `r * 0.85`) perfectly replicates the shading of a 3D light rig without the complexity of actual 3D rendering. |
| **Isometric Text Alignment** | `python-pptx` (Rotation) | Rotating a 2D textbox by the exact angle of the isometric grid (`atan2(dy, dx)`) seamlessly aligns the text to the 3D face. |

> **Feasibility Assessment**: 100% reproducible. By calculating the orthographic projection math for a 30°/26.5° isometric grid, we can perfectly recreate the exact geometry and aesthetic of the tutorial using native python-pptx shapes.
