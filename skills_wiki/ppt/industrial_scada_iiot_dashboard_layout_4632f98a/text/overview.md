# Industrial SCADA & IIoT Dashboard Layout

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Industrial SCADA & IIoT Dashboard Layout

* **Core Visual Mechanism**: This design replicates the look and feel of a Supervisory Control and Data Acquisition (SCADA) system or Industrial IoT (IIoT) dashboard. Its signature is a highly structured, compartmentalized layout: a top row dedicated to circular KPI gauges (Overall Equipment Effectiveness, Quality, etc.), a central "digital twin" area featuring a flat, schematic representation of a physical process (conveyor belts, sensors, robotic arms), and a bottom data grid for real-time tag monitoring. 

* **Why Use This Skill (Rationale)**: Industrial dashboards are designed for rapid cognitive processing. They use strict structural hierarchy and universally recognized status colors (Green = Running/Optimal, Red = Stopped/Error, Amber = Warning) to allow an operator (or audience) to grasp system health in seconds. Translating this to a presentation slide creates an instantly recognizable, highly authoritative data visualization format.

* **Overall Applicability**: This layout is highly effective for:
  - Smart factory / Industry 4.0 presentations
  - Manufacturing or supply chain performance reports
  - Digital transformation and IoT platform pitches
  - Technical process flow explanations

* **Value Addition**: Instead of presenting manufacturing or process data in standard bullet points or basic bar charts, this layout visually grounds the data in its physical context (the "machine" view), making the information feel real-time, actionable, and technologically advanced.
...
* **Step A: Core Visual Elements**
  - **Color Logic**: 
    - Backgrounds: Functional, low-contrast greys to reduce eye strain and make status lights pop. Main BG: `(230, 230, 235)`, Panel BGs: `(200, 200, 205)`.
    - Status Indicators (RAG): Optimal Green `(46, 204, 113)`, Warning Amber `(241, 196, 15)`, Critical Red `(231, 76, 60)`.
    - Machinery: Dark greys `(80, 80, 85)` and structural blues `(52, 152, 219)`.
  - **Typography**: Sans-serif, heavily weighted towards numbers. Metric values are large and bold; labels are small and muted.
  - **Widgets**: Circular progress rings (donuts) for percentages, digital segment displays for counts, and simulated LED indicator lights.

* **Step B: Compositional Style**
  - **Zoning**: The canvas is rigidly divided.
    - **Top 25%**: High-level KPIs (The "Executive View").
    - **Middle 50%**: The Process Schematic (The "Operational View").
    - **Bottom 25%**: The Data Grid (The "Engineering View").
  - **Symmetry & Alignment**: Highly orthogonal. Elements are aligned to a strict grid, mimicking engineering software.

* **Step C: Dynamic Effects & Transitions**
  - *In the source video*, this is a live web app with changing numbers and moving boxes.
  - *In PowerPoint*, this translates to static representations of a specific moment in time. However, to simulate the "live" feel, one could duplicate the slide, change the gauge levels and box positions, and use the "Morph" transition to animate the factory process across slides.
...
|---|---|---|
| Panel Layout & Machine Schematic | `python-pptx` shapes | Basic rectangles, lines, and circles are perfect for the flat, schematic look of SCADA machinery. |
| KPI Gauges (Donut Rings) | `PIL` (Pillow) image generation | Native PPT doughnut charts can be clunky to size and format via code. Generating clean, anti-aliased transparent PNG rings via PIL ensures a pixel-perfect "UI widget" look. |
| Data Grid | `python-pptx` Table | Native tables are the best way to represent structured tag/log data at the bottom of the dashboard. |
| Widget Text & Metrics | `python-pptx` TextBoxes | Layered over the PIL rings so the numbers remain easily editable in PowerPoint. |

> **Feasibility Assessment**: 90% reproduction of the static visual aesthetic. The code accurately generates the three-tier SCADA layout, custom KPI donut widgets, the schematic machine drawing (conveyor, box, sensors), and the data table. The interactive web elements (live sliders, real-time animation) obviously cannot be natively compiled into static PPTX, but the visual style is captured perfectly.
