# Alternating Horizontal Process Flow

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Alternating Horizontal Process Flow

* **Core Visual Mechanism**: A linear chronological progression is visualized through a central, horizontal "spine" (axis). From this spine, nodes alternate upwards and downwards, branching into detailed information cards. The cards feature a distinct split-color layout (dark thematic header, light body) to establish a strong structural hierarchy.
* **Why Use This Skill (Rationale)**: Standard bullet points or vertical lists fail to convey a sense of continuous process or timeline. This alternating layout maximizes horizontal slide real estate (especially on 16:9 formats) without cluttering. The zig-zag rhythm allows text boxes to remain wide enough to hold multi-line bullet points without colliding into each other.
* **Overall Applicability**: Ideal for manufacturing steps, project timelines, strategic roadmaps, software deployment phases, or any sequential logic containing 4 to 7 distinct steps with associated descriptive text.
* **Value Addition**: Transforms a boring sequence of text into a scannable, architectural diagram. The use of floating cards with drop shadows adds a premium, "dashboard-like" aesthetic often found in professional management consulting decks.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  - **Color Logic**:
    - Primary Structural Color: Deep Slate Teal `(38, 64, 72, 255)` — used for headers, node borders, and primary typography.
    - Axis/Connection Line Color: Muted Steel Blue `(160, 180, 190, 255)`.
    - Background: A very subtle gradient transition from pure white to a light cool grey `(245, 247, 248, 255)` to ensure the white cards pop.
  - **Text Hierarchy**: Large bold title, subdued grey subtitle. Card headers are bold and white; card bodies contain smaller, dark-grey bulletized text.

* **Step B: Compositional Style**
  - **Spatial Feel**: Structured, symmetrical, and linear. The central axis acts as an anchor.
  - **Proportions**: The central line sits roughly at `55%` down the vertical canvas. The cards are approximately `15%` of the slide width, spaced evenly across the horizontal bounds (leaving ~10% padding on the left/right margins).

* **Step C: Dynamic Effects & Transitions**
  - **Floating Effect**: Achieved via OOXML (`lxml`) injected drop shadows on the card bodies, elevating them slightly above the background and connector lines.
  - **Directionality**: Re-enforced by an injected right-facing arrowhead on the central spine.
...
|---|---|---|
| Background Gradient | PIL / Pillow | `python-pptx` cannot generate pixel-perfect linear gradients natively; PIL guarantees a smooth, high-quality base layer. |
| Shapes, Layout & Text | `python-pptx` native | Drawing rectangles, lines, ovals, and textframes is fully supported and easiest to control programmatically. |
| Drop Shadows | `lxml` XML injection | Native `python-pptx` does not expose the shape shadow API. Manipulating the Open XML directly enables exact blur, distance, and alpha control. |
| Line Arrowhead | `lxml` XML injection | Applying standard arrowheads to native connector shapes requires XML modification for reliability. |

> **Feasibility Assessment**: 95%. The layout, spacing, shadow effects, and typography perfectly replicate the structural and aesthetic intent of the tutorial's process flows. 
