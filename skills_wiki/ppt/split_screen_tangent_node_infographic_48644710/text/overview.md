# Split-Screen Tangent Node Infographic

## Analysis

### 1. High-level Design Pattern Extraction

> **Skill Name**: Split-Screen Tangent Node Infographic

* **Core Visual Mechanism**: A stark vertical split background divided by a continuous central axis. The design uses intersecting geometric primitives (nested offset circles) to create the illusion of custom "cut-out" crescent shapes. Branches originate from intersection points and terminate precisely at the tangent lines of the circular nodes, creating an incredibly satisfying, interconnected flow.
* **Why Use This Skill (Rationale)**: The 50/50 split creates natural contrast and comparison (e.g., Before/After, Product A/B, Pros/Cons). The tangent connecting lines draw the eye sequentially from the center out into the content branches, making dense information feel organized and narrative-driven. 
* **Overall Applicability**: Perfect for comparison slides, timeline milestones, dual-feature showcases, or high-level strategic agendas. 
* **Value Addition**: Transforms a standard bullet-point list into an integrated infographic. The "cut-out" crescent technique visually merges the images into the background rather than letting them float arbitrarily, elevating the presentation's production value.

### 2. Visual Breakdown

* **Step A: Core Visual Elements**
  - **Color Logic**: High-contrast dual tone. 
    - Vivid Red accent: `(238, 59, 36)`
    - Dark Navy/Gray background: `(43, 50, 60)`
    - Pure White unifier: `(255, 255, 255)`
  - **Typography**: Clean sans-serif hierarchy. A prominent central badge holds the category numbers ("01", "02").
  - **Geometry Tricks**: By placing a circular image offset inside a slightly larger white circle, a perfect crescent frame is created without needing complex boolean shape subtractions.

* **Step B: Compositional Style**
  - Exact 50% split down the center (`X = 6.66"` on a 16:9 slide).
  - The left timeline drops downward, while the right timeline flows upward, balancing the visual weight dynamically in opposing quadrants.
  - Branches connect precisely to the tangency points of the image outer rings.

* **Step C: Dynamic Effects & Transitions**
  - This layout benefits immensely from "Push" or "Pan" slide transitions, emphasizing the continuous connected vertical line across multiple slides.
...
|---|---|---|
| **Circular Images** | `PIL/Pillow` | Native python-pptx cannot dynamically center-crop and mask web images into perfect transparent circles. |
| **Crescent Frame Cutout** | Precise Shape Z-ordering | Offsetting a PIL image inside a white PPTX shape creates the visual effect of "Merge Shapes -> Subtract" without complex XML injections. |
| **Connecting Lines & Layout** | `python-pptx` native | Standard connectors ensure crisp lines, perfectly positioned to touch shape tangents via coordinate math. |

> **Feasibility Assessment**: 100% of the core visual aesthetic is reproduced. The layout's structural integrity, tangent line logic, and boolean-like cutouts are fully automated.
