...
> **Skill Name**: EDM Ghost Kick Sidechain Pump & Build

* **Core Musical Mechanism**: The defining characteristic of this pattern is the "four-on-the-floor" rhythmic ducking (pumping) applied to sustained elements like synths, pads, or basslines. The volume of the sustained chords drops sharply on every downbeat (1, 2, 3, 4) and swells back up, creating a driving, breathing rhythm. In EDM, this is traditionally achieved by routing a muted "ghost kick" to a sidechain compressor on the chord track. 
* **Why Use This Skill (Rationale)**: This creates intense rhythmic momentum without cluttering the arrangement. By ducking the chords, you open up the frequency spectrum for the kick drum's low-end transient, preventing muddy mixes. Even when the kick is muted (as shown in the tutorial's verse/build arrangement), the psychoacoustic memory of the dance beat remains, keeping the listener physically engaged through "implied" rhythm.
* **Overall Applicability**: Essential for Electronic Dance Music (House, Trance, Future Bass, Techno). It is highly effective in Intros and Pre-Chorus Builds where you want to tease the energy of the drop, or in Verses where the main drums are pulled out to create contrast before hitting hard.
* **Value Addition**: This skill transforms static, lifeless chords into a driving rhythmic sequence. It encodes both standard EDM harmonic progression logic (i-VI-III-VII) and automated volume shaping to guarantee a perfect sidechain pump effect without complex compressor calibration.

### 2. Technical Breakdown

* **Step A: Rhythm & Timing**
  - **Tempo**: 120-130 BPM (Standard House/EDM tempo).
  - **Grid**: 4/4 time signature.
  - **Pumping Rhythm**: A volume dip occurs exactly on every quarter note (1/4 grid). The volume curves back up (attack) over the next 1/8th or 1/16th note, holding until the next downbeat. 

* **Step B: Pitch & Harmony**
  - **Key/Scale**: Typically Minor (Aeolian). 
  - **Progression**: A driving, emotional EDM sequence like i - VI - III - VII (e.g., in C minor: Cm, Ab, Eb, Bb).
  - **Voicing**: 4-note chord voicings spanning two octaves (a deep root note for presence, plus the triad above it) sustained for a full bar each.

* **Step C: Sound Design & FX**
  - **Ghost Kick**: A basic synthesized kick (sine wave with a fast pitch drop and short decay).
  - **Pumping Chords**: A synthesizer playing a saw wave, smoothed slightly with EQ.
  - **The "Pump"**: Simulated via precise Volume automation on the chord track. While sidechain compression is the traditional routing method, volume modulation is frequently used in modern production (via tools like LFOTool or Kickstart) for a cleaner, perfectly mathematically timed pump.

* **Step D: Mix & Automation**
  - The Chords track has a volume envelope shaped like a reverse-sawtooth on every beat.
  - The Kick track is provided to anchor the rhythm and can be muted to achieve the pure "ghost kick" build effect discussed in the video.
...
|---|---|---|
| EDM Chord Progression | MIDI note insertion | Allows parameterized key/scale generation for exact 4-bar progressions. |
| Synth Sound Design | FX chain (ReaSynth) | Uses stock REAPER tools to generate the necessary pad and kick tones without external VSTs. |
| "Sidechain" Pumping Effect | Track Volume Envelope Automation | While the video uses ReaComp auxiliary routing, native API routing can be brittle across system setups. Injecting exact volume envelope points provides a 100% reliable, mathematically perfect "pump" that achieves the exact same audio result demonstrated in the tutorial. |
| Four-on-the-floor Kick | MIDI note insertion | Creates the foundational groove for the sidechain timing context. |

> **Feasibility Assessment**: 95% reproduction. The code perfectly generates the rhythmic pumping, the EDM chord progression, and the foundational kick drum. The only deviation is using volume automation instead of active audio-triggered sidechain compression, ensuring foolproof execution in the API while matching the sonic result identically.
