...
> **Skill Name**: Tension-Building Subtractive Transition (The "Drop Gap")

* **Core Musical Mechanism**: Creating structural contrast by employing subtractive arrangement. During the build-up or verse, high frequencies are filtered out (low-pass filter) and rhythmic elements are sparse (half-time hi-hats). Just before the chorus/drop, the drum track is entirely muted for 1-2 beats (creating a "vacuum" or gap), and the filter rapidly sweeps open to reintroduce the full frequency spectrum exactly as the heavy, full-time beat hits.
* **Why Use This Skill (Rationale)**: This is a fundamental psychoacoustic trick. By removing elements (low-end kicks, high-end frequencies, dense hi-hats) over time, the listener's ear adjusts to a smaller, "muffled" sonic space. The sudden silence (gap) right before the drop resets the ear's baseline, making the immediate re-entry of the un-filtered, full-frequency chorus feel exponentially more massive and hard-hitting. 
* **Overall Applicability**: Essential for modern hip-hop, trap, EDM, and pop arrangements. It transforms a static, monotonous loop into a dynamic song structure with clear "verse" and "chorus" distinctions.
* **Value Addition**: Instead of just generating a flat loop, this skill encodes *arrangement dynamics*. It demonstrates how to time-stretch patterns (half-time vs. full-time hats), how to carve out space using silence, and how to automate an FX envelope curve to build anticipation.

### 2. Technical Breakdown

* **Step A: Rhythm & Timing**
  - **Tempo**: Typically 120-140 BPM (Trap/Hip-hop groove).
  - **Structure**: 8 Bars total. 
    - *Bars 1-2*: 1st half of verse (no hi-hats).
    - *Bars 3-4*: 2nd half of verse (half-time quarter-note hi-hats). Last 2 beats of Bar 4 are completely silenced (the "drop gap").
    - *Bars 5-8*: Chorus/Drop (full 8th-note hi-hats, full kick/snare pattern).
* **Step B: Pitch & Harmony**
  - **Progression**: A simple 4-bar progression (e.g., i - VI - III - VII in minor).
  - Sustained, long MIDI notes are used to create a "Pad" foundation that clearly demonstrates the filter sweep effect.
* **Step C: Sound Design & FX**
  - **Pad Track**: `ReaSynth` configured with a slow attack and high sustain to act as a pad, followed by `JS: Filters/resonantlowpass`.
  - **Drum Track**: `ReaSynth` configured with a near-zero attack and fast decay to act as a percussive placeholder.
* **Step D: Mix & Automation (if applicable)**
  - A parabolic/curved automation envelope (`Shape 2: Slow start/end`) is applied to the Cutoff parameter of the JS Lowpass filter on the Pad track. 
  - The envelope starts heavily muffled (~15% open), slowly rises through the verse, and ramps up exponentially to 100% open just a fraction of a second before the drop at Bar 5.
...
|---|---|---|
| Subtractive Arrangement | MIDI note insertion logic | Precise control over which beats receive notes (omitting beats for the "gap" and half-time hi-hats). |
| Filter Sweep | FX Automation Envelope (`RPR_GetFXEnvelope`) | Allows for smooth, parabolic curves (Shape 2) that build tension gradually before spiking at the drop. |
| Sound Sources | `ReaSynth` parameter tuning | Guarantees self-contained playback without relying on external VSTs or sample packs, by tweaking stock synth envelopes. |

> **Feasibility Assessment**: 100% reproducible. The arrangement structuring, track separation, half-time logic, and filter automation envelope can all be precisely executed using the native REAPER API.
