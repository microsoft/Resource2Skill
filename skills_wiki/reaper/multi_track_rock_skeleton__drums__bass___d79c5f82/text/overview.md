...
> **Skill Name**: Multi-Track Rock Skeleton (Drums, Bass, Rhythm, Lead)

* **Core Musical Mechanism**: This pattern generates a tightly synchronized, four-part multi-track MIDI arrangement (Drums, Bass, Rhythm Guitar, and Lead Guitar). It employs a driving 8th-note bassline, syncopated power chords, continuous 16th-note arpeggios, and a standard rock backbeat drum groove.
* **Why Use This Skill (Rationale)**: The core tutorial emphasizes optimizing REAPER's MIDI Editor for viewing and editing multiple instruments simultaneously (using "ghost notes" and track-color coding). This skill provides the exact musical template required to utilize that workflow. Musically, it uses a timeless `i - VI - III - VII` progression, capitalizing on rhythmic interlocking (the kick drum and rhythm guitar hit the syncopated 8th-note push before beat 3, while the bass and hi-hat maintain the anchor). 
* **Overall Applicability**: Ideal for rock, metal, synthwave, or orchestral mockups. It serves as a structural foundation where multi-track MIDI referencing is essential to ensure harmonies don't clash and rhythmic hits are tightly aligned across the ensemble.
* **Value Addition**: Instead of a single blank track, this injects a complete band ecosystem. It establishes four pre-colored tracks and populates them with a mathematically correct, key-locked scale progression, immediately demonstrating how multi-track MIDI relationships work in REAPER.

### 2. Technical Breakdown

* **Step A: Rhythm & Timing**
  - **Time Signature/BPM**: 4/4 time, optimally around 120-130 BPM.
  - **Drums**: Kick on `1`, `2.5`, and `3`. Snare on `2` and `4`. Hi-hats on steady 8th notes.
  - **Bass**: Straight 8th-note driving pulse.
  - **Rhythm GTR**: Syncopated strumming (dotted quarter, dotted quarter, quarter).
  - **Lead GTR**: Flowing 16th-note sweep arpeggios covering 4 notes per beat.

* **Step B: Pitch & Harmony**
  - **Progression**: `i - VI - III - VII` (Roots: scale degrees 0, 5, 2, 6).
  - **Voicings**: 
    - Bass: Single root notes (Octave 2).
    - Rhythm: Open power chords (Root + Perfect 5th + Octave) spanning Octave 3 to 4.
    - Lead: Broken triads plus octave (Root, 3rd, 5th, Octave) in Octave 4.
  - All pitches are computed dynamically from user-defined root keys and scales (defaults to B minor).

* **Step C: Sound Design & FX**
  - The script relies strictly on MIDI notes mapped to standard GM parameters (Drums on Channel 10, pitches 36, 38, 42, 49) so that it is instantly plug-and-play with any standard drum sampler (like MT Power DrumKit or ReaSamplOmatic5000) and synth VSTs.
  - Custom track colors (Indigo, Purple, Orange, Teal) are explicitly set via ReaScript to match the visual workflow taught in the tutorial.

* **Step D: Mix & Automation**
  - Four distinct tracks are grouped together visually. (To fully experience the tutorial's workflow, ensure REAPER's preferences are set to: *Options > Preferences > MIDI Editor -> One MIDI editor per project*, and *Selection is linked to visibility*).
...
|---|---|---|
| **Multi-track Architecture** | `RPR_InsertTrackAtIndex` & `RPR_SetMediaTrackInfo_Value` | Necessary to create the four distinct instruments and color-code them for visual separation in the MIDI editor. |
| **Synchronized Arrangement** | `RPR_CreateNewMIDIItemInProj` | Creates clean, bounded MIDI items on exactly the same timeline across all 4 tracks. |
| **Harmonic & Rhythmic Generation** | `RPR_MIDI_InsertNote` with scale algorithms | Provides precise control over the 16th-note arpeggios, syncopated power chords, and drum mappings without needing external MIDI files. |

> **Feasibility Assessment**: 100% reproducible. The script perfectly recreates the multi-track MIDI skeleton seen at 07:27 in the tutorial using native ReaScript algorithms and standard REAPER track properties.
