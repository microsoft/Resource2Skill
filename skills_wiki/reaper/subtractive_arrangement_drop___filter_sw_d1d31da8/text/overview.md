...
> **Skill Name**: Subtractive Arrangement Drop & Filter Sweep Transition

* **Core Musical Mechanism**: This arrangement technique creates a transition between a high-energy section (Chorus) and a lower-energy section (Verse) using three simultaneous moves:
  1. **Subtractive Beat Drop**: Muting the Kick drum on the very first downbeat of the new section.
  2. **Rhythmic Halftime**: Stretching the hi-hat pattern from fast (1/8th notes) to slow (1/4 notes).
  3. **Low-Pass Filter Sweep**: Automating an EQ to muffle the high frequencies in the final bar before the drop, snapping back to full brightness exactly on the downbeat.

* **Why Use This Skill (Rationale)**: In modern music production, "deleting things is adding things." Instead of piling on crash cymbals and massive risers, removing foundational elements (like the kick drum) creates a psychoacoustic moment of "weightlessness." The listener's brain expects the kick on the '1'; denying it creates immense groove tension. Preceding this with a low-pass filter sweep restricts the frequency spectrum, making the eventual return of the high-end feel incredibly explosive by contrast.

* **Overall Applicability**: Essential for beatmakers, hip-hop producers, and EDM artists transitioning from intros to drops, or choruses to verses. It keeps the listener engaged without cluttering the arrangement with transition FX.

* **Value Addition**: This skill transforms a static 8-bar loop into a dynamic, two-part arrangement with professional tension-and-release mechanics, automatically handling the automation curves and MIDI note permutations.

### 2. Technical Breakdown

* **Step A: Rhythm & Timing**
  - **BPM**: 120-140 (Standard Trap/Hip-Hop/Pop).
  - **Grid**: 4/4 time signature.
  - **Pattern**:
    - *Bars 1-4 (Chorus)*: 1/8th note hi-hats. Kick on 1, 2.5, and 3.
    - *Bars 5-8 (Verse)*: 1/4 note hi-hats (halftime feel). Kick skips beat 1, enters on 2.5 and 3.

* **Step B: Pitch & Harmony**
  - Uses the provided `key` and `scale` to generate a continuous foundational chord progression (e.g., i - v) for the synth pad to ensure the filter sweep has harmonic content to act upon.

* **Step C: Sound Design & FX**
  - **Instrument**: Stock `ReaSynth` for the pad, basic MIDI notes for the drums.
  - **FX**: `ReaEQ` placed on the Synth pad. Band 4 (High Shelf) is used to simulate a low-pass filter. 

* **Step D: Mix & Automation**
  - **Envelope**: Automating ReaEQ Band 4 Gain.
  - **Shape**: Drops from 0dB (val = 0.5) down to max cut (val = 0.0) over the final bar of the chorus, using a curved bezier shape for an accelerating "suck out" effect, then immediately snapping back to 0dB on the downbeat of the verse.
...
|---|---|---|
| Rhythmic variations (Halftime & Kick Drop) | MIDI note insertion | Allows programmatic modification of the loop based on the current bar number. |
| Subtractive Drop impact | Omitted MIDI note | Easiest way to reproduce the "missing kick on the 1" arrangement trick. |
| Filter Sweep | ReaEQ FX Envelope Automation | Native, cross-platform way to restrict high-frequencies to build tension. We automate the High Shelf gain down to simulate the filter cut. |

> **Feasibility Assessment**: 100% reproducible. The arrangement tricks (MIDI drop, halftime, and envelope automation) rely entirely on core REAPER functionalities and basic music theory logic.
