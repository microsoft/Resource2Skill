...
> **Skill Name**: Rule of 3 (AAB Melodic/Harmonic Variation)

* **Core Musical Mechanism**: The "Rule of 3" states that a musical motif should be introduced (1st time), reinforced through repetition (2nd time), and then intentionally altered or abandoned on the 3rd iteration. The variation can be a completely new idea, or it can start the same but end differently. This skill implements the "start the same, end differently" technique (A - A - A' - B phrase structure).

* **Why Use This Skill (Rationale)**: The human brain thrives on recognizing patterns, but it quickly habituates (gets bored) once a pattern is entirely predictable. The first play of a motif captures attention. The second play confirms it is a deliberate pattern. By the third time, the brain anticipates the exact same result and tunes out. Introducing a variation exactly at this point provides a "dopamine hit" of surprise, re-engaging the listener's attention and pushing the song's momentum forward.

* **Overall Applicability**: This principle is universally applicable across all genres. It is foundational for writing compelling lead melodies, chord progressions (e.g., standard 4-bar or 8-bar pop loops), drum fills (playing a standard beat for 3 bars and a fill on the 4th), and arrangement structures. 

* **Value Addition**: A standard looped MIDI clip is static and causes listener fatigue. This skill encodes functional variation, transforming a basic loop into a dynamic 8-bar musical phrase with built-in tension and resolution.

### 2. Technical Breakdown

* **Step A: Rhythm & Timing**
  - **Tempo Range**: 110-130 BPM (120 BPM default).
  - **Grid/Structure**: 8-bar phrase. We divide the idea into 2-bar motifs.
    - Bars 1-2: Iteration 1 (Establish)
    - Bars 3-4: Iteration 2 (Reinforce)
    - Bars 5-6: Iteration 3 (Vary/Branch off)
    - Bars 7-8: Iteration 4 (Resolve)
  - **Note Duration**: Chords play as whole notes. The melody plays an arpeggiated 1/4 and 1/8 note rhythm that follows the variation.

* **Step B: Pitch & Harmony**
  - **Harmony (Relative Degrees)**: 
    - Iteration 1 & 2: IV → I (e.g., F major to C major)
    - Iteration 3 (Variation): IV → V (e.g., F major to G major)
    - Iteration 4 (Resolution): vi → I (e.g., A minor to C major)
  - **Melody**: Starts with ascending intervals matching the chords, but on the 3rd iteration, the melody diverges upwards to match the new dominant (V) chord, creating tension before resolving in the final iteration.

* **Step C: Sound Design & FX**
  - **Instrument**: 2 separate tracks using REAPER's stock `ReaSynth`.
  - **Mix**: The chord track volume is reduced to 40% to act as a backing pad, while the melody track is louder and panned slightly to stand out.
...
|---|---|---|
| Rhythmic Variation | MIDI Note Insertion (`RPR_MIDI_InsertNote`) | Allows precise programmatic control over when the pattern stays the same and when it branches off based on the bar number. |
| Harmonic/Melodic Scaling | Relative Scale Indexing | Ensures the pattern works in any key/scale by mathematically deriving pitches instead of hardcoding MIDI notes. |
| Sound generation | `ReaSynth` Track FX | Provides a self-contained, native sound without relying on external VSTs or audio files. |

> **Feasibility Assessment**: 100% — The theoretical concept described in the tutorial (The Rule of 3 variation) is perfectly replicable via programmatic MIDI generation and standard diatonic harmony.
