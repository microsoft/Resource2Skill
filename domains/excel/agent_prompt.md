...
`get_skill_code`, and `get_skill_visual` before building. After that,
commit — either execute a task-matched skill or, if none matches, go straight
to `execute_xlsx_code`. Don't loop on discovery.
...
2. For the best task-matched executable candidate, read the multimodal bundle:
   ...
   `get_skill_visual(skill_id)`. Keep these calls in the trace. If a visual
   path is missing, continue after observing that result.
...
4. Create the workbook, then either attempt the matching skill with the strongest
   available execution path:
   - T5 / archetype: `apply_skill(...)` or `init_from_archetype(...)`
...
6. Only go directly to `execute_xlsx_code` when you have inspected the relevant
   skill(s) and the call is grounded with provenance. Final save requires at
   least 3 grounded workbook roles: layout, charts/tables, formulas/data/model,
   or formatting/reporting polish.

Only use a workbook archetype when the skill name and category directly match
the requested workbook. Do **not** use a budget/actuals or task-management
archetype for unrelated marketing, lab, RFM, fundraising, inventory, HR, or
SLA workbooks. If the match is weak, skip the archetype and build the exact
sheet structure with `execute_xlsx_code`, using component/shell skills only as
visual references.
...
apply_skill(skill_id, target_id=workbook_id, kwargs_json='{...}')  # preferred wiki dispatch when available
init_from_archetype(workbook_id, archetype_id, kwargs_json='{...}')   # if a multi-sheet archetype matches
# OR
...
# Tiered skills
apply_skill(skill_id, target_id, kwargs_json)       # universal wiki dispatch for T3/T4/T5
init_from_archetype(workbook_id, archetype_id, kwargs_json)
