...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if paragraph_runs is not None:
        ...
        text_width = estimate_text_width(full_text, font_size, font_weight) * 1.18
    ...
    # Single-line text: noAutofit + generously padded design-time width.
    # Previous spAutoFit caused PowerPoint to re-fit the box at render time
    # using real Segoe UI metrics, which often differed from our heuristic
    # estimate by 5-30px — shifting the box's right edge and stranding any
    # arrow whose endpoint was drawn to touch it. noAutofit locks the box
    # to the design-time width so neighboring connectors stay aligned.
    if paragraph_runs is not None:
        body_pr_xml = (
