...
def _is_motion_path_effect(name: str) -> bool:
    """R2S motion-path effects emit <p:animMotion> and must only attach to
    explicitly-marked anchor groups, not every top-level shape."""
    return isinstance(name, str) and name.startswith('r2s_')
...
def _build_sequence_targets(
    ...
    # When the slide-level animation is a motion-path effect (r2s_*), only
    # the explicitly-marked motion-anchor-* groups should receive motion.
    # Otherwise the orbit/spin/pulse path gets applied to copy blocks and
    # decorative groups, which makes the whole composition appear to wobble.
    if _is_motion_path_effect(animation):
        filtered = [
            (sid, svg_id) for (sid, svg_id) in anim_targets
            if isinstance(svg_id, str) and svg_id.startswith('motion-anchor-')
        ]
        if filtered:
            anim_targets = filtered
