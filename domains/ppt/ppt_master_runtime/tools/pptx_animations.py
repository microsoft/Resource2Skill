...
Supported entrance/emphasis animations (per-element):
...
Supported motion-path animations:
    r2s_orbit, r2s_bounce_path, r2s_reveal_sweep, r2s_spin, r2s_pulse
...
ANIMATIONS: Dict[str, Dict[str, Any]] = {
    ...
    'r2s_orbit': {
        'name': 'R2S Orbit Motion Path',
        'kind': 'motion',
        'filter': None,
        'presetID': 14,
        'presetClass': 'path',
        'presetSubtype': 0,
        # Continuous elliptical orbit. Coordinates are normalized fractions of
        # the slide (1280×720 → rx≈160px, ry≈112px). This is sized to match the
        # visible orbit ring the agent draws as decoration; the orb starts at
        # the ring's right edge and traces the full ring counter-clockwise.
        'path': 'M 0 0 a 0.12500 0.15600 0 1 1 -0.25000 0 a 0.12500 0.15600 0 1 1 0.25000 0',
        'repeat': 'indefinite',
        'duration': 6.0,
    },
    'r2s_bounce_path': {
        'name': 'R2S Bounce Motion Path',
        'kind': 'motion',
        'filter': None,
        'presetID': 14,
        'presetClass': 'path',
        'presetSubtype': 0,
        # Arc that climbs and falls across roughly the right half of the slide
        # (~512px horizontal sweep, ~144px vertical amplitude). Visible bounce.
        'path': 'M 0 0 L 0.10000 -0.20000 L 0.20000 0.00000 L 0.30000 -0.13000 L 0.40000 0.00000',
        'repeat': 'indefinite',
        'autoRev': True,
        'duration': 2.6,
    },
    'r2s_reveal_sweep': {
        'name': 'R2S Reveal Sweep Motion Path',
        'kind': 'motion',
        'filter': None,
        'presetID': 14,
        'presetClass': 'path',
        'presetSubtype': 0,
        # Wide horizontal sweep (~410px) that returns; covers the whole reveal
        # band agent draws (left curtain to right curtain).
        'path': 'M -0.16000 0 L 0.16000 0',
        'repeat': 'indefinite',
        'autoRev': True,
        'duration': 2.8,
    },
    'r2s_spin': {
        'name': 'R2S Spin With Motion Path',
        'kind': 'motion_rotate',
        'filter': None,
        'presetID': 14,
        'presetClass': 'path',
        'presetSubtype': 0,
        # Spin in place — motion path is a tiny no-op orbit so the shape stays
        # put; the rotateBy attribute does the actual spinning (full revolution
        # over the duration).
        'path': 'M 0 0 a 0.00200 0.00200 0 1 1 -0.00400 0 a 0.00200 0.00200 0 1 1 0.00400 0',
        'repeat': 'indefinite',
        'duration': 4.0,
        'rotateBy': -21600000,
    },
    'r2s_pulse': {
        'name': 'R2S Pulse With Motion Path',
        'kind': 'motion_scale',
        'filter': None,
        'presetID': 14,
        'presetClass': 'path',
        'presetSubtype': 0,
        # Tiny vertical jiggle so the path is non-zero; scaleTo does the
        # actual pulse (112% of original size, ping-ponged via autoRev).
        'path': 'M 0 0 L 0 -0.00400',
        'repeat': 'indefinite',
        'autoRev': True,
        'duration': 1.6,
        'scaleTo': 112000,
...
def _build_effect_xml(
    ...
    if anim_info.get('duration') is not None:
        duration_ms = int(float(anim_info['duration']) * 1000)
  ...
  <p:to><p:strVal val="visible"/></p:to>
    ...
    if anim_info.get('kind') in {'motion', 'motion_rotate', 'motion_scale'}:
        path = anim_info.get('path', 'M 0 0 L 0.03000 0')
        repeat_attr = f' repeatCount="{anim_info["repeat"]}"' if anim_info.get('repeat') else ''
        auto_rev_attr = ' autoRev="1"' if anim_info.get('autoRev') else ''
        motion_xml = f'''
<p:animMotion origin="layout" path="{path}" pathEditMode="relative" rAng="0">
  <p:cBhvr>
    <p:cTn id="{eff_id}" dur="{duration_ms}"{repeat_attr}{auto_rev_attr} fill="hold"/>
    <p:tgtEl><p:spTgt spid="{shape_id}"/></p:tgtEl>
    <p:attrNameLst>
      ...
      <p:attrName>ppt_y</p:attrName>
        ...
        if anim_info.get('kind') == 'motion_rotate':
            motion_xml += f'''
<p:animRot by="{int(anim_info.get('rotateBy', -21600000))}">
  <p:cBhvr>
    <p:cTn id="{eff_id + 1000}" dur="{duration_ms}"{repeat_attr} autoRev="1" fill="hold"/>
    <p:tgtEl><p:spTgt spid="{shape_id}"/></p:tgtEl>
  </p:cBhvr>
</p:animRot>'''
        elif anim_info.get('kind') == 'motion_scale':
            scale_to = int(anim_info.get('scaleTo', 112000))
            motion_xml += f'''
<p:animScale>
  <p:cBhvr>
    <p:cTn id="{eff_id + 1000}" dur="{duration_ms}"{repeat_attr} autoRev="1" fill="hold"/>
  ...
  <p:to x="{scale_to}" y="{scale_to}"/>
</p:animScale>'''
        return set_block + motion_xml
    if anim_info['filter'] is None:
        return set_block
    return set_block + f'''
